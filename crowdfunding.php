...
if ($_REQUEST['act'] == 'detail') {
    ...
    if (empty($zhongchou)) {
        ecs_header('location:crowdfunding.php\n');
    }
...
if ($_REQUEST['act'] == 'done') {
    ...
    $sql = " SELECT COUNT(order_id) FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id = '$_SESSION[user_id]' AND is_zc_order = 1 AND zc_goods_id = '$_POST[gid]' AND is_delete = 0 AND pay_status = 0 AND order_status !=2 ";
...
function get_order_confirm_address($address_id) {
    ...
    $row = $GLOBALS['db']->getRow($sql);
    $row['province_name'] = get_goods_region_name($row['province']);
    $row['city_name'] = get_goods_region_name($row['city']);
    $row['district_name'] = get_goods_region_name($row['district']);
    $row['street_name'] = get_goods_region_name($row['street']);
    $row['address'] = $row['province_name'].' '.$row['city_name'].' '.$row['district_name'].' '.$row['street_name'].' '.$row['address'];
