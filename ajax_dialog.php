...
if ($_REQUEST['act'] == 'shipping_type') {
    ...
    /* 对商品信息赋值 */
    $cart_goods = cart_goods($flow_type, $_SESSION['cart_value'], 0, $warehouse_id, $area_id); // 取得商品列表，计算合计
...
elseif ($_REQUEST['act'] == 'edit_invoice')
 {
    ...
    if (empty($cart_goods) && empty($from) || !check_consignee_info($consignee, $flow_type) && empty($from)) {
        $result['error'] = 1;
		$result['content'] = $_LANG['cart_and_info_null'];
    ...
    if(empty($user_id) || empty($inv_payee)){
        ...
        if(empty($invoice_id)){
            ...
            if(!$GLOBALS['db']->getOne($sql)){
				...
				$result['error'] = 1;
				$result['msg'] = $_LANG['invoice_top_exists'];
			}
        }else{
...
elseif ($_REQUEST['act'] == 'gotoInvoice')
{
    ...
    else
    {
        ...
        $result['invoice_type'] = $_LANG['invoice_ordinary'];
        if ($invoice_type) {
            $result['type'] = 1;
            $result['invoice_type'] = $_LANG['need_invoice'][1];
        }
...
/*------------------------------------------------------ */
    ...
    if (count($id) > 1) {
        ...
        $record_count = $db->getAll("SELECT oi.order_id FROM " . $ecs->table('order_info') . " as oi" .
                " left join " . $ecs->table('order_goods') . " as og on oi.order_id = og.order_id" .
                $left_join .
                " WHERE oi.user_id = '$user_id' and oi.is_delete = '$type' AND oi.extension_code = 'auction' " .
                " and (select count(*) from " . $GLOBALS['ecs']->table('order_info') . " as oi_2 where oi_2.main_order_id = oi.order_id) = 0 " . //主订单下有子订单时，则主订单不显示
                $where . " group by oi.order_id");
        $record_count = count($record_count);
    } else {
...
/*------------------------------------------------------ */
    ...
    if($id){
        ...
        $store_shop_list = get_store_shop_list($libType, $keywords, $count, $size, $page, $sort, $order, $region_id, $area_id, $area_city, $store_province, $store_city, $store_district, $store_user);  
...
/*------------------------------------------------------ */
    ...
    $store_id = isset($_REQUEST['store_id']) && !empty($_REQUEST['store_id'])  ? intval($_REQUEST['store_id']) : 0;//门店id
    $store_seller = isset($_REQUEST['store_seller']) && !empty($_REQUEST['store_seller'])  ? $_REQUEST['store_seller'] : '';//门店id
    
    if (!empty($rec_number)) {
        $cart_value = get_sc_str_replace($_SESSION['cart_value'], $rec_number, 1);

        /* 对商品信息赋值 */
        $cart_goods_list = cart_goods($flow_type, $rec_number, 1, $warehouse_id, $area_id, $area_city); // 取得商品列表，计算合计 
...
/*------------------------------------------------------ */
    ...
    $area_city = !empty($_REQUEST['area_city']) ? intval($_REQUEST['area_city']) : 0;
    ...
    if ($shipping_prompt) {
        ...
        /* 对商品信息赋值 */
        $cart_goods_list = cart_goods($flow_type, $shipping_prompt, 1, $warehouse_id, $area_id, $area_city); // 取得商品列表，计算合计 
        $cart_goods_list_new = cart_by_favourable($cart_goods_list);
        $GLOBALS['smarty']->assign('goods_list', $cart_goods_list_new);
        $GLOBALS['smarty']->assign('cart_value', $cart_value);
        $GLOBALS['smarty']->assign('store_seller', $store_seller);
...
/*------------------------------------------------------ */
    ...
    if ($rec_id == 0 || $g_id == 0) {
        ...
        $properties = get_goods_properties($g_id, $order_goods['warehouse_id'], $order_goods['area_id'], $order_goods['area_city']);  // 获得商品的规格和属性	
        $spec = $properties['spe'];
...
/*------------------------------------------------------ */
    ...
    $area_city = !empty($area->area_city) ? intval($area->area_city) : 0;
    ...
    $GLOBALS['smarty']->assign('area_city',  $area_city); //地区ID 
    ...
    if(!empty($seller_store)){
        foreach($seller_store as $k=>$v){
            if(is_spec($is_spec) == true){
                $products = get_warehouse_id_attr_number($v['goods_id'],$spec_arr, $v['ru_id'], 0, 0, 0, '',$v['id']);//获取属性库存
...
//-- 首页文章列表检测
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'checked_article_cat') {
    $result = array('error' => 0, 'content' => '', 'message' => '');
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'guess_goods')
{
    ...
    $guess_goods = get_guess_goods($user_id, 1, $page, 7, $warehouse_id, $area_id, $area_city);
...
elseif ($_REQUEST['act'] == 'flow_shipping') {
                ...
                if ($list && $list[3]) {
                    ...
                    if ($list[2] == 2) {
                        ...
                        if($shipping_code == 'cac'){
                            ...
                            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('shipping_point') . " AS sp " .
    ...
    if($rec_id){
        ...
        /* 对商品信息赋值 */
        $cart_goods_list = cart_goods($flow_type, $rec_id, 1, $warehouse_id, $area_id, $area_city); // 取得商品列表，计算合计 
        $cart_goods_list_new = cart_by_favourable($cart_goods_list);
        $GLOBALS['smarty']->assign('goods_list', $cart_goods_list_new);
        $GLOBALS['smarty']->assign('cart_value', $cart_value);
        $GLOBALS['smarty']->assign('store_seller', $store_seller);
...
elseif ($_REQUEST['act'] == 'see_more_goods') {
    ...
    if ($see_more_goods === false) {
        ...
        $see_more_goods = get_filter_goods_list(array('cat_ids' => $top_cat['cat_id']), 5, 1, "click_count", "DESC", $warehouse_id, $area_id, $area_city, 'goods');
...
elseif ($_REQUEST['act'] == 'guess_goods_love') {
    ...
    $guess_goods = get_guess_goods($user_id, 1, 1, 7, $warehouse_id, $area_id, $area_city);
...
//-- 加载首页弹出广告
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'ajax_Homeindex_Bonusadv') {
...
//-- 更新首页排行商品
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'checked_home_rank'){
...
//-- 检测客服
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'check_kefu') {
...
//-- 批发-改变发票的设置
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_wholesale_invoice')
 {
    ...
    /* 对商品信息赋值 */
    $cart_goods = wholesale_cart_goods(0, $_SESSION['cart_value']); // 取得商品列表，计算合计
...
//-- 批发-修改并保存发票的设置
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'wholesale_gotoInvoice')
{
    require_once('includes/lib_wholesale.php');
    require_once('includes/lib_suppliers.php');
