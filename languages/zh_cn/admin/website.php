<?php 

// 插件语言包
/*
**/

$_LANG['website_name'] = '插件名称';
$_LANG['website_qq'] = '作者QQ';
$_LANG['website_author'] = '作者';
$_LANG['website_email'] = '邮箱地址';
$_LANG['ur_here'] = '第三方登录插件';
$_LANG['ur_view']  =  '查看第三方插件';
$_LANG['ur_install'] =  '安装第三方插件';
$_LANG['webstte_list'] = '插件列表';
$_LANG['user_rank'] = '用户等级名称';
$_LANG['yes_install'] = '安装完成';
$_LANG['yes_update'] = '修改插件成功';
$_LANG['version'] = '版本';

$_LANG['yes_uninstall'] = '卸载成功';
$_LANG['no_uninstall'] = '卸载失败,找不到文件';
$_LANG['yes_init'] = '初始化成功';
$_LANG['init'] = '初始化插件';
$_LANG['help_app_key'] = '第三方网站申请的APP_KEY , 可能有的名称不同， 如不同，请填写<申请时所得的最短的一行>';
$_LANG['help_app_secret'] = '第三方网站申请的APP_SECRET ，可能有的名称不同，如不同，请填写<申请时所得的最长的一行>';
$_LANG['help_rank_name'] = '所属的用户组名称, 如果是 QQ\支付宝 插件,如 QQ\支付宝用户';
$_LANG['update_time'] = '更新日期';
$_LANG['website_web'] = '申请地址';
$_LANG['once'] = '立即申请';
$_LANG['batch_create'] = '批量生成';
$_LANG['warning'] = '提示:批量生成调用代码并复制到您的模版文件应用处';
$_LANG['warning2'] = '提示：代码已经生成成功,请先复制此 {insert_scripts files="website.js"} 段代码到您的模版头部';


$_LANG['is_show_name'] = '显示登录名';
$_LANG['is_show_title'] = '显示窗口标题';
$_LANG['js_languages']['confrim_uninstall'] = '您确定批量卸载？';
$_LANG['batch_yes'] = '批量处理成功';
$_LANG['qita'] = '其他登录方式';

$_LANG['is_show_help'] = '显示气泡';
$_LANG['is_open'] = '弹出窗';
$_LANG['login'] = '登录';

$_LANG['eavl'] = '代码执行结果';

$_LANG['copycode'] = '复制代码';
$_LANG['help_evaljavascript'] = '请复制此段代码到您的模版任意位置，放置前须知。 <br> 您必须先复制 {insert_scripts files="website.js"} 这段代码到此段代码的前面执行一次！否则后果自负';

/* 教程名称 */
$_LANG['tutorials_bonus_list_one'] = '第三方登录插件（QQ互联）申请流程';

/* 页面顶部操作提示 */
$_LANG['operation_prompt_content']['list'][0] = '列表页展示了所有的登录插件名称。';
$_LANG['operation_prompt_content']['list'][1] = '安装插件时需到各第三方应用中申请ID和KEY方可使用。';

$_LANG['operation_prompt_content']['install'][0] = '填写在第三方申请的信息参数。';

?>