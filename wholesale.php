...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 搜索类别 */
    if ($search_category) {
        $where .= " AND " . get_children($search_category); //by wu
    ...
    $smarty->assign('have', $have); // 进货单里的商品ID by yanxin
...
{
    $list = array();
    $sql = "SELECT w.*, g.goods_thumb, g.goods_name as goods_name,g.shop_price,market_price " .
            "FROM " . $GLOBALS['ecs']->table('wholesale') . " AS w, " .
                      $GLOBALS['ecs']->table('goods') . " AS g " . $where .
            " AND w.goods_id = g.goods_id AND w.review_status = 3 ";
