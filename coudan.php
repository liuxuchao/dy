...
{
    $position = assign_ur_here(0, $_LANG['shopping_list']);
    $smarty->assign('page_title',       $position['title']);    // 页面标题

	assign_template();
	$categories_pro = get_category_tree_leve_one();
	$smarty->assign('categories_pro',  $categories_pro); // 分类树加强版
	$smarty->assign('guess_goods',     get_guess_goods($_SESSION['user_id'], 1, 1, 7,$region_id, $area_id, $area_city)); //猜你喜欢
    ...
    // 查询活动包含的所有商品
    $favourable_goods_list = favourable_goods_list($_SESSION['user_rank'], $active_id, $sort, $order, $size, $page,$region_id, $area_info['region_id'], $area_city);
    ...
    $smarty->assign('cart_fav_num', $cart_fav_num); // 同一优惠活动添加到购物车的商品数量
...
elseif ($_REQUEST['act']== 'ajax_update_cart')
{
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $warehouse_id, $area_id, $area_city, -1, 0, '', '', $goods->active_id)) //ecmoban模板堂 --zhuo $goods->warehouse_id
        {
...
// 取得当前活动 已经加入购物车的商品
function cart_favourable_goods($user_rank, $favourable_id, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    $user_rank = ',' . $user_rank . ',';
    $now = gmtime();
    ...
    if (!empty($selected)) {
        if (defined('THEME_EXTENSION')) {
            switch ($selected['act_type']) {
                case 0:
                $act_type_txt = $GLOBALS['_LANG']['With_a_gift']. $GLOBALS['_LANG']['man'] .$selected['min_amount']. $GLOBALS['_LANG']['change_purchase_gift'];
                    break;
                case 1:
                    $act_type_txt = $GLOBALS['_LANG']['Full_reduction']. $GLOBALS['_LANG']['man'] .$selected['min_amount']. $GLOBALS['_LANG']['reduction_gift'].$selected['act_type_ext'].$GLOBALS['_LANG']['yuan'];
                    break;
                case 2:
                    $act_type_txt = $GLOBALS['_LANG']['discount']. $GLOBALS['_LANG']['man'] .$selected['min_amount']. $GLOBALS['_LANG']['discount_gift'];
                    break;
