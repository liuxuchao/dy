...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    get_del_batch('', $card_id, array('card_img'), 'card_id', 'card', 0, DATA_DIR . '/cardimg/'); //删除图片
...
function cards_list($ru_id)
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT card_id, card_name, card_img, card_fee, free_money, card_desc, user_id".
               " FROM ".$GLOBALS['ecs']->table('card'). $where .
    ...
    foreach($card_list as $key=>$row){
        $arr[$key] = $row;
        $arr[$key]['ru_name'] = get_shop_name($row['user_id'], 1); //ecmoban模板堂 --zhuo
    }
