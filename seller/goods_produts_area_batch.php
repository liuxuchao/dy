...
if ($_REQUEST['act'] == 'add') {
    ...
    $area_id = isset($_REQUEST['area_id']) ? intval($_REQUEST['area_id']) : 0; //地区
    $city_id = isset($_REQUEST['city_id']) ? intval($_REQUEST['city_id']) : 0; //城市

    if ($goods_id > 0) {
        $smarty->assign('action_link', array('text' => $_LANG['back_to_goods_detail'], 'href' => 'goods.php?act=product_list&goods_id=' . $goods_id));
    }
...
elseif ($_REQUEST['act'] == 'upload') {
    ...
    if ($_FILES['file']['name']) {
        ...
        $attr_names = array_slice($attr_names, 7, $end);
...
elseif ($_REQUEST['act'] == 'ajax_insert') {
    ...
    if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
        ...
        $other['city_id'] = $result['list']['city_id'];
        ...
        if (!empty($result['list']['goods_attr'])) {
            ...
            //查询数据是否已经存在;
            $sql = "SELECT product_id FROM " . $GLOBALS['ecs']->table('products_area') . " WHERE goods_id = '" . $result['list']['goods_id'] . "'" .
                    " AND area_id = '" . $result['list']['area_id'] . "'" . 
                    " AND city_id = '" . $result['list']['city_id'] . "'" . 
                    $where;
            $product_id = $GLOBALS['db']->getOne($sql);
            
            if ($product_id) {
                $db->autoExecute($ecs->table('products_area'), $other, 'UPDATE', "product_id = '$product_id'");
                ...
                if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                    ...
                    $db->autoExecute($ecs->table('goods'), $goods_other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND product_id = '$product_id' AND product_table = 'products_area'");
                }

                $result['status_lang'] = '<span style="color: red;">'.$_LANG['updated_data_success'].'</span>';
            } else {
                ...
                if($product_id){
                    $result['status_lang'] = '<span style="color: red;">'.$_LANG['added_data_success'].'</span>';
                }else{
                    $result['status_lang'] = '<span style="color: red;">'.$_LANG['added_data_fail'].'</span>';
                }
            }
        }
    }
...
elseif ($_REQUEST['act'] == 'download') {
    ...
    $goods_id = isset($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
    $model = isset($_REQUEST['model']) ? intval($_REQUEST['model']) : 0;
    $area_id = isset($_REQUEST['area_id']) ? intval($_REQUEST['area_id']) : 0; //当前地区;
    $city_id = isset($_REQUEST['city_id']) ? intval($_REQUEST['city_id']) : 0; //当前地区城市;
    $goods_attr = isset($_REQUEST['goods_attr']) ? explode(',', $_REQUEST['goods_attr']) : array();
    
    // 文件标签
    // Header("Content-type: application/octet-stream");
    header("Content-type: application/vnd.ms-excel; charset=utf-8");
    Header("Content-Disposition: attachment; filename=goods_produts_area_batch" .$goods_id ."_" ."$area_id" ."_" ."$city_id". ".csv");

    // 下载
    if ($_GET['charset'] != $_CFG['lang']) {
        $lang_file = '../languages/' . $_GET['charset'] . '/' . ADMIN_PATH . '/goods_produts_area_batch.php';
    ...
    if (isset($_LANG['upload_product'])) {
        ...
        if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw') {
            ...
            $city_name = get_table_date('region_warehouse', "region_id = '$city_id'", array('region_name'), 2);

            $attr_info = get_list_download($goods_info['goods_sn'], array($region_name, $city_name), $_attribute, count($_attribute), $model);
    ...
    if (count($warehouse_info) > 0 && $model == 2) {
        ...
        if ($attr) {
            ...
            foreach ($res as $k => $v) {
                ...
                $arr[$k]['city_name'] = $warehouse_info[1];
                $arr[$k]['attr_value'] = $v;
