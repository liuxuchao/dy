...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (admin_priv('gift_gard_manage')) {
        $tab_menu[] = array('curr' => 1, 'text' => $_LANG['gift_gard_type_list'], 'href' => 'gift_gard.php?act=list');
    }
    if (admin_priv('take_manage')) {
        $tab_menu[] = array('curr' => 0, 'text' => $_LANG['take_list'], 'href' => 'gift_gard.php?act=take_list');
    }
    ...
    $smarty->assign('action_link', array('text' => $_LANG['gift_gard_type_add'], 'href' => 'gift_gard.php?act=add', 'class' => 'icon-plus'));
...
if($_REQUEST['act'] == 'handle_log'){
    $smarty->assign('menu_select', array('action'=>'02_promotion', 'current'=>'take_list'));
     /* 权限判断 */
    admin_priv('gift_gard_manage');
    $smarty->assign('primary_cat',     $_LANG['02_promotion']);
    $smarty->assign('ur_here',     $_LANG['handle_log']);
    $smarty->assign('action_link', array('text' => $_LANG['take_list'], 'href' => 'gift_gard.php?act=take_list', 'class' => 'icon-reply'));
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $bonus_arr = $db->getRow("SELECT ru_id FROM " .$ecs->table('gift_gard_type'). " WHERE gift_id = '$id'");
...
if ($_REQUEST['act'] == 'add')
{
    admin_priv('gift_gard_manage');
    $smarty->assign('primary_cat',     $_LANG['02_promotion']);
    $smarty->assign('menu_select', array('action'=>'02_promotion', 'current'=>'gift_gard_list'));
    
    $smarty->assign('controller','card_add');
    
    $smarty->assign('lang',         $_LANG);
    $smarty->assign('ur_here',      $_LANG['gift_gard_type_add']);
    $smarty->assign('action_link',  array('href' => 'gift_gard.php?act=list', 'text' => $_LANG['gift_gard_type_list'], 'class' => 'icon-reply'));
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('href' => 'gift_gard.php?act=list&' . list_link_postfix(), 'text' => $_LANG['gift_gard_type_list'], 'class' => 'icon-reply'));
...
if ($_REQUEST['act'] == 'send') {
    ...
    $smarty->assign('ur_here', $_LANG['gift_from_goods']);
...
if ($_REQUEST['act'] == 'bonus_list')
{
    ...
    $smarty->assign('record_count', $list['record_count']);
    $smarty->assign('page_count',   $list['page_count']);
    $smarty->assign('action_link2',   array('href' => 'gift_gard.php?act=gift_add&bonus_type=' . intval($_REQUEST['bonus_type']), 'text' => $_LANG['gift_gard_add'], 'class' => 'icon-plus'));
    $smarty->assign('action_link3',   array('href' => 'gift_gard.php?act=export_gift_gard&bonus_type=' . intval($_REQUEST['bonus_type']), 'text' => $_LANG['gift_gard_export'], 'class' => 'icon-download-alt'));
...
if ($_REQUEST['act'] == 'take_list')
{
	...
	$tab_menu = array();
		if(admin_priv('gift_gard_manage')){
			$tab_menu[] = array('curr' => 0, 'text' => $_LANG['gift_gard_type_list'], 'href' => 'gift_gard.php?act=list');	
		}
		if(admin_priv('take_manage')){
			$tab_menu[] = array('curr' => 1, 'text' => $_LANG['take_list'], 'href' => 'gift_gard.php?act=take_list');			
		}
	$smarty->assign('tab_menu', $tab_menu);
	//页面分菜单 by wu end	
	...
	$smarty->assign('full_page',    1);
	$smarty->assign('ur_here',      $_LANG['gift_gard_list']);
	$smarty->assign('action_link',   array('href' => 'gift_gard.php?act=take_excel', 'text' => '导出订单', 'class' => 'icon-download-alt'));
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        elseif (isset($_POST['configure_goods']))
        {
        	...
        	$smarty->assign('primary_cat',     $_LANG['02_promotion']);
        	$smarty->assign('ur_here',      $_LANG['gift_from_goods']);
        	$smarty->assign('action_link',  array('href' => 'gift_gard.php?act=bonus_list&bonus_type=' . $bonus_type_id, 'text' => $_LANG['gift_gard_list'], 'class' => 'icon-reply'));
