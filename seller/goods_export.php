...
if ($_REQUEST['act'] == 'goods_export')
{
	...
	if(admin_priv('goods_export')){
		$tab_menu[] = array('curr' => 1, 'text' => $_LANG['14_goods_export'], 'href' => 'goods_export.php?act=goods_export');
	}
	$smarty->assign('tab_menu', $tab_menu);
	//页面分菜单 by wu end	
	
	$smarty->assign('current','goods_export');
...
elseif($_REQUEST['act'] == 'act_export_ecshop')
{
    ...
    $sql = "SELECT count(*) FROM " . $ecs->table('goods') . " AS g LEFT JOIN " . $ecs->table('merchants_shop_brand') . " AS b " .
    ...
    if ($result['page'] <= ceil($count / $result['page_size']))
    {
        ...
        $sql = "SELECT g.* FROM " . $ecs->table('goods') ." AS g ". $where;  
        ...
        {
            //品牌
            $sql = "SELECT b.brand_name as brandname " .
               " FROM " . $ecs->table('goods') . " AS g LEFT JOIN " . $ecs->table('brand') . " AS b " .
               "ON g.brand_id = b.brand_id where g.goods_id = '".$row['goods_id']."' "; 
            $brand_name = $GLOBALS['db']->getOne($sql); 

            $goods_value['goods_name'] = '"' . $row['goods_name'] . '"';
            $goods_value['goods_sn'] = '"' . $row['goods_sn'] . '"';
            $goods_value['brand_name'] = '"' . $brand_name . '"';
...
elseif($_REQUEST['act'] == 'act_export_step_search')
{
    ...
    $where = $arr['where'];
    $page_size = 50; // 默认50张/页
    $sql = "SELECT count(*) FROM " . $ecs->table('goods') . " AS g LEFT JOIN " . $ecs->table('brand') . " AS b " .
    ...
    {
        $title = '商品管理数据导出';
        $result = array('error' => 0, 'mark' => 0, 'message' => '', 'content' => '','done' => 1, 'title' => $title, 'page_size' => $page_size,
    ...
    else
    {
        ...
        $result['row']     = array('new_page'  => sprintf($_LANG['page_format'], 1),
                           'new_total' => sprintf($_LANG['total_format'], ceil($count/$page_size)),
                           'new_time'  => $_LANG['wait'],
                           'cur_id'    => 'time_1');
        ...
        {
            $result['mark'] = 1;
            $result['content'] = $_LANG['download_complete'];
            die($json->encode($result));
        }
    }
}
