...
{
    return array(
        'store_best'    => $GLOBALS['_LANG']['store_best'],
        'store_new'     => $GLOBALS['_LANG']['store_new'],
        'store_hot'     => $GLOBALS['_LANG']['store_hot'],
...
function handle_goods_area($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('link_area_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND ru_id = (SELECT ru_id FROM " . $GLOBALS['ecs']->table('admin_user') . "  WHERE user_id = '$_SESSION[seller_id]') ";
    $GLOBALS['db']->query($sql);
}
...
function delete_goods($goods_id)
{
    ...
    /* 删除商品图片和轮播图片文件 */
    $sql = "SELECT goods_id, goods_thumb, goods_img, original_img, goods_video " .
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        if($filter['cat_type'] == 'seller'){
            ...
            if ($filter['cat_id'] > 0) {
                ...
                $extension_goods_array = $extension_goods_array ? array_unique($extension_goods_array) : [];
	...
	/* 审核商品状态 ecmoban模板堂 --zhuo */
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
                " WHERE $where AND g.is_delete = '$is_delete'";
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, " .
                "g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, g.commission_rate, " .
                "g.is_promote, g.model_price, g.model_inventory, g.model_attr, g.review_status, g.review_content, g.store_best, " .
                "g.store_new , g.store_hot , g.is_real, g.is_shipping, g.stages,g.goods_thumb, add_time, freight, tid, " .
                "g.is_alone_sale, g.is_xiangou, g.promote_end_date, g.xiangou_end_date, g.bar_code " . $select .
