...
$smarty->assign('menu_select', array('action' => '02_cat_and_goods', 'current' => 'area_batch'));
/* ------------------------------------------------------ */
//-- 批量上传
/* ------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'add') {
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->assign('current', 'area_batch');
...
elseif ($_REQUEST['act'] == 'upload') {
    ...
    if ($_FILES['file']['name']) {
        ...
        $arr = array();
        $goods_list = array();
        $field_list = array_keys($_LANG['upload_area']); // 字段列表
