...
elseif ($_REQUEST['act'] == 'add_albun_pic')
{
    ...
    /*检查是否重复*/
        ...
        {
            $result['error'] = 0;
            $result['content'] = $_LANG['album_exist'][0].$album_mame.$_LANG['album_exist'][1];
            die(json_encode($result));
        }
         /* 取得文件地址 */
        $file_url = '';
        if ((isset($_FILES['album_cover']['error']) && $_FILES['album_cover']['error'] == 0) || (!isset($_FILES['album_cover']['error']) && isset($_FILES['album_cover']['tmp_name']) && $_FILES['album_cover']['tmp_name'] != 'none'))
        {
            ...
            {
                $result['error'] = 0;
                $result['content'] = $_LANG['album_cover_format_reupload'];
                die(json_encode($result));
            }
...
elseif ($_REQUEST['act'] == 'upload_img') {
    ...
    if (isset($_POST)) {
        ...
        if (empty($name)) {
            $result['error'] = $_LANG['no_select_album_yet'];
        }
        $type = strtolower(substr(strrchr($name, '.'), 1)); //获取文件类型		
        if (!in_array($type, $typeArr)) {
            $result['error'] = $_LANG['please_upload_jpg_gif_png'];
        }
    }
    ...
    if ($upload_status) { //临时文件转移到目标文件夹
        ...
        $result['error'] = $_LANG['upload_error_check_server_config'];
    }
    die(json_encode($result));
}
			...
			foreach($goods_ids as $key=>$val){
				...
				if (!$db->getOne($sql)) {
					...
					$sql = "INSERT INTO " . $ecs->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id, group_id) " .
							"VALUES ('$goods_id', '$val', '$price_goods', '$_SESSION[seller_id]', '$group_id')"; //by mike add
					$db->query($sql, 'SILENT');
				}
			}
		}
...
/*编辑导航名称*/
elseif($_REQUEST['act'] == 'edit_navname'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    ...
    if($id > 0 && !empty($nav_name)){
        ...
        {
            $result['error'] = 0;
            $result['content'] = $_LANG['nav_exist_nav'].$nav_name.$_LANG['album_exist'][1];
        }else{
            $sql = "UPDATE".$ecs->table('merchants_nav')." SET name = '$nav_name' WHERE id = '$id' AND ru_id = ".$adminru['ru_id'];
            $db->query($sql);
            $result['error'] = 1;
            $result['content'] = $_LANG['edit_success'];
        }
    }else{
        $result['error'] = 0;
        $result['content'] = $_LANG['nav_not_exist_or_empty'];
    }
    die(json_encode($result));
}
/*编辑导航链接*/
elseif($_REQUEST['act'] == 'edit_navurl'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    ...
    if($id > 0){
        ...
        $result['error'] = 0;
        $result['content'] = $_LANG['js_languages']['jl_navbar_no_exist'];
    }
    die(json_encode($result));
}
/*编辑导航排序*/
elseif($_REQUEST['act'] == 'edit_navvieworder'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    ...
    if($id > 0){
        if(preg_match('/^\d+$/i', $order)){
            ...
            $result['error'] = 0;
            $result['content'] = $_LANG['sort_must_number'];
        }
    }else{
        $result['error'] = 0;
        $result['content'] = $_LANG['js_languages']['jl_navbar_no_exist'];
...
/*编辑导航排序*/
elseif($_REQUEST['act'] == 'remove_nav'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    $id = isset($_REQUEST['id'])  ? intval($_REQUEST['id']) : 0;
    if($id > 0){
        ...
        $db->query($sql);
        $result['error'] = 1;
        $result['content'] = $_LANG['delete_success_alt'];
    }else{
        $result['error'] = 0;
        $result['content'] = $_LANG['js_languages']['jl_navbar_no_exist'];
    }
...
elseif($_REQUEST['act'] == 'add_nav'){
    ...
    if(!empty($name)){
        ...
        {
            $result['error'] = 0;
            $result['content'] = $_LANG['nav_exist_nav'].$name.$_LANG['album_exist'][1];
        }else{
        ...
        $result['error'] = 0;
        $result['content'] = $_LANG['nav_name_not_null'];
    }
...
/*切换导航显示*/
elseif($_REQUEST['act'] == 'edit_ifshow'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    $id = isset($_REQUEST['id'])  ? intval($_REQUEST['id']) : 0;
