...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $smarty->assign('page_count', $apply_list['page_count']);
    $smarty->assign('page_count_arr',   $page_count_arr);
    make_json_result($smarty->fetch('seller_apply.dwt'), '',
    ...
    if($is_confirm != 0){
       ...
       if($is_confirm == 1){
           ...
           /*判断是否已经存在等级，是则修改，不是则插入*/
            ...
            /*退款*/
            if($back_price > 0){
                log_account_change($ru_id, $apply['back_price'], 0, 0,0, $_LANG['record_id'].$apply['apply_sn'].$_LANG['seller_level_prepay_refund']);
            }
        }else{
            ...
            if($apply['pay_status'] == 1  && $apply['payable_amount'] > 0 && $is_confirm != 0){
                log_account_change($ru_id, $apply['payable_amount'], 0, 0,0, $_LANG['record_id'].$apply['apply_sn'].$_LANG['seller_level_not_pass_refund']);
            }
        }
        if($is_confirm != 0){
            $confirm_time=gmtime();
...
//    check_authz_json('seller_apply');
    /* 判断标准类型，如果上传文件，删除文件 */
    $entry_criteria = unserialize($db->getRow("SELECT entry_criteria FROM " . $ecs->table('seller_apply_info') . " WHERE apply_id = '$id' LIMIT 1"));//获取标准
