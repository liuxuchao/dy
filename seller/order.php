...
if ($_REQUEST['act'] == 'order_download') {
    header("Content-Disposition: attachment; filename=".$_LANG['order_export']."-".date('YmdHis').".zip");
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '') {
            ...
            if ($GLOBALS['_CFG']['sms_type'] == 0) {
                ...
                if ($result) {
                    ...
                    sys_msg($_LANG['dayu_sms_config_error'], 1);
                }
            }
        }
        ...
        if (file_exists($file)) {
            $pushData = [
                'first' => ['value' => $_LANG['you_order_delivery']], // 标题
                ...
                'remark' => ['value' => $_LANG['order_delivery_wait']]
            ];
 ...
 */ elseif ($_REQUEST['act'] == 'back_cause_list') {
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['10_back_order']);
    $smarty->assign('action_link', array('href' => 'order.php?act=add_return_cause', 'text' => $_LANG['add_return_reason']));
 ...
 */ elseif ($_REQUEST['act'] == 'add_return_cause') {
    ...
    /* 检查权限 */
    admin_priv('order_back_cause');
    $smarty->assign('action_link', array('href' => 'order.php?act=back_cause_list', 'text' => $_LANG['return_reason_list']));
 ...
 */ elseif ($_REQUEST['act'] == 'inser_cause') {
    ...
    if ($db->autoExecute($ecs->table('return_cause'), $cause) !== false) {
        ...
        $link[1]['text'] = $_LANG['return_list_page'];
 ...
 */ elseif ($_REQUEST['act'] == 'edit_cause_info') {
    ...
    if ($db->autoExecute($ecs->table('return_cause'), $cause, 'UPDATE', "cause_id='$c_id'") !== false) {
        ...
        $link[0]['text'] = $_LANG['back_return_list_page'];
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    elseif ('refund' == $func)
    {
        ...
        if($order['order_amount'] < 0 && $refund_amount > $refund_order_amount){
            ...
            sys_msg($_LANG['return_money_fail_tip'], 1, $link);
            exit;
        }
        ...
        if ($is_ok == 2 && $refund_type == 1) {
            ...
            sys_msg($_LANG['return_money_fail_account_fu'], 1, $links);
            exit;
        }
...
//        var_dump($order);die;
    ...
    elseif (isset($_POST['return']))
    {
        ...
        if ($ret_id > 0) {
            ...
            sys_msg($_LANG['order_have_return_cant_return'], 0, $links);
        } else {
    ...
    elseif ('refound' == $operation) {
        ...
        if ($order['pay_status'] != PS_UNPAYED) {
            ...
            if (!empty($_REQUEST['action_note'])) {
                ...
                if($is_ok == 2 && $refund_type == 1){
                    /* 提示信息 */
                    $links[] = array('href' => 'order.php?act=return_info&ret_id=' .$ret_id. '&rec_id=' . $return_info['rec_id'], 'text' => $_LANG['back_return_list_oprate']);
                    sys_msg($_LANG['return_money_fail_account_fu'], 1, $links);
                    exit;
                }
...
//         /*判断是否需要退还积分  如果需要 则跟新退还日志   by kong*/
        if ($_REQUEST['refound_pay_points'] > 0) {
            log_account_change($order['user_id'], 0, 0, 0, $_REQUEST['refound_pay_points'], $_LANG['order_refund_return_order_integral'][0] . $order['order_sn'] . $_LANG['order_refund_return_order_integral'][1]);
        }
...
function order_list($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.pay_status, o.order_amount, o.money_paid, o.is_delete," .
                ...
                "o.extension_id, o.user_id, o.referer, o.froms, o.chargeoff_status, o.confirm_take_time, o.pay_id, o.pay_name, o.shipping_id, o.shipping_name, o.goods_amount, " .
                "(" . order_amount_field('o.') . ") AS total_fee, (o.goods_amount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee - o.discount) AS total_fee_order, o.pay_id " .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        if ($value['shipping_status'] == SS_RECEIVED && empty($value['confirm_take_time'])) {
            ...
            if (empty($confirm_take_time)) {
                ...
                order_action($value['order_sn'], $value['order_status'], $value['shipping_status'], $value['pay_status'], $note, $_SESSION['seller_name'], 0, $confirm_take_time);
        ...
        if ($value['extension_code'] == 'group_buy') {
            ...
            if ($status == 0) {
                $row[$key]['cur_status'] = $_LANG['no_start'];
            } elseif ($status == 1) {
                $row[$key]['cur_status'] = $_LANG['ongoing'];
            } elseif ($status == 2) {
                $row[$key]['cur_status'] = $_LANG['ended_to_group_check_page'];
            } elseif ($status == 3) {
                $row[$key]['cur_status'] = $_LANG['group_success'];
            } else {
                $row[$key]['cur_status'] = $_LANG['group_fail'];
            }
        }
...
function get_order_goods($order)
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.model_inventory, g.model_attr AS model_attr, g.suppliers_id AS suppliers_id, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, " .
            "g.goods_thumb, oi.order_sn,oi.extension_code as oi_extension_code,oi.extension_id " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS o ".
            "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            "LEFT JOIN " . $GLOBALS['ecs']->table('brand') . " AS b ON g.brand_id = b.brand_id " .
...
function download_orderlist($result) {
    ...
    if(empty($result)) {
        return i($GLOBALS['_LANG']['no_data_match_condition']);
    }

    $data = i($GLOBALS['_LANG']['order_php_download_orderlist']."\n");
    $count = count($result);
    for ($i = 0; $i < $count; $i++) {
        ...
        $order_status = i(preg_replace("/\<.+?\>/", "", $GLOBALS['_LANG']['os'][$result[$i]['order_status']])); //去除标签
