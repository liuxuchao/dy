...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    $smarty->assign('seller_stages', $_CFG['seller_stages']); //商家设置商品分期
...
elseif ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
    ...
    if ($goods_model == 0) {
        ...
        $model_name = $_LANG['warehouse'];
    } elseif ($goods_model == 2) {
        $model_name = $_LANG['area'];
    }
        ...
        foreach ($attr_group as $key => $val) {
            ...
            if (!empty($product_info)) {
                ...
                if($product_info){
                    ...
                    $group = insert_attr_changelog($goods_id, $attr_info, $goods_model, $region_id, $city_id);//录入新的零时表数据，且取出
                }
                $group['changelog'] = 1;
            }
...
elseif ($_REQUEST['act'] == 'edit_link_desc')
{
    ...
    $smarty->assign('primary_cat', $_LANG['02_cat_and_goods']);
    $smarty->assign('menu_select', array('action' => '02_cat_and_goods', 'current' => '01_goods_list'));
    $smarty->assign('ur_here', $_LANG['intro_edit']);
...
elseif ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc')
{
    ...
    if(!empty($desc_name)){
        ...
        if($id > 0){
           ...
           $link_cnt = $_LANG['edit_success']; 
        }else{
           $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('link_goods_desc'), $other, "INSERT");
           $id = $GLOBALS['db']->insert_id();
           $link_cnt = $_LANG['add_success']; 
        }
        
    }else{
        $link_cnt = $_LANG['intro_name_not_null'];
    }
    ...
    $link[1] = array('text' => $_LANG['add_related_goods_intro'], 'href' => "goods.php?act=add_desc");
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if($GLOBALS['_CFG']['review_goods'] == 0){
        ...
        if($adminru['ru_id'] > 0){
            ...
            $review_goods = $db->getOne($sql, true); //判断
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, goods_video, bar_code, " .
        ...
        foreach ($notice_list as $key => $val) {
            ...
            if ($shop_price <= $val['hopeDiscount'] && $val['cellphone'] && $_CFG['sms_price_notice'] == '1') {
                ...
                $smsParams = array(
                    ...
                    'goodsname' => sub_str($goods_name, 20),
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        if ($_POST['type'] == 'trash')
        {
            ...
            if ($seckill) {
                sys_msg($_LANG['del_goods_sn'] .$seckill. $_LANG['del_seckill'], 0); // 秒杀
            }
...
elseif ($_REQUEST['act'] == 'edit_goods_name')
{
    ...
    if($old_goods_name != $goods_name){
        ...
        if ($exc->edit("goods_name = '$goods_name', review_status = '$review_status', last_update=" .gmtime(), $goods_id))
        {
            clear_cache_files();
 ...
 elseif ($_REQUEST['act'] == 'check_goods_sn') {
    ...
    if (!empty($goods_sn)) {
        ...
        if (!empty($goods_sn)) {
            ...
            $sql = "SELECT p.product_id FROM " . $ecs->table('products') . " AS p," .
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
    ...
    $old_goods_price = $exc->get_name($goods_id,'shop_price');

    if($goods_price != $old_goods_price){
        ...
        else
        {
            ...
            if ($exc->edit("shop_price = '$goods_price', market_price = '$price_rate', review_status = '$review_status', last_update=" .gmtime(), $goods_id))
            {
                clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_goods_number')
{
    ...
    if ($exc->edit("goods_number = '$goods_num', review_status = '$review_status', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'toggle_on_sale')
{
    ...
    $change = "is_on_sale = '$on_sale', last_update=" .gmtime();
    if($review_goods && $on_sale==1){
        $change .= ", review_status = 1";
    }
    if ($exc->edit($change, $goods_id))
    {
        // 下架后清理购物车中的此商品
        if ($on_sale == 0)
        {
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($adminru['ru_id'] > 0 && $adminru['ru_id'] != $goods['user_id']) {
        make_json_error($_LANG['illegal_operate_info_log']);
    }
    ...
    $seckill = is_seckill($goods_id);
    if ($seckill) {
        make_json_error($_LANG['del_goods_sn'] . $seckill . $_LANG['del_seckill'], 0); // 秒杀
    }
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    $sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, user_id, " .
                "goods_img, original_img, goods_video, goods_desc " .
...
elseif ($_REQUEST['act'] == 'add_group_goods')
{
    ...
    if($groupCount < 1000){
        ...
        $error = 1;
        $message = $_LANG['one_group_peijian_5_goods'];
    }
    ...
    if(!empty($keyword))
    {
            ...
            if($options){
                foreach($options AS $key=>$row){
                    $options[0]['cat_id'] = 0;
                    $options[0]['cat_name'] = $_LANG['all_category'];
                    $key += 1;
                    $options[$key] = $row;
                }
            }else{
                $res['error'] = 1;
                $res['message'] = $_LANG['no_search_cate'];
            }
    }
...
/*------------------------------------------------------ */
 elseif ($_REQUEST['act'] == 'addWarehouse') {
    ...
    if (empty($ware_name)) {
        $result['error'] = '1';
        $result['massege'] = $_LANG['js_languages']['jl_select_warehouse'];
    } else {
        ...
        if ($w_id > 0) {
            $result['error'] = '1';
            $result['massege'] = $_LANG['this_goods_warehouse_stock_exist'];
        } else {
            if ($ware_number == 0) {
                $result['error'] = '1';
                $result['massege'] = $_LANG['warehouse_stock_not_0'];
            } elseif ($ware_price == 0) {
                $result['error'] = '1';
                $result['massege'] = $_LANG['warehouse_price_not_0'];
            } else {
...
elseif ($_REQUEST['act'] == 'addRegion') {
    ...
    if (empty($area_name)) {
        $result['error'] = '1';
        $result['massege'] = $_LANG['js_languages']['jl_select_region'];
    } else {
        if ($region_number == 0) {
            $result['error'] = '1';
            $result['massege'] = $_LANG['region_stock_not_0'];
        } elseif ($region_price == 0) {
            $result['error'] = '1';
            $result['massege'] = $_LANG['region_price_not_0'];
        } else {
            $add_time = gmtime();
            ...
            if ($a_id > 0) {
                $result['error'] = '1';
                $result['massege'] = $_LANG['goods_region_price_exist'];
            } else {
...
elseif ($_REQUEST['act'] == 'scan_code')
{
	...
	if($data['status'] != 0){
		...
		//重量（用毛重）
		$goods_weight = 0;	
		if(strpos($data['result']['grossweight'], $_LANG['kilogram']) !== false){
			$goods_weight = floatval(str_replace($_LANG['kilogram'], '', $data['result']['grossweight']));
		}elseif(strpos($data['result']['grossweight'], $_LANG['gram']) !== false){
			$goods_weight = floatval(str_replace($_LANG['kilogram'], '', $data['result']['grossweight']))/1000;
		}
		//详情
		$goods_desc = "";
...
function get_order_goods_cout($goods_id = 0){
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('order_info'). " AS oi WHERE " . 
            "(SELECT order_id FROM " .$GLOBALS['ecs']->table('order_goods'). " AS og WHERE og.goods_id = '$goods_id' LIMIT 1) = oi.order_id";
