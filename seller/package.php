...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link',  array('text' => $_LANG['package_add'], 'href'=>'package.php?act=add', 'class' => 'icon-plus'));
    $smarty->assign('primary_cat',     $_LANG['02_promotion']);
    $packages = get_packagelist($adminru['ru_id']);
            
	//分页
	$page_count_arr = seller_page($packages,$_REQUEST['page']);
...
elseif ($_REQUEST['act'] == 'add') {
    ...
    $smarty->assign('primary_cat',     $_LANG['02_promotion']);
    $smarty->assign('menu_select', array('action' => '02_promotion', 'current' => '14_package_list'));

    /* 组合商品 */
    $group_goods_list = array();
    $sql = "DELETE FROM " . $ecs->table('package_goods') .
            " WHERE package_id = 0 AND admin_id = '$_SESSION[seller_id]'";
    ...
    $smarty->assign('action_link', array('text' => $_LANG['14_package_list'], 'href' => 'package.php?act=list', 'class' => 'icon-reply'));
    //分类列表 by wu
...
/*------------------------------------------------------ */
 elseif ($_REQUEST['act'] == 'edit') {
    ...
    $package_goods_list = get_package_goods($act_id, $_SESSION['seller_id'],1); // 礼包商品

    $smarty->assign('package', $package);
    $smarty->assign('ur_here', $_LANG['package_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['14_package_list'], 'href' => 'package.php?act=list&' . list_link_postfix(), 'class' => 'icon-reply'));
    //分类列表 by wu
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $package = get_package_info($id, "seller");
...
elseif ($_REQUEST['act'] == 'add_package_goods')
{
        ...
        if(!empty($goods_ids_arr)){
            foreach($goods_ids_arr as $v){
                ...
                if($goods_count == 0 || $type == 1){
                    ...
                    "VALUES ('$package_id', '" . $v . "', '$number', '$_SESSION[seller_id]')";
                    $db->query($sql, 'SILENT');
                }
            }
        }
    ...
    $arr = get_package_goods($package_id,$_SESSION[seller_id],1);
...
elseif ($_REQUEST['act'] == 'edit_package_nuber')
{
   include_once(ROOT_PATH . 'includes/cls_json.php');
    ...
    if($num > 0){
        ...
        $result['error'] = 1;
        $result['msg'] = $_LANG['goods_number_must_large_0_int'];
    }
...
elseif ($_REQUEST['act'] == 'edit_package_product')
{
   include_once(ROOT_PATH . 'includes/cls_json.php');
    ...
    if($product_id > 0){
        ...
        if($product_count > 0){
            $result['error'] = 1;
            $result['msg'] = $_LANG['goods_attr_exist_reselect'];
        }else{
             $sql = "UPDATE" .$ecs->table('package_goods') .
        ...
        $result['error'] = 1;
        $result['msg'] = $_LANG['system_error_refresh_retry'];
    }
