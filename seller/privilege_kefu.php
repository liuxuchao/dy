...
{
    /* 模板赋值 */
    $smarty->assign('ur_here',$_LANG['kefu_list']);
...
elseif ($_REQUEST['act'] == 'remove'){
    ...
    $res = $db->getRow($sql);
    if(!$res){
        make_json_error($_LANG['kefu_not_exist']);
    }
    if($res['chat_status'] == 1){
        make_json_error($_LANG['kefu_in_login_cant_delete']);
    }
    ...
    $smarty->assign('record_count',    $services['record_count']);
    $smarty->assign('page_count',      $services['page_count']);
    make_json_result($smarty->fetch('privilege_kefu_list.dwt'), $_LANG['delete_success_alt']);
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here', $_LANG['services_list']); // 当前导航
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if(empty($services_name)){
        sys_msg($_LANG['input_nick_name'], 1);
    }elseif(empty($services_desc)){
        sys_msg($_LANG['input_intro'], 1);
    }elseif(empty($services)){
        sys_msg($_LANG['select_admin'], 1);
    }
    ...
    if($_REQUEST['act'] == 'insert'){
        ...
        $res = $db->getRow($sql);
        if($res['status'] == 1){
            sys_msg($_LANG['this_admin_was_kefu'], 1);
        ...
        $res = $db->query($sql);
        if(!$res){
            sys_msg($_LANG['update_kefu_fail'], 1);
        }
    }
...
elseif ($_REQUEST['act'] == 'dialog_list'){
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['dialog_log']);
    $smarty->assign('menu_select',array('action' => '10_priv_admin', 'current' => 'kefu_list'));
...
function services_list(){
    ...
    /** 获得总记录数据 */
    $sql = 'SELECT COUNT(s.id) FROM ' .$GLOBALS['ecs']->table('im_service') . ' s'
            . " INNER JOIN " . $GLOBALS['ecs']->table('admin_user') . " a ON a.user_id = s.user_id  AND a.ru_id = " . $adminru['ru_id']
            . ' WHERE s.status = 1';
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    $filter = page_and_size($filter);
    $list = array();
    $sql = 'SELECT s.* FROM ' .$GLOBALS['ecs']->table('im_service') . ' s'
        . " INNER JOIN " . $GLOBALS['ecs']->table('admin_user') . " a ON a.user_id = s.user_id  AND a.ru_id = " . $adminru['ru_id']
        . ' WHERE s.status = 1';
...
function admin_list(){
    ...
    $sql = "SELECT a.user_id, a.user_name FROM " . $GLOBALS['ecs']->table("admin_user") . " a"
            . " LEFT JOIN " . $GLOBALS['ecs']->table("im_service") . " s ON a.user_id = s.user_id AND s.status = 1"
            . " WHERE a.ru_id = " . $ruId ;
...
function dialog_list ($id, $val = 0){
    ...
    foreach($res as $k => $v){
        ...
        $res[$k]['end_time'] = empty($v['end_time']) ? $GLOBALS['_LANG']['have_not_end'] : date('Y-m-d H:i:s', $v['end_time']);
    }
...
function service_info($id){
    $sql = 'SELECT id, nick_name, post_desc FROM ' . $GLOBALS['ecs']->table("im_service") . '  WHERE status = 1 AND id = ' . $id ;
