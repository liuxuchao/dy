...
if ($_REQUEST['act'] == 'delete_icon')
{
	...
	if(!empty($cat_info))
	{
		...
		if($GLOBALS['db']->query($sql))
		{
			...
			$result=array('error'=>1, 'msg'=>$_LANG['delete_success']);
		}		
	}
...
if ($_REQUEST['act'] == 'insert') {
    ...
    if ($cat['level'] < 2 && $adminru['ru_id'] > 0) {
        ...
        sys_msg($_LANG['you_power_add_level4_cate'], 0, $link);
        exit;
    }
    //ecmoban模板堂 --zhuo end
    ...
    if (!empty($_FILES['cat_icon']['name'])) {
        if ($_FILES["cat_icon"]["size"] > 200000) {
            sys_msg($_LANG['upload_img_max_200kb'], 0, $link);
        }
        $type = end(explode('.', $_FILES['cat_icon']['name']));
        if ($type != 'jpg' && $type != 'png' && $type != 'gif') {
            sys_msg($_LANG['please_upload_jpg_gif_png'], 0, $link);
        }
...
if ($_REQUEST['act'] == 'update')
{
    ...
    if($cat['parent_id'] == $cat_id  || in_array($cat['parent_id'], $reject_cat)){
        sys_msg($_LANG['cate_cant_parent'], 0, $link);
        exit;
    }
    if($cat['level'] < 2 && $adminru['ru_id'] > 0){
...
function get_cat_level($parent_id = 0, $level = 0, $ru_id = 0) {
    ...
    $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order " .
            " FROM " . $GLOBALS['ecs']->table('category') . " AS c WHERE  $where" .
    ...
    if ($res) {
        foreach ($res as $k => $row) {
            ...
            if ($extension_goods_array) {
                $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table('goods') . " WHERE is_delete = 0 AND user_id = '" . $adminru['ru_id'] . "'" .
