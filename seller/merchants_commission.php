...
elseif ($_REQUEST['act'] == 'edit') {
    ...
    $sql = "SELECT ms.*, mp.percent_value FROM " . $ecs->table('merchants_server') ." AS ms ". 
            "LEFT JOIN " .$ecs->table('merchants_percent') ." AS mp ON mp.percent_id = ms.suppliers_percent". 
...
elseif($_REQUEST['act'] == 'order_list')
{
    ...
    $smarty->assign('user_id',   $adminru['ru_id']);	//liu  供批量结算用
...
function order_download_list($result) {
    if(empty($result)) {
        return i($_LANG['no_data_match_condition']);
    }
...
function merchants_order_list($page = 0)  
{
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        {
            $row[$i]['settlement_status'] = $_LANG['settled'];
        }
        else
        {
            $row[$i]['settlement_status'] = $_LANG['not_settled'];
        }
        $row[$i]['settlement_frozen'] = "";
        if($row[$i]['is_frozen'])
        {
            $row[$i]['settlement_frozen'] = $_LANG['frozen'];
        }
    ...
    if($count){		
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            $row['brokerage_amount']['is_settlement'] = price_format($is_settlement, false); //liu
            $row['brokerage_amount']['no_settlement'] = price_format($no_settlement, false); //liu
            
            $row['brokerage_amount']['all_price'] = number_format($all_commission, 2, '.', '');
            $row['brokerage_amount']['all'] = price_format($all_commission, false); //liu 
...
function changeSettlement($str){
	if($str=='0'){
		$str = $GLOBALS['_LANG']['not_settled'];
	}else{
		$str = $GLOBALS['_LANG']['settled'];
	}
	return $str;
}
...
function commission_download_list($result) {
    ...
    $data = i($GLOBALS['_LANG']['merchants_commission_commission_download_list']."\n");
    $count = count($result);
