...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',      $_LANG['bargain_goods_list']);//标题
    $smarty->assign('action_link',  array('href' => 'bargain.php?act=add', 'text' => $_LANG['add_bargain_goods'], 'class' => 'icon-plus'));
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('bargain_goods_list.dwt'), '',
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
	...
	if ($_REQUEST['act'] == 'edit'){
		$smarty->assign('ur_here', $_LANG['edit_bargain_goods']);//标题
	}else{
		$smarty->assign('ur_here', $_LANG['add_bargain_goods']);//标题
	}

    $smarty->assign('action_link', list_link($_REQUEST['act'] == 'add'));
    $smarty->assign('brand_list', get_brand_list());//品牌列表
    $smarty->assign('ru_id',  $adminru['ru_id']);//店铺id
...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    $target_price = $_REQUEST['target_price'];      //目标价格
    $product_id = $_REQUEST['product_id'];          //货品ID
    $activity_goods_attr = $_REQUEST['bargain_id']; //活动商品属性列表id

    if($goods['goods_id'] <= 0){
        sys_msg($_LANG['please_add_bargain_goods'], 0, $links);
    }

    if($goods['start_time'] >= $goods['end_time']){
        sys_msg($_LANG['start_time_no_large_end_time'], 0, $links);
    }
    ...
    if ($id > 0)
    {
        ...
        /* 提示信息 */
        $links = array(
            array('href' => 'bargain.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_bargain_list'])
        );
    ...
    else
    {
        ...
        if ($res['num'] >= 1) {
            /* 提示信息 */
        $links = array(
            array('href' => 'bargain.php?act=add', 'text' => $_LANG['continue_add_bargain_goods'])
        );
        sys_msg($_LANG['cant_add_before_bargin_end'], 0, $links);
        }
        ...
        $links = array(
            array('href' => 'bargain.php?act=add', 'text' => $_LANG['continue_add_bargain_goods']),
            array('href' => 'bargain.php?act=list', 'text' => $_LANG['return_bargain_list'])
        );
        sys_msg($_LANG['success_add_bargain_goods'], 0, $links);
    }
...
elseif ($_REQUEST['act'] == 'remove_down')
{
...
if ($_REQUEST['act'] == 'bargain_log')
{
    ...
    $smarty->assign('primary_cat',     $_LANG['02_promotion']);
    $smarty->assign('full_page',    1);
    $smarty->assign('ur_here',      $_LANG['act_detail']);
    $smarty->assign('menu_select', array('action'=>'02_promotion', 'current'=>'19_bargain'));

    //页面分菜单 by wu start
    $tab_menu = array();
    $tab_menu[] = array('curr' => 0, 'text'=> $_LANG['bargain_goods_list'], 'href' => 'bargain.php?act=list');
    $tab_menu[] = array('curr' => 1, 'text'=> $_LANG['act_detail'], 'href' => 'bargain.php?act=bargain_log');
...
elseif ($_REQUEST['act'] == 'bargain_log_query')
{
    ...
    make_json_result($smarty->fetch('bargain_log_list.dwt'), '',
        array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
...
if ($_REQUEST['act'] == 'bargain_statistics_list')
{
    ...
    $smarty->assign('primary_cat',     $_LANG['02_promotion']);
    $smarty->assign('full_page',    1);
    $smarty->assign('ur_here',      $_LANG['friends_and_relatives']);
    $smarty->assign('menu_select', array('action'=>'02_promotion', 'current'=>'19_bargain'));

    //页面分菜单 by wu start
    $tab_menu = array();
    $tab_menu[] = array('curr' => 0, 'text'=> $_LANG['bargain_goods_list'], 'href' => 'bargain.php?act=list');
    $tab_menu[] = array('curr' => 1, 'text'=> $_LANG['friends_and_relatives'], 'href' => 'bargain.php?act=bargain_statistics_list');
    $smarty->assign('tab_menu', $tab_menu);
...
elseif ($_REQUEST['act'] == 'bargain_query')
{
    ...
    make_json_result($smarty->fetch('bargain_statistics_list.dwt'), '',
        array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
...
function bargain_goods_list($ru_id)
{
    ...
    if ($result === false)
    {
        ...
        //管理员查询的权限 -- 店铺查询 end
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('bargain_goods') ." AS bg left join ". $GLOBALS['ecs']->table('goods') ." as g  ON bg.goods_id = g.goods_id ".
                " WHERE $where";
       ...
       $sql = "SELECT bg.*,g.user_id,g.goods_sn,g.goods_name,g.shop_price,g.goods_number,g.goods_img,g.goods_thumb ".
                "FROM " . $GLOBALS['ecs']->table('bargain_goods') ." AS bg "." left join ". $GLOBALS['ecs']->table('goods') ." as g on bg.goods_id=g.goods_id".
                " WHERE  $where ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr['user_name'] = get_shop_name($arr['user_id'], 1);//商家名称
        $arr['shop_price'] = price_format($arr['shop_price']);
        $target_price = get_bargain_target_price($arr['id']);//默认获取属性最低砍价
        if($target_price){
		...
		if($arr['status'] > 0){
			$status = $_LANG['act_closed'];
		}else{
			if ($time >= $arr['end_time']) {
				$status = $_LANG['act_ended'];
			} else {
				$status = $_LANG['act_ing'];
			}
		}
        ...
        if ($arr['is_audit'] == 1) {
            $is_audit = $_LANG['audited_not_adopt'];
        } elseif ($arr['is_audit'] == 2) {
            $is_audit = $_LANG['audited_yes_adopt'];
        } else {
            $is_audit = $_LANG['not_audited'];
        }
...
{
    $sql = "SELECT bg.*,g.user_id,g.goods_sn,g.goods_name,g.shop_price,g.market_price,g.goods_number,g.goods_img,g.goods_thumb ".
                "FROM " . $GLOBALS['ecs']->table('bargain_goods') ." AS bg "." left join ". $GLOBALS['ecs']->table('goods') ." as g on bg.goods_id=g.goods_id".
                " WHERE bg.id = '$id' ".
               " LIMIT 1 ";
...
function bargain_log_list($bargain_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if($arr['goods_attr_id']){
            ...
            $target_price = bargain_target_price($arr['bargain_id'],$arr['goods_id'],$spec);//底价
            $arr['target_price'] =  price_format($target_price);
        }else{
            $arr['target_price'] = price_format($arr['target_price']);//底价
        }
        ...
        //团状态
        if ($arr['status'] == 1 ) {
            $arr['status'] = $_LANG['act_success'];
        } elseif($arr['status'] != 1 and $time >= $arr['start_time'] and $time <= $arr['end_time'] ) {
            $arr['status'] = $_LANG['act_ing'];
        }elseif($arr['status'] != 1 and $time > $arr['end_time']) {
            $arr['status'] = $_LANG['act_fail'];
        }
...
function bargain_statistics_list($id =0)
{
    ...
    /* 查询 */
    $sql = "SELECT *  FROM " . $GLOBALS['ecs']->table('bargain_statistics') . " WHERE bs_id ='".$id."' ".
           " ORDER BY $filter[sort_by] $filter[sort_order] ".
           " LIMIT ". $filter['start'] .", $filter[page_size]";
    set_filter($filter, $sql);
    }
...
function list_link($is_add = true)
{
    ...
    return array('href' => $href, 'text' => $GLOBALS['_LANG']['bargain_goods_list'], 'class' => 'icon-reply');
}
