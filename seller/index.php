...
if ($_REQUEST['act'] == '') {
    ...
    $seller_goods_info['is_sell'] = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('goods') . " WHERE user_id ='$ru_id' AND is_on_sale = 1 AND is_delete = 0");
    $seller_goods_info['is_delete'] = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('goods') . " WHERE user_id ='$ru_id' AND is_delete = 1");
    $seller_goods_info['is_on_sale'] = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('goods') . " WHERE user_id ='$ru_id' AND is_on_sale = 0 AND is_delete = 0");
    $seller_goods_info['is_warn'] = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('goods') . " WHERE user_id ='$ru_id' AND goods_number <= warn_number AND is_delete = 0");
    ...
    $order['await_ship'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . " as oi " .
        " WHERE 1 AND oi.shipping_status = 0 AND oi.pay_status = 2 AND oi.order_status = 1 AND (SELECT ore.ret_id FROM ".$GLOBALS['ecs']->table('order_return')." as ore WHERE ore.order_id = oi.order_id LIMIT 1) IS NULL " . $where_og); /*. order_query_sql('await_ship')*/
 ...
 elseif ($_REQUEST['act'] == 'merchants_second') {
    ...
    if (!empty($domain_name)) {
        ...
        if ($db->getOne($sql) > 0) {
            ...
            sys_msg($_LANG['domain_exist'], 0, $lnk);
        }
    }
    $seller_domain = array(
        'ru_id' => $adminru['ru_id'],
...
//				$GLOBALS['db']->query($sql);			
        ...
        $lnk[] = array('text' => $_LANG['back_prev_step'], 'href' => 'index.php?act=merchants_first');
        sys_msg($_LANG['add_shop_info_success'], 0, $lnk);
    } else {
...
//			$GLOBALS['db']->query($sql);			
        ...
        $lnk[] = array('text' => $_LANG['back_prev_step'], 'href' => 'index.php?act=merchants_first');
        sys_msg($_LANG['update_shop_info_success'], 0, $lnk);
    }
}
...
elseif ($_REQUEST['act'] == 'shop_top') {
    ...
    $smarty->assign('primary_cat',     $_LANG['19_merchants_store']);
    $smarty->assign('ur_here', $_LANG['03_merchants_shop_top']);
    //获取入驻商家店铺信息 wang 商家入驻
...
elseif($_REQUEST['act'] == 'shop_top_edit')
{
    ...
    $lnk[] = array('text' => $_LANG['back_prev_step'], 'href' => 'index.php?act=shop_top');

    sys_msg($_LANG['shop_head_edit_success'], 0, $lnk);
}
...
function get_sales($day_num){
    ...
    /* 查询订单 */
    $sql = "SELECT IFNULL(SUM(" . order_commission_field('o.') . "),0) AS 'ga', COUNT(o.order_id) AS 'oi' " .
...
function surplus_time($ru_id) {
    ...
    if ($_SESSION['verify_time']) {
        ...
        if ($enter_overtime <= $time) {//审核过期
            ...
            $GLOBALS['db']->query($sql);
            sys_msg($GLOBALS['_LANG']['exam_expire_repay_retry'], 1);
            return false;
        } elseif ($enter_overtime < $one_month_later) {//审核过期前30天
            $link[] = array('text' => $GLOBALS['_LANG']['back_list'], 'href' => "index.php");
            ...
            $link[] = array('text' => $GLOBALS['_LANG']['back_list'], 'href' => "index.php");
            sys_msg($GLOBALS['_LANG']['exam_repay_tip_2month'], 0, $link);
        } else {//未到提醒期
            return true;
        }
    } else {
