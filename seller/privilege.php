    ...
    if($_REQUEST['type']  == 'password'){
        ...
        if(!empty($ec_salt))
        {
            $sql = "SELECT COUNT(*)".
                ...
                " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5(md5($_POST['password']).$ec_salt) . "' AND ru_id != 0 AND suppliers_id =0"; //限制商家登录后台 by wu
        }
        else
        {
            $sql = "SELECT COUNT(*)".
                " FROM " . $ecs->table('admin_user') .
                " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5($_POST['password']) . "' AND ru_id != 0 AND suppliers_id =0"; //限制商家登录后台 by wu
        }
