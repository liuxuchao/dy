...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'download')
{
	...
	$smarty->assign('current','adsense_list');
    ...
    /* 赋值给模板 */
    $smarty->assign('action_link', array('href' => 'ads.php?act=list', 'text' => $_LANG['ad_list'], 'class' => 'icon-reply'));
    $smarty->assign('action_link2', array('href' => 'adsense.php?act=download', 'text' => $_LANG['download_ad_statistics'], 'class' => 'icon-download-alt'));
    $smarty->assign('ur_here',     $_LANG['adsense_js_stats']);
    $smarty->assign('lang',        $_LANG);

    /* 显示页面 */
    assign_query_info();
    $smarty->display('adsense.dwt');
}
