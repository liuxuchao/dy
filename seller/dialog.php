...
elseif($_REQUEST['act'] == 'transport_insert' || $_REQUEST['act'] == 'transport_update')
{
    ...
    if ($_REQUEST['act'] == 'transport_update') {
        $result['message'] = $_LANG['edit_express_tpl_success'];
        $db->autoExecute($ecs->table('goods_transport'), $data, "UPDATE", "tid = '$data[tid]'");
        $tid = $s_tid;
        
        $where = " tid = '$tid'";
    } else {
        $result['message'] = $_LANG['add_express_tpl_success'];
...
elseif($_REQUEST['act'] == 'cate_insert')
{
    ...
    // 上传手机菜单图标 by kong start
        if (!empty($_FILES['touch_icon']['name'])) {
            if ($_FILES["touch_icon"]["size"] > 200000) {
                $result['error'] = 2;
                $result['message'] = $_LANG['upload_img_max_200kb'];
                die($json->encode($result)); 
            }
            $type = end(explode('.', $_FILES['touch_icon']['name']));
            if ($type != 'jpg' && $type != 'png' && $type != 'gif') {
                $result['error'] = 2;
                $result['message'] = $_LANG['please_upload_jpg_gif_png'];
                die($json->encode($result));    
            }
    ...
    if (cat_exists($cat['cat_name'], $cat['parent_id'], 0, $adminru['ru_id'])) {
        /* 同级别下不能有重复的分类名称 */
        $result['error'] = 2;
        $result['message'] = $_LANG['same_level_no_same_name'];
        die($json->encode($result));     
    }
    ...
    if ($cat['grade'] > 10 || $cat['grade'] < 0) {
        /* 价格区间数超过范围 */
        $result['error'] = 2;
        $result['message'] = $_LANG['price_range_beyond'];
        die($json->encode($result));  
    }
    ...
    $result['content'] = $GLOBALS['smarty']->fetch('library/dialog.lbi');
    $result['message'] = $_LANG['add_cate_success_reselect'];    
