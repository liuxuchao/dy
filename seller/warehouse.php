...
if ($_REQUEST['act'] == 'list') {
    ...
    /* 仓库运费模板 by wu */
    $smarty->assign('action_link2', array('href' => 'warehouse.php?act=ship_list', 'text' => $_LANG['warehouse_shipping_tpl'], 'class' => 'icon-edit'));
    /* 取得参数：上级地区id */
...
elseif ($_REQUEST['act'] == 'ship_list') {
    ...
    $smarty->assign('ur_here', $_LANG['warehouse_shipping_tpl']);
    $smarty->assign('current', 'warehouse');
...
elseif ($_REQUEST['act'] == 'remove_tpl') {
    ...
    if ($db->query($sql)) {
        $data = $_LANG['delete_success_alt'];
    } else {
        $data = $_LANG['js_languages']['confirm_delete_fail'];
    }
    die($data);
...
if ($_REQUEST['act'] == 'tpl_list') {
    ...
    $smarty->assign('primary_cat', $_LANG['11_system']);
    $smarty->assign('ur_here', $_LANG['warehouse_shipping_tpl_list'].' - ' . $shipping['shipping_name']);
    $smarty->assign('action_link2', array('href' => 'warehouse.php?act=ship_tpl&shipping_id=' . $shipping_id,
        'text' => $_LANG['new_warehouse_shipping_tpl']));
    $smarty->assign('action_link', array('href' => 'warehouse.php?act=ship_list', 'text' => $_LANG['return_delivery_list']));
    $smarty->assign('full_page', 1);
...
elseif ($_REQUEST['act'] == 'ship_tpl') {
    ...
    assign_query_info();
    $smarty->assign('ur_here', $_LANG['warehouse_shipping_tpl_list'].' - ' . $shipping_name);
    $smarty->assign('current', 'warehouse');
...
elseif ($_REQUEST['act'] == 'freight_tpl_insert') {
    ...
    if ($shipping_id == 0 || empty($tpl_name) || empty($warehouse_id) || empty($regionId)) {
        $add_to_mess = $_LANG['please_input_info_complete'];
    ...
    foreach ($res as $key => $val) {
        ...
        if ($warehouse_state && $region_state) {
            $add_to_mess = $_LANG['tpl_arrive_area_exist'];
...
elseif ($_REQUEST['act'] == 'freight_insert') {
    ...
    if ($shipping_id == 0) {
        $add_to_mess = $_LANG['js_languages']['jl_select_deliver'];
