...
elseif ($_REQUEST['act'] == 'kdniao_print')
{
    ...
    //判断订单
    if (empty($order_ids)) {
        sys_msg($_LANG['no_select_order'], 1, $link);
    }
    ...
    if (count($shipping_ids) > 1) {
        sys_msg($_LANG['select_express_same_batch_print'], 1, $link);
    }
    ...
    if($order_ids && $order_ids[0]){
        ...
        foreach ($order_ids as $order_id) {
            ...
            if ($result["ResultCode"] != "100") {
                $batch_error[] = $_LANG['04_order']."（{$order_id}）：".$_LANG['dzmd_order_fail']."：{$result['Reason']}";
                continue;
            }
            ...
            if (!empty($result['PrintTemplate'])) {
                ...
                $batch_error[] = $_LANG['04_order']."（{$order_id}）：".$_LANG['no_print_tpl'];
                continue;
            }
