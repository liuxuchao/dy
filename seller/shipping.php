...
if ($_REQUEST['act'] == 'list')
{
    ...
    //页面分菜单 by wu start
    $tab_menu = array();
    $tab_menu[] = array('curr' => 1, 'text' => $_LANG['03_shipping_list'], 'href' => 'shipping.php?act=list');
    $tab_menu[] = array('curr' => 0, 'text' => $_LANG['shipping_transport'], 'href' => 'goods_transport.php?act=list');
    
    $smarty->assign('tab_menu', $tab_menu);
    //页面分菜单 by wu end	
    
    //商品运费 by wu
    $action_link2 = ($_REQUEST['act'] == 'list') ? array('href' => 'goods_transport.php?act=list', 'text' => $_LANG['goods_shipping_tpl'], 'class' => 'icon-edit') : '';
    $smarty->assign('action_link2', $action_link3);
    ...
    $smarty->assign('kdniao_account_use',$_CFG['kdniao_account_use']); 
...
elseif ($_REQUEST['act'] == 'date_list')
{
    ...
    $smarty->assign('ur_here', $_LANG['delivery_time_list']);
    $smarty->assign('action_link', array('href' => 'shipping.php?act=date_add', 'text' => $_LANG['add_delivery_time']));
...
elseif($_REQUEST['act'] == 'date_add')
{
    ...
    $smarty->assign('ur_here', $_LANG['add_delivery_time']);
    $smarty->assign('action_link', array('href' => 'shipping.php?act=date_list', 'text' => $_LANG['delivery_time_list']));
...
elseif($_REQUEST['act'] == 'date_edit')
{
    ...
    $smarty->assign('ur_here', $_LANG['edit_delivery_time']);
    $smarty->assign('action_link', array('href' => 'shipping.php?act=date_list', 'text' => $_LANG['delivery_time_list']));
...
elseif ($_REQUEST['act'] == 'date_remove') 
{
    ...
    if (($db->query($sql))) {
        ...
        sys_msg($_LANG['js_languages']['confirm_delete_fail'], 0, $link);
    }
}
...
elseif ($_REQUEST['act'] == 'edit_print_template')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['03_shipping_list'], 'href' => 'shipping.php?act=list', 'class' => 'icon-reply'));
    $smarty->assign('shipping_id', $shipping_id);
