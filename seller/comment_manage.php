...
if ($_REQUEST['act'] == 'single_check')
{
    ...
    if ($_REQUEST['check'] == 'allow')
    {
            ...
            {
                    log_account_change($_REQUEST[user_id], $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = floatval($_REQUEST['integ']), $change_desc = $_LANG['show_img_reward']);
            }
    ...
    else
    {
            ...
            if(!empty($_REQUEST['integ']))
            {
                    log_account_change($_REQUEST[user_id], $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = floatval(-$_REQUEST['integ']), $change_desc = $_LANG['show_img_no_reduce_intergral']);
            }
...
if ($_REQUEST['act']=='reply')
{
    ...
    $sql = "SELECT * FROM ".$ecs->table('comment'). " WHERE parent_id = '$_REQUEST[id]'".
            " AND single_id = 0 AND dis_id = 0 AND user_id = '".$_SESSION['seller_id']."' AND user_name = '".$admin_info['user_name']."' AND ru_id = '".$adminru['ru_id']."' ";
...
if ($_REQUEST['act']=='action')
{
    ...
    elseif($adminru['ru_id']==$comment_info['ru_id'])
    {
        ...
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
               ...
               "VALUES('3', '$_POST[id_value]','$_POST[email]', " .//评论类型3为管理员评论
                    "'$_SESSION[seller_name]','$_POST[content]','" . gmtime() . "', "
                . "'$ip', '0', '$_POST[comment_id]', '$admin_info[user_id]', '$adminru[ru_id]')";
    }
	else
	{
...
function get_comment_list($ru_id)
{
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " AS c1 WHERE $where " .
