...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href'=>'shipping_area.php?act=add&shipping='.$shipping_id,
        'text' => $_LANG['new_area'], 'class' => 'icon-plus'));
    $smarty->assign('action_link2', array('href'=>'shipping.php?act=list',
        'text' => $_LANG['03_shipping_list'], 'class' => 'icon-reply'));
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
    ...
    $smarty->assign('action_link', array('href'=>'shipping_area.php?act=list&shipping='.$_REQUEST['shipping'],
        'text' => $_LANG['09_region_area_management'], 'class' => 'icon-reply'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    else
    {
        ...
        if($shipping_code == "cac"){
            ...
            if($district == 0){
                sys_msg($_LANG['select_jurisd_area'], 1);
            }
...
//       }
    ...
    $smarty->assign('action_link', array('href'=>'shipping_area.php?act=list&shipping='.$row['shipping_id'],
    'text' => $_LANG['09_region_area_management'], 'class' => 'icon-reply'));
    assign_query_info();
...
elseif($_REQUEST['act'] == 'remove_point')
{
    ...
    if($db->query($sql))
	{
		$data = array('error' => 2, 'message' => $_LANG['delete_success_alt'], 'content' => '');
		admin_log($name, 'remove', 'shipping_area');
	}
    else
	{
		$data = array('error' => 0, 'message' => $_LANG['js_languages']['confirm_delete_fail'], 'content' => '');
	}
	$json  = new JSON;
