    ...
    if ($_FILES[$name]) {
            ...
            if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {
                ...
                if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                    $result['error'] = 1;
                    $result['prompt'] = $_LANG['upload_correct_format_img']."（$allow_file_types）";
                } else {
                    ...
                    if (move_upload_file($file['tmp_name'], $file_name)) {
                        ...
                        $result['error'] = 1;
                        $result['prompt'] = $_LANG['system_error_reupload'];
                    }
                }
            }
        } else {
            $result['error'] = 1;
            $result['prompt'] = $_LANG['select_upload_img'];
        }
...
elseif($_REQUEST['act'] == 'release'){
    ...
    if($suffix && $type == 0){
        ...
        if($db->query($sql)== true){
            ...
            $result['error'] = 0;
            $result['content'] = $_LANG['system_error_refresh_wait_retry'];
        }
    }else if($type == 1){
        $result['error'] = $error;
    }else{
        $result['error'] = 0;
        $result['content'] = $_LANG['please_select_tpl'];
    }
    $result['tem'] = $suffix;
    die(json_encode($result));
}
...
elseif ($_REQUEST['act'] == 'purchase_temp') {
    ...
    if ($pay_id > 0) {
        if ($temp_id > 0) {
            ...
            sys_msg($_LANG['system_error_retry'], 0, $link);
        }
    } else {
        $link[] = array('text' => $_LANG['go_back'], 'href' => 'visual_editing.php?act=templates');
        sys_msg($_LANG['please_select_payment'], 0, $link);
    }
}
elseif($_REQUEST['act'] == 'temp_pay'){
    ...
    $smarty->assign('ur_here', $_LANG['tpl_pay']);
    $smarty->assign('action_link', array('text' => $_LANG['tpl_list'], 'href' => 'visual_editing.php?act=templates'));
    ...
    if ($payment_info['pay_code'] == 'balance') {
        ...
        if ($user_money > $apply_info['order_amount']) {
            ...
            $db->query($sql);
            log_account_change($adminru['ru_id'], $apply_info['order_amount'] * (-1), 0, 0, 0, $_LANG['record_id'] . $apply_info['order_sn'] . $_LANG['seller_buy_visual_tpl_pay']);
            ...
            sys_msg($_LANG['pay_success_backup_first'], 0, $link);
        } else {
            sys_msg($_LANG['balance_no_enough_select_other_payment']);
        }
    } else {
        /* 调用相应的支付方式文件 */
...
elseif ($_REQUEST['act'] == 'template_apply_list') {
    //页面赋值
    $smarty->assign('menu_select', array('action' => '19_merchants_store', 'current' => '10_visual_editing'));
    $smarty->assign('primary_cat', $_LANG['visual_manage']);
    $smarty->assign('ur_here', $_LANG['tpl_pay_record']);
     //模板赋值
...
elseif($_REQUEST['act'] == 'generate'){
    ...
    if($db->query($sql) == true){
        ...
        $result['error'] = 2;
        $result['content'] = $_LANG['system_error_retry'];
    }
    die(json_encode($result));
}
/*删除图片*/
...
elseif($_REQUEST['act'] == 'edit_information'){
    ...
    if ((isset($_FILES['ten_file']['error']) && $_FILES['ten_file']['error'] == 0) || (!isset($_FILES['ten_file']['error']) && isset($_FILES['ten_file']['tmp_name']) && $_FILES['ten_file']['tmp_name'] != 'none'))
    {
        ...
        {
            sys_msg($_LANG['img_format_wrong']);
        }
    ...
    if ($html === false) {
        ...
        sys_msg("' . $file_dir . '/tpl_info.txt".$_LANG['no_write_power_edit'],1,$link);
    }else{
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'visual_editing.php?act=templates';
...
elseif($_REQUEST['act'] == 'removeTemplate')
{
    ...
    if($default_tem == $code){
        $result['error'] = 1;
        $result['content'] = $_LANG['using_cant_delete_want_change'];
    }else{
...
elseif($_REQUEST['act'] == 'backupTemplates'){
    ...
    if($code){
        ...
        $result['error'] = 1;
        $result['content'] = $_LANG['please_backup_tpl'];
    }
    die(json_encode($result));
}
/*导出*/
elseif($_REQUEST['act'] == 'export_tem'){
    ...
    if(!empty($checkboxes)){
        ...
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'visual_editing.php?act=templates';
        sys_msg($_LANG['please_select_export_tpl'], 1, $link);
    }
}
//发布
