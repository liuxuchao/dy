    ...
    if (!empty($_POST['action']) && $_POST['action'] == 'get_pwd')
    {
        ...
        if(!$seller_shopinfo){
            ...
            if ($field['bool']) {
                ...
                $sql = " SELECT contactEmail AS seller_email FROM " . $ecs->table('merchants_steps_fields') . " WHERE user_id = '" .$adminru['ru_id']. "' LIMIT 1";
                $shopinfo = $db->getRow($sql);

                $seller_email = $shopinfo['seller_email'];
            }
        }else{
            $seller_email = !empty($seller_shopinfo['seller_email']) ? $seller_shopinfo['seller_email'] : '';
