...
elseif ($_REQUEST['act'] == 'date_list')
{
    ...
    $smarty->assign('ur_here', $_LANG['self_lift_time']);
    $smarty->assign('action_link', array('href' => 'shipping.php?act=date_add', 'text' => $_LANG['add_self_lift_time']));
...
elseif($_REQUEST['act'] == 'date_add')
{
    ...
    $smarty->assign('ur_here', $_LANG['add_self_lift_time']);
    $smarty->assign('action_link', array('href' => 'shipping.php?act=date_list', 'text' => $_LANG['self_lift_time_list']));
...
elseif($_REQUEST['act'] == 'date_edit')
{
    ...
    $smarty->assign('ur_here', $_LANG['edit_self_lift_time']);
    $smarty->assign('action_link', array('href' => 'shipping.php?act=date_list', 'text' => $_LANG['self_lift_time_list']));
...
elseif($_REQUEST['act'] == 'date_update')
{
    ...
    if (($db->query($sql))) {
        /* 提示信息 */
        $link[0]['text'] = $_LANG['back_list_page'];
        $link[0]['href'] = 'shipping.php?act=date_list';

        sys_msg($_LANG['edit_success'], 0, $link);
    } else {
        /* 提示信息 */
        $link[0]['text'] = $_LANG['back_again_edit'];
        $link[0]['href'] = 'javascript:history.back(-1)';

        $link[1]['text'] = $_LANG['back_list_page'];
        $link[1]['href'] = 'shipping.php?act=date_list';
...
elseif($_REQUEST['act'] == 'date_remove')
{
    ...
    if (($db->query($sql))) {
        ...
        sys_msg($_LANG['carddrop_fail'], 0, $link);
    }
}
