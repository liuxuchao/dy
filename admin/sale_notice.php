...
if ($_REQUEST['act']=='view')
{
    ...
    $smarty->assign('ur_here',      $_LANG['sale_notice']);
    $smarty->assign('action_link',  array('text' => $_LANG['depreciate_notice_list'],
...
if ($_REQUEST['act']=='action')
{
    ...
    if (isset($_POST['remail']) && !empty($detail[email])) {
        ...
        if (send_mail($detail['user_name'], $detail['email'], $template['template_subject'], $content, $template['is_html'])) {
            ...
            notice_log($detail['goods_id'], $detail['email'], $send_ok, $send_type);
            /* 提示信息 */
            $link[] = array('text' => $_LANG['back_depreciate_list'], 'href' => 'sale_notice.php?act=list');
            sys_msg($_LANG['send_fail'], 0, $link);
        }
    }
    ...
    if (isset($_POST['resms']) && !empty($detail['cellphone']) && $_CFG['sms_price_notice'] == 1) {
        ...
        $smsParams = array(
            ...
            'goodsname' => sub_str($detail['goods_name'], 20),
        ...
        if ($res) {
            ...
            /* 提示信息 */
            $link[] = array('text' => $_LANG['back_depreciate_list'], 'href' => 'sale_notice.php?act=list');
            sys_msg($_LANG['send_success'], 0, $link);
        } else {
    ...
    /* 记录管理员操作 */
    admin_log($_LANG['handle_depreciate_notice'], 'edit', 'sale_notice');
...
function sale_notice_list($ru_id)
{
	...
	$statusArr = array(1=>$GLOBALS['_LANG']['has_been_sent'] , 2=>$GLOBALS['_LANG']['unsent'] , 3=>$GLOBALS['_LANG']['system_send_fail']);
	$send_typeArr = array(1=>$GLOBALS['_LANG']['mail'] , 2=>$GLOBALS['_LANG']['short_message']);
