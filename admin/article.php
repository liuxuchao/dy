...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('article_list.dwt'), '',
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    //检查是否是最下级分类
    $sql = " SELECT COUNT(*) FROM ".$ecs->table('article_cat')." WHERE parent_id = '".$_POST['article_cat']."' ";
    $cat_count = $db->getOne($sql);
    if($cat_count > 0){
        sys_msg($_LANG['not_min_cat'], 1);
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,sort_order) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                ...
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]','$sort_order')";
    $db->query($sql);
...
if ($_REQUEST['act'] =='update')
{
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]',sort_order = '$sort_order' ", $_POST['id']))
    {
        ...
        $link[1]['text'] = $_LANG['back_cat_list'];
...
elseif ($_REQUEST['act'] == 'del_file')
{
    ...
    $links[] = array('text' => $_LANG['back_edit_article_content'], 'href' => "article.php?act=edit&id=$article_id");
    sys_msg($_LANG['delete_upload_file_success'], 0, $links);
}
