...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        {
            $sql = "DELETE FROM " . $ecs->table('seckill') .
            " WHERE sec_id " . db_create_in($ids);
        ...
        elseif ($_POST['type'] == 'review_to')
        {
            ...
            $sql = "UPDATE " . $ecs->table('seckill') ." SET review_status = '$review_status' "
                . " WHERE sec_id " . db_create_in($ids);
            
            if($db->query($sql))
            {
                $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'seckill.php?act=list&seller_list=1&' . list_link_postfix());
                sys_msg($_LANG['seckill_adopt_status_success'], 0, $lnk);
            }
        }
    }
}
...
function get_seckill_list(){
    ...
    if ($result === false)
    {
        ...
        /* 获活动数据 */
        $sql = "SELECT sec_id, begin_time, acti_title, is_putaway, acti_time, review_status, ru_id ".
               " FROM " . $GLOBALS['ecs']->table('seckill') . " AS s ".
...
function get_seckill_info(){
	$sql = " SELECT sec_id, begin_time, acti_title, acti_time, is_putaway, ru_id, review_status FROM ". $GLOBALS['ecs']->table('seckill') .
