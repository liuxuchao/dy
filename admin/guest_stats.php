...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT COUNT(*) AS order_num, " . $total_fee.
           ...
           " WHERE o.user_id > 0 " . order_query_sql('finished', 'o.') .
    ...
    if($_GET['flag'] == 'download')
    {
        ...
        $data .= $_LANG['member_sum'] . "\t" . $_LANG['average_member_order'] . "\t" .
                $_LANG['m ember_order_sum'] . "\n";

        $ave_user_ordernum = $user_num > 0 ? sprintf("%0.2f", $user_all_order['order_num'] / $user_num) : 0;
        $ave_user_turnover = $user_num > 0 ? strip_tags(price_format($user_all_order['turnover'] * $user_num)) : 0;
        ...
        $order_num = $guest_all_order['order_num'] > 0 ? strip_tags(price_format($guest_all_order['turnover'] / $guest_all_order['order_num'])) : 0;
        $data .= strip_tags(price_format($guest_all_order['turnover'])) . "\t" . $guest_all_order['order_num'] . "\t" .
                $order_num;
    ...
    $smarty->assign('one_user_order_unm',sprintf('%0.2f',$user_all_order['order_num']/$user_num));//每会员订单数
	...
	if($user_all_order['order_num']){
		$smarty->assign('ave_user_turnover',  $user_num > 0 ? price_format($user_all_order['turnover'] * sprintf('%0.2f',$user_all_order['order_num']/$user_num))  : 0);
	}else{
		$smarty->assign('ave_user_turnover', $_LANG['not_order']);
	}
