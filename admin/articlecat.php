...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    //检查改分类下是否存在文章 如果存在文章则不能添加子分类
    $sql = " SELECT COUNT(*) FROM ".$ecs->table('article')." WHERE cat_id = '".$_POST['parent_id']."' ";
    $cat_count = $db->getOne($sql);
    if($cat_count > 0){
        sys_msg($_LANG['has_article_on_this_cat'], 1);
    }
