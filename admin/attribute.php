...
if ($_REQUEST['act'] == 'gcolor_insert')
{
    ...
    foreach ($_POST as $key_c => $value_c) {
        if (empty($value_c)) {
            ...
            sys_msg($_LANG['select_color'], 0, $link);
            exit;
        }
    }
    ...
    $link[] = array('text' => $_LANG['go_back'], 'href' => 'attribute.php?act=set_gcolor&attr_id=' . $attr_id);
    sys_msg($_LANG['attr_color_edit_success'], 0, $link);
}
//by zhuo end
...
elseif ($_REQUEST['act'] == 'insert_img')
{
    ...
    $link[1] = array('text' => $_LANG['back_attr'], 'href' => 'attribute.php?act=edit&attr_id=' . $attr_id);
