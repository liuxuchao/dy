...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    if($GLOBALS['_CFG']['attr_set_up'] == 0){
            ...
            sys_msg($_LANG['temporary_not_attr_power'], 0, $links);
                    exit;
            }
    }
    //ecmoban模板堂 --zhuo end
...
function get_goodstype($ru_id)
{
    ...
    if ($result === false)
    {
        ...
        if($filter['seller_list'] == 2){
            $where .= " AND t.user_id = 0  AND t.suppliers_id > 0 " ; //区分商家和自营 
        }else{
            $where .= !empty($filter['seller_list']) ? " AND t.user_id > 0 AND t.suppliers_id = 0 " : " AND t.user_id = 0 AND t.suppliers_id = 0 "; //区分商家和自营 
        }
