...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('menu_select', array('action' => '01_system', 'current' => '04_shipping_transport'));
    
    $ru_id = $adminru['ru_id'];
    $transport_list = get_transport_list($ru_id);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('action_link', array('href' => 'goods_transport.php?act=list', 'text' => $_LANG['goods_transport']));
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'get_shipping_tem') 
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* " .
            " FROM " . $ecs->table('goods_transport_tpl') . " AS b " .
...
elseif ($_REQUEST['act'] == 'the_national') 
{
    ...
    $sql = "SELECT GROUP_CONCAT(region_id) AS region_id FROM " .$GLOBALS['ecs']->table('goods_transport_tpl'). " WHERE user_id = '" .$adminru['ru_id']. "'"  .
...
elseif ($_REQUEST['act'] == 'add_shipping_tpl') 
{
    ...
    if ($shipping_id == 0 || empty($regionId)) {
        $result['error'] = 1;
        $result['message'] = $_LANG['info_fill_complete'];
...
function get_transport_list($ru_id = 0) {
    ...
    /* 查询记录 */
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_transport') . $where .
            " ORDER BY tid DESC";
