    ...
    if ($_FILES[$name]) {
            ...
            if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {
                ...
                if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                    $result['error'] = 1;
                    $result['prompt'] = $_LANG['file_img_success'] . "（$allow_file_types）";
                } else {
                    ...
                    if (move_upload_file($file['tmp_name'], $file_name)) {
                        ...
                        $result['error'] = 1;
                        $result['prompt'] = $_LANG['system_error_alt'];
                    }
                }
            }
        } else {
            $result['error'] = 1;
            $result['prompt'] = $_LANG['select_file_img'];
        }
        die(json_encode($result));
}
/*页面左侧属性*/
    ...
    if (isset($_POST['type']))
    {
        ...
        elseif ($_POST['type'] == 'review_to')
        {
            ...
            if($db->query($sql))
            {
                ...
                sys_msg($_LANG['topic_adopt_status_success'], 0, $lnk);
            }
        }
    }
}
