...
require(dirname(__FILE__) . '/includes/cls_FileUpload.php'); //ecmoban
...
elseif($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit'){
    ...
    if($_REQUEST['act'] == 'insert'){
        ...
        $time = gmtime();
        $sql = "INSERT INTO".$ecs->table("gallery_album")."(`parent_album_id`,`album_mame`,`album_cover`,`album_desc`,`sort_order`,`add_time`)"
                . " VALUES ('$parent_id','$album_mame','$file_url','$album_desc','$sort_order','$time')";
...
elseif($_REQUEST['act'] == 'view')
{
    ...
    if ($parent_album_id) {
        $smarty->assign('action_link', array('text' => $_LANG['upload_image'], 'spec' => "ectype='addpic_album'", 'href' => "gallery_album.php?act=view&id=$parent_album_id"));
    } else {
        $smarty->assign('action_link', array('text' => $_LANG['upload_image'], 'spec' => "ectype='addpic_album'", 'href' => "gallery_album.php?act=list"));
    }
...
elseif($_REQUEST['act'] == 'pic_query'){
    ...
    $offline_store = get_pic_album($album_id);
	// var_dump($offline_store['pzd_list']);
    $smarty->assign('pic_album', $offline_store['pzd_list']);
...
elseif($_REQUEST['act'] == 'remove')
{
    require(ROOT_PATH . '/includes/lib_visual.php');
     ...
     //存在下级相册 不让删除
     if($album_count > 0){
         make_json_error($_LANG['not_unremove_succeed']);
     }else{
...
elseif($_REQUEST['act'] == 'gallery_query'){
    $album_id = isset($_REQUEST['id'])  ? intval($_REQUEST['id']) : 0;
    $sql = "SELECT album_mame FROM ".$ecs->table("gallery_album")." WHERE album_id = '$album_id'";
    $album_mame = $db->getOne($sql);
    $smarty->assign('album_id',     $album_id);
...
elseif($_REQUEST['act'] == 'pic_remove'){
    ...
    if($db->query($sql)){
        ...
        $result['error'] = 1;
        $result['content'] = $_LANG['system_error_notic']; 
    }
    ...
    if (!empty($checkboxes)) {
        ...
        foreach($checkboxes  as $k=>$v){
            ...
            if($album_count > 0){
                ...
                //删除该相册目录下的所以图片
                $dir = ROOT_PATH . 'data/gallery_album/' . $v; //模板目录
                $rmdir = del_DirAndFile($dir);
                //删除图片数据库
...
elseif ($_REQUEST['act'] == 'upload_pic')
 {
    ...
    if ($extname == 'mp4') {
        ...
        if ((isset($_FILES['file']['error']) && $_FILES['file']['error'] == 0) || (!isset($_FILES['file']['error']) && isset($_FILES['file']['tmp_name']) && $_FILES['file']['tmp_name'] != 'none')) {
            ...
            $result['error'] = 1;
            $result['massege'] = $_LANG['upload_error_notic'];
        }
    }
...
elseif ($_REQUEST['act'] == 'goods_video')
{
    ...
    if ($extname == 'mp4') {
        ...
        $result['error'] = 2;
        $result['massege'] = $_LANG['upload_format_error'];
    }
    ...
    if($album_id > 0){
        if($type == 1){
         ...
         $sql = "SELECT  album_id FROM ".$GLOBALS['ecs']->table('gallery_album').
            "WHERE parent_album_id = '$album_id'";
...
function get_pzd_list($ru_id) {
    ...
    if ($result === false)
    {
                ...
                if($filter['store_search'] > 0){
                    ...
                    if($filter['store_search'] > 1){
                        $where .= " AND (SELECT msi.user_id FROM " . $GLOBALS['ecs']->table('merchants_shop_information') .' as msi ' .  
                                  " WHERE msi.user_id = ga.ru_id $store_where) > 0 ";
                    }
                }else{
        ...
        if($filter['seller_list'] == 2){
            $where .= " AND ga.ru_id = 0  AND suppliers_id > 0"; //区分商家和自营 
        }else{
            $where .= !empty($filter['seller_list']) ? " AND ga.ru_id > 0 AND suppliers_id = 0" : " AND ga.ru_id = 0 AND suppliers_id = 0"; //区分商家和自营 
        }
...
function get_pic_album($album_id = 0) {
    ...
    if ($result === false) {
		...
		$gsql = "SELECT album_id, album_mame, parent_album_id, sort_order, album_cover, album_desc, add_time FROM" .
				$GLOBALS['ecs']->table('gallery_album') . 
				" WHERE parent_album_id = '" . $filter['album_id'] . 
