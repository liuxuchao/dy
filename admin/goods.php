...
elseif ($_REQUEST['act'] == 'edit_link_desc')
{
    ...
    $smarty->assign('menu_select', array('action' => '02_cat_and_goods', 'current' => 'add_desc'));
    $smarty->assign('ur_here', $_LANG['desc_edit']);
    ...
    $smarty->assign('action_link',  $action_link);
    $action_link2 = array('href' => 'goods.php?act=list', 'text' => $_LANG['01_goods_list']);
    $smarty->assign('action_link2',  $action_link2);
...
elseif ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc')
{
    ...
    if(!empty($desc_name)){
        ...
        $link_cnt = $_LANG['confirm_batch_delete'];
    }
    ...
    $link[1] = array('text' => $_LANG['add_relation_desc'], 'href' => "goods.php?act=add_desc");
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        ...
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, goods_video, bar_code, cat_id,  " .
                    "user_cat, brand_id, shop_price, market_price, cost_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, " .
    ...
    $changelog_product_id = isset($_POST['changelog_product_id']) ? $_POST['changelog_product_id'] : array(); //货品零时表product_id
    ...
    if (!empty($goods_sn)) {
        ...
        if (!empty($goods_sn)) {
            ...
            $sql = "SELECT p.product_id FROM " . $ecs->table('products') . " AS p," .
                    ...
                    "WHERE p.product_sn = '$goods_sn' AND p.goods_id = g.goods_id AND g.user_id = '$seller_id'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($adminru['ru_id'] > 0 && $adminru['ru_id'] != $goods['user_id']) {
        make_json_error($_LANG['illegal_handle_error']);
    }
...
/*------------------------------------------------------ */
 elseif ($_REQUEST['act'] == 'addWarehouse') {
    ...
    if (empty($ware_name)) {
        $result['error'] = '1';
        $result['massege'] = $_LANG['select_warehouse'];
    } else {
        ...
        if ($w_id > 0) {
            $result['error'] = '1';
            $result['massege'] = $_LANG['warehouse_goods_stock_exi'];
        } else {
            if ($ware_number == 0) {
                $result['error'] = '1';
...
elseif ($_REQUEST['act'] == 'addRegion') {
    ...
    if (empty($area_name)) {
        $result['error'] = '1';
        $result['massege'] = $_LANG['select_region_alt'];
    } else {
        if ($region_number == 0) {
            $result['error'] = '1';
            ...
            if ($a_id > 0) {
                $result['error'] = '1';
                $result['massege'] = $_LANG['region_goods_stock_exi'];
            } else {
...
elseif($_REQUEST['act'] == 'img_default'){
    ...
    if($img_id > 0){
        ...
        if($sql = true){
            ...
            $result['error']=2;
            $result['massege']=$_LANG['modify_failure'];
        }
    }
    die($json->encode($result));
}
...
elseif ($_REQUEST['act'] == 'scan_code')
{
	...
	if($data['status'] != 0){
		...
		//重量（用毛重）
		$goods_weight = 0;	
		if(strpos($data['result']['grossweight'], $_LANG['unit_kg']) !== false){
			$goods_weight = floatval(str_replace($_LANG['unit_kg'], '', $data['result']['grossweight']));
		}elseif(strpos($data['result']['grossweight'], $_LANG['unit_g']) !== false){
			$goods_weight = floatval(str_replace($_LANG['unit_kg'], '', $data['result']['grossweight']))/1000;
		}
		//详情
		$goods_desc = "";
