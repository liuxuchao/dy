...
$smarty->assign('menu_select',array('action' => '01_system', 'current' => 'dscsms_configure'));
/*------------------------------------------------------ */
//-- 阿里大鱼短信模板列表
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link',  array('text' => $_LANG['alidayu_add'], 'href'=>'dscsms_configure.php?act=add'));
    $smarty->assign('action_link2',  array('text2' => $_LANG['alidayu_set'], 'href'=>'dscsms_configure.php?act=set_up'));
    
    /* 模板赋值 */
    $smarty->assign('ur_here',      $_LANG['dscsms_configure']);
    ...
    /* 列表页面 */
    assign_query_info();
    $smarty->display('dscsms_configure_list.dwt');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('dscsms_configure_list.dwt'), '', array('filter' => $note_list['filter'], 'page_count' => $note_list['page_count']));
}
...
if ($_REQUEST['act'] == 'add')
{
    $smarty->assign('action_link',  array('text' => $_LANG['alidayu_list'], 'href'=>'dscsms_configure.php?act=list'));
    ...
    /* 列表页面 */
    assign_query_info();
    $smarty->display('dscsms_configure_info.dwt');
}
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 提示信息 */
    $link[] = array('text' => $_LANG['go_back'], 'href'=>'dscsms_configure.php?act=list');
