...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $services = services_list();
    $smarty->assign('services_list', $services['list']);
    $smarty->assign('ur_here', $_LANG['services_manage']); // 当前导航
    ...
    if(empty($admins)){
        sys_msg($_LANG['not_select_manage'], 1);
    }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if(empty($services_name)){
        sys_msg($_LANG['please_enter_nickname'], 1);
    }elseif(empty($services_desc)){
        sys_msg($_LANG['please_enter_desc'], 1);
    }elseif(empty($services)){
        sys_msg($_LANG['please_enter_manage'], 1);
    }
    ...
    if($_REQUEST['act'] == 'insert'){
        ...
        $res = $db->getRow($sql);
        if($res['status'] == 1){
            sys_msg($_LANG['is_manage_services'], 1);
        ...
        if(!$res){
            sys_msg($_LANG['add_services_fail'], 1);
        }
        ...
        $res = $db->query($sql);
        if(!$res){
            sys_msg($_LANG['update_services_success'], 1);
        }
    }
...
elseif ($_REQUEST['act'] == 'remove'){
    ...
    $res = $db->getRow($sql);
    if(!$res){
        make_json_error($_LANG['services_existent']);
    }
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        if (isset($_POST['remove']))
        {
            ...
            /* 清除缓存 */
            clear_cache_files();
            $link[] = array('text' => $_LANG['back'], 'href' => 'services.php?act=list');
...
elseif ($_REQUEST['act'] == 'edit'){
    ...
    // 验证客服管理员账号状态
    $sql = "SELECT user_id FROM " . $ecs->table('admin_user') . " WHERE user_id = " . $customer['user_id'];
    $admin = $db->getRow($sql);
    if (empty($admin)) {
        sys_msg($_LANG['not_manage'], 1);
    }
