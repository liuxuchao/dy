...
elseif($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit'){
    ...
    if ($_REQUEST['act'] == 'add') {
        ...
        $smarty->assign('cities', []);
        $smarty->assign('districts', []);
    } else {
...
//-- 获取店铺统计结果
/* ------------------------------------------------------ */ 
elseif ($_REQUEST['act'] == 'stat_filter') {
...
function get_offline_store_list() 
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT o.ru_id,o.id,o.stores_name,o.stores_address,o.stores_tel,o.stores_opening_hours,"
                ...
                . "FROM" . $GLOBALS['ecs']->table('offline_store') . " AS o "
                . "LEFT JOIN " . $GLOBALS['ecs']->table('region') . " AS a ON a.region_id = o.country "
                . "LEFT JOIN " . $GLOBALS['ecs']->table('region') . " AS b ON b.region_id = o.province "
                . "LEFT JOIN " . $GLOBALS['ecs']->table('region') . " AS c ON c.region_id = o.city "
                . "LEFT JOIN " . $GLOBALS['ecs']->table('region') . " AS d ON d.region_id = o.district $where ORDER BY o.id ASC LIMIT " . $filter['start'] . "," . $filter['page_size'];
...
function get_data_list($type = 0)
{
    ...
    if ($type != 0) {
        ...
        if ($result === false) {
            ...
            if ($filter['type']) {
                ...
                if ($filter['store_search'] != 0) {
                    if ($adminru['ru_id'] == 0) {
                        ...
                        if ($filter['store_search'] > 1 && $filter['store_search'] != 4) {
                            $where .= " AND (SELECT msi.user_id FROM " . $GLOBALS['ecs']->table('merchants_shop_information') . ' as msi ' .
                                    " WHERE msi.user_id = og.ru_id $store_where) > 0 ";
                        }
                    }
                } else {
            ...
            $sql_all = "SELECT og.goods_id, og.order_id, og.goods_id, og.goods_name, og.ru_id, og.goods_sn, og.goods_price, o.add_time, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, og.goods_number ,sto.store_id " .
                    " FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " AS o " . " ON o.order_id = og.order_id " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table("store_order") . " AS sto ON sto.order_id = o.order_id " .
...
function stat_filter()
{
	...
	$sql = "SELECT COUNT(o.order_id) AS order_num , SUM(o.goods_amount) AS total_fee " .
			" FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og " .
			" LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " AS o " . " ON o.order_id = og.order_id " .
			" LEFT JOIN " . $GLOBALS['ecs']->table("store_order") . " AS sto ON sto.order_id = o.order_id " .
			" WHERE " . $where . " AND sto.store_id > 0  GROUP BY o.order_id ORDER BY og.goods_id DESC ";
	$list = $GLOBALS['db']->getAll($sql);
	$result = array('order_num'=>'', 'total_fee'=>'','goods_list'=>array());
	foreach ($list as $v){
		$result['order_num'] += $v['order_num'];
		$result['total_fee'] += $v['total_fee'];
	}
	$sql_all = "SELECT og.goods_name, og.goods_price, MAX(o.add_time) AS last_sale_time , " .
			" SUM(og.goods_number) AS sales_num ,sto.store_id " .
			" FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og " .
			" LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " AS o " . " ON o.order_id = og.order_id " .
			" LEFT JOIN " . $GLOBALS['ecs']->table("store_order") . " AS sto ON sto.order_id = o.order_id " .
			" WHERE " . $where . " AND sto.store_id > 0  GROUP BY og.goods_id ORDER BY og.goods_id DESC LIMIT 10 ";
