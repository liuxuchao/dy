...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if(!empty($other['mobile_phone']))
    {
        ...
        if($db->getOne($sql) > 0)
        {
            sys_msg($_LANG['mobile_phone_existed'], 1);
        }
    }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , " .
		...
		" LEFT JOIN ".$ecs->table('users_real')." r ON u.user_id = r.user_id ".
...
elseif($_REQUEST['act'] == 'users_log_query')
{
    ...
    $smarty->assign('page_count',   $user_log['page_count']);    
    $smarty->assign('user_id',   $user_id);    
    make_json_result($smarty->fetch('users_log.dwt'), '',
        array('filter' => $user_log['filter'], 'page_count' => $user_log['page_count']));
}
//删除会员日志
elseif($_REQUEST['act'] == 'batch_log'){
    ...
    else
    {
        $ids = $_POST['checkboxes'];
            /* 删除记录 */
            $sql = "DELETE FROM " . $ecs->table('users_log') .
                    " WHERE log_id " . db_create_in($ids)." AND user_id = '$user_id'";
            $db->query($sql);

            /* 清除缓存 */
            clear_cache_files();
            $link[] = array('text' => $_LANG['back'], 'href' => 'users.php?act=users_log&id='.$user_id);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $checked = empty($_REQUEST['checkboxes']) ? [] : $_REQUEST['checkboxes'];
        ...
        $sql = "SELECT u.user_rank,u.user_id, u.user_name, u.nick_name, u.mobile_phone, u.email, u.is_validated, u.user_money, u.frozen_money, u.rank_points, u.pay_points, u.reg_time,rank_points ".
