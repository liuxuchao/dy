...
elseif ($_REQUEST['act'] == 'ajax_insert') {
    ...
    if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
        ...
        //查询数据是否已经存在;
        $sql = "SELECT id FROM " . $GLOBALS['ecs']->table('warehouse_area_attr') . " WHERE goods_id = '" . $result['list']['goods_id'] . "' AND area_id = '" .$result['list']['area_id']. "'" .
...
function get_goods_attr_list($goods_id){
    $where = "";
    if(empty($goods_id)){
        $where = " AND ga.admin_id = '" .$_SESSION['admin_id']. "'";
    }
    ...
    $sql = "SELECT ga.goods_attr_id, ga.goods_id, ga.attr_id, ga.attr_value, ga.attr_sort, ga.admin_id, war.attr_price AS attr_price FROM " .$GLOBALS['ecs']->table("goods_attr") ." AS ga " . 
            " LEFT JOIN " .$GLOBALS['ecs']->table("warehouse_area_attr"). " AS war ON ga.goods_attr_id = war.goods_attr_id AND ga.goods_id = war.goods_id" .
