...
//    $smarty->assign('action_link', array('text' => $_LANG['list_link'], 'href' => 'keywords_manage.php'));
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('keywords_list.dwt'), '',
...
function get_keywords_list() {
    ...
    /* 获得总记录数据 */
//        $sql = 'SELECT distinct COUNT(*) FROM ' .$GLOBALS['ecs']->table('search_keyword');
//        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
//        
//        $filter = page_and_size($filter);
    /* 获取数据 */
    $sql = 'SELECT *' .
            ' FROM ' . $GLOBALS['ecs']->table('search_keyword') . " GROUP BY keyword " .
