...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href'=>'shipping_area.php?act=add&shipping='.$shipping_id,
        'text' => $_LANG['new_area']));
    $smarty->assign('action_link1', array('href'=>'shipping.php?act=list','text' => $_LANG['area_shipping']));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    else
    {
        ...
        if($shipping_code == "cac"){
            ...
            if($district == 0){
                sys_msg($_LANG['select_jurisdiction_region'], 1);
            }
        ...
        if ($point_name) {
            foreach ($point_name as $key => $val) {
                ...
                $sql = "INSERT INTO " . $ecs->table('shipping_point') . " (shipping_area_id, name, user_name, mobile,address,img_url,anchor,line) " .
        ...
        $lnk[] = array('text' => $_LANG['back_list'], 'href'=>'shipping_area.php?act=list&shipping='.$shipping_id);
        $lnk[] = array('text' => $_LANG['add_continue'], 'href'=>'shipping_area.php?act=add&shipping='.$shipping_id);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $shipping_area_name = isset($_POST['shipping_area_name']) && !empty($_POST['shipping_area_name']) ? dsc_addslashes($_POST['shipping_area_name']) : '';
    ...
    $sql = "SELECT COUNT(*) FROM " .$ecs->table("shipping_area").
            " WHERE shipping_id='$_POST[shipping]' AND ".
                    "shipping_area_name='$shipping_area_name' AND ".
                    "shipping_area_id <> '$shipping_area_id' and ru_id='".$adminru['ru_id']."'";
    ...
    else
    {
        ...
        $sql = "UPDATE " .$ecs->table('shipping_area').
                " SET shipping_area_name='$shipping_area_name', ".
                    "configure='" .serialize($config). "' ".
                "WHERE shipping_area_id = '$shipping_area_id' and ru_id='".$adminru['ru_id']."'";

        $db->query($sql);
        
        /* 自提点名称，地址，电话 */
        $point_id = isset($_POST['point_id']) && !empty($_POST['point_id']) ? dsc_addslashes($_POST['point_id'], 0) : array();
        $point_name = isset($_POST['point_name']) ? $_POST['point_name'] : array();
        $user_name = isset($_POST['user_name']) ? $_POST['user_name'] : array();
        $mobile = isset($_POST['mobile']) ? $_POST['mobile'] : array();
        $address = isset($_POST['address']) ? $_POST['address'] : array();
        $old_map_img = isset($_POST['map_img']) ? $_POST['map_img'] : array();
        $anchor = isset($_POST['anchor']) ? $_POST['anchor'] : array();
        $line = isset($_POST['line']) ? $_POST['line'] : array();
        ...
        if ($point_name && $mobile && $address) {
            foreach ($point_name as $key => $val) {
                ...
                if ($point_id[$key]) {
                    $sql = "UPDATE " . $ecs->table('shipping_point') . "SET name='$point_name[$key]' , user_name='$user_name[$key]' ," .
                            " mobile='$mobile[$key]' , address='$address[$key]', img_url='$map_img', anchor='$anchor[$key]', line='$line[$key]'" .
                            "WHERE id = '$point_id[$key]'";
                } else {
                    $sql = "INSERT INTO " . $ecs->table('shipping_point') . " (shipping_area_id, name, user_name, mobile, address, img_url, anchor, line) " .
                            " VALUES ('$shipping_area_id', '$point_name[$key]' , '$user_name[$key]' ,'$mobile[$key]' , '$address[$key]', '$map_img', '$anchor[$key]', '$line[$key]')";
                }
        ...
        /* 清除原有的城市和地区 */
		$db->query("DELETE FROM ".$ecs->table("area_region")." WHERE shipping_area_id='$shipping_area_id' AND ru_id='".$adminru['ru_id']."'");
        ...
        if($shipping_code == "cac"){
            ...
            $sql = "INSERT INTO ".$ecs->table('area_region')." (shipping_area_id, region_id) VALUES ('$shipping_area_id', '$district')";
            $db->query($sql);
        }else{
            /* 添加选定的城市和地区 */
            foreach ($selected_regions AS $key => $val)
            {
            $sql = "INSERT INTO ".$ecs->table('area_region')." (shipping_area_id, region_id,ru_id) VALUES ('$shipping_area_id', '$val','".$adminru['ru_id']."')";
                $db->query($sql);
            }
        }
...
elseif($_REQUEST['act'] == 'remove_point')
{
    ...
    if($db->query($sql))
	{
		$data = array('error' => 2, 'message' => $_LANG['carddrop_succeed'], 'content' => '');
		admin_log($name, 'remove', 'shipping_area');
	}
    else
	{
		$data = array('error' => 0, 'message' => $_LANG['carddrop_fail'], 'content' => '');
	}
	$json  = new JSON;
