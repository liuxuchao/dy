...
elseif ($_REQUEST['act'] == 'allot'){
    ...
    if($adminId > 0){
            $sql = "update " .$ecs->table('admin_user'). " set user_name = '$user_name', email = '$email', " . 
                    "nav_list = '$row[nav_list]', action_list = '$action_list' where ru_id = '$user_id' AND parent_id = 0 AND suppliers_id = 0 ";//by kong 
    }else{
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    if (!empty($login_name)) {
        ...
        if($db->getOne($sql) == 0)
        {
			...
			sys_msg($_LANG['login_name_existent']);
            exit;
            
        }
    } else {
        sys_msg($_LANG['login_name_not_null']);
    }
    
    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " . $ecs->table('admin_user') . " SET action_list = '$act_list', role_id = '' " . $login_name . $login_password . $ec_salt .
            " WHERE ru_id = '$user_id' AND parent_id = 0 AND suppliers_id = 0 ";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    sys_msg($_LANG['carddrop_succeed'], 0, $link);
}
...
elseif($_REQUEST['act'] == 'addBrand'){
    ...
    foreach($shopInfo_list as $k=>$v){
        ...
        foreach($v['steps_title'] as $key=>$val){
            ...
            if($val['steps_style'] == 3 && $val['fields_titles'] == $_LANG['new_brand_info']){
                $title=$val;
            }
        }
    }
...
elseif ($_REQUEST['act'] == 'seller_shopinfo')
{
	...
	$smarty->assign('action_link', array('text' => $_LANG['back'], 'href' => 'merchants_users_list.php?act=list'));
...
elseif ($_REQUEST['act'] == 'save_seller_shopinfo')
{
	...
	if(empty($adminru['ru_id'])){
		$lnk[] = array('text' => $_LANG['go_back_step'], 'href' => 'merchants_users_list.php?act=seller_shopinfo&id='.$adminru['ru_id']);
		sys_msg($_LANG['invalid_data'], 0, $lnk);
	}
...
elseif ($_REQUEST['act'] == 'see_shopinfo'){
    ...
    $smarty->assign('action_link', array('text' => $_LANG['back'], 'href' => 'merchants_users_list.php?act=list'));
    $smarty->assign('ru_id', $adminru['ru_id']);
    // u.user_id, u.user_name, u.mobile_phone,u.email
    $sql = "SELECT * FROM " .
            $GLOBALS['ecs']->table('merchants_shop_information') . " as mis " .
            " LEFT JOIN " . $GLOBALS['ecs']->table('users') . " as u on mis.user_id = u.user_id " .
            " LEFT JOIN " . $GLOBALS['ecs']->table('seller_shopinfo') . " as ss on mis.user_id = ss.ru_id " .
            " WHERE ss.ru_id = '".$user_id."' LIMIT 1";

    $shop_information = $db->getRow($sql);

    $shop_name = get_shop_name($adminru['ru_id']);
    $shop_information['is_dsc'] = $adminru['ru_id'] == 0 ? true : false;//判断当前商家是平台,还是入驻商家

    // 申请时间、开店时间、到期时间
    $shop_information['add_time'] = local_date('Y-m-d H:i:s', $shop_information['add_time']);
    
    // 平台保证金
    $seller_apply_info = get_seller_apply_info($user_id);
    $shop_information['total_amount'] = $seller_apply_info['total_amount'];
    ...
    // 店铺评分
    $merch_cmt = get_merchants_goods_comment($user_id); //商家总体评分
    $smarty->assign('merch_cmt',$merch_cmt);
    // get_print_r($shop_information);exit;
    ...
    $smarty->assign('ur_here', $_LANG['04_self_basic_info']);
    $smarty->display('merchants_users_see_shopinfo.dwt');
}
/*------------------------------------------------------ */
//-- 商家店铺等级到期时间 短信或邮件提醒
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'ajax_send_message'){
    ...
    if ($type == 1 && !empty($to_email)) {
        ...
        $template = [
            ...
            'is_html' => 1,
            'template_content' => '亲爱的{$user_name}，你好！

            您的店铺等级到期时间快过期了，到期时间：
            “{$grade_end_time}”

            如需继续使用，请您及时续费，以免造成不必要的损失。
            {$shop_name}
            {$send_date}'
        ];

        $smarty->assign('user_name',   $user_name);
        $smarty->assign('grade_end_time', $grade_end_time);
        $smarty->assign('shop_name',   "<a href='".$ecs->url()."'>" . $shop_name . '</a>');
        $smarty->assign('send_date',   $send_date);

        $content = $smarty->fetch('str:' . $template['template_content']);

        /* 发送邮件 */
        if (send_mail($user_name, $to_email, $template['template_subject'], $content, $template['is_html'])) {
            $send_ok = 0;
        } else {
            $send_ok = 1;
        }
    ...
    if ($type == 0 && !empty($to_mobile)) {
        ...
        if ($GLOBALS['_CFG']['sms_type'] == 0) {
            $resp = huyi_sms($pt_smsParams, 'sms_seller_grade_time');
        } elseif ($GLOBALS['_CFG']['sms_type'] >= 1) {
            $pt_result = sms_ali($pt_smsParams, 'sms_seller_grade_time'); //阿里大鱼短信变量传值，发送时机传值
            $resp = $GLOBALS['ecs']->ali_yu($pt_result);
        }
        $send_ok = ($resp == true) ? 0 : 1;
    }
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['steps_style'] == 1) {
			...
			switch ($shop_info['shoprz_type']){
				case 1:$shop_info['shoprz_type'] = $GLOBALS['_LANG']['flagship_store'];
				  break;  
				case 2:$shop_info['shoprz_type'] = $GLOBALS['_LANG']['exclusive_shop'];
				  break;
				case 3:$shop_info['shoprz_type'] = $GLOBALS['_LANG']['franchised_store'];
				  break;  
				case 4:$shop_info['shoprz_type'] = $GLOBALS['_LANG']['shop_store'];
				  break;
...
// 商家等级申请记录
function get_seller_apply_info($user_id)
{
    $sql = "SELECT total_amount, apply_status FROM " . $GLOBALS['ecs']->table('seller_apply_info'). " WHERE ru_id = '".$user_id."' ";
