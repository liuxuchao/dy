...
// 			make_json_error('此订单已经完成，不能取消发货');
        ...
        make_json_error($_LANG['order_not_existent']);
    }
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        {
            /* 删除记录 */
			$sql = "DELETE FROM " . $ecs->table('gift_gard_type') .
					" WHERE gift_id " . db_create_in($ids);
			$res = $db->query($sql);
			if($res){
        ...
        elseif ($_POST['type'] == 'review_to')
        {
            ...
            $sql = "UPDATE " . $ecs->table('gift_gard_type') ." SET review_status = '$review_status' "
                . " WHERE gift_id " . db_create_in($ids);
            
            if($db->query($sql))
            {
                $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'gift_gard.php?act=list&seller_list=1&' . list_link_postfix());
                sys_msg($_LANG['gift_card_success'], 0, $lnk);
            }
        }
    }
}
...
if ($_REQUEST['act'] == 'gift_insert') {
    ...
    if ($db->getOne($sql) > 0) {
        $link[0] = array('text' => $_LANG['back_gift_add'], 'href' => 'javascript:history.back(-1)');
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('lang',        $_LANG);
    $smarty->assign('ur_here',     $_LANG['edit_gift_type']);
...
if ($_REQUEST['act'] == 'update')
{
   ...
   /* 提示信息 */
   $link[] = array('text' => $_LANG['gift_list'], 'href' => 'gift_gard.php?act=list&' . list_link_postfix());
...
if ($_REQUEST['act'] == 'take_list')
{
    ...
    /* 赋值是否显示发邮件操作和是否发过邮件 */ elseif ($bonus_type['send_type'] == SEND_BY_USER) {
        $smarty->assign('show_mail', 1);
    }
...
if ($_REQUEST['act'] == 'take_excel')
{
        ...
        foreach ($gift_gard_list['item'] AS $key => $value)
        {
	            ...
	            echo ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['status_ok']) . "\t";
	            echo "\n";
        }
        exit;
}
...
if ($_REQUEST['act'] == 'export_gift_gard')
{
	...
	echo ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['pwd']) . "\t";
...
function get_type_list($ru_id)
{
    ...
    if ($result === false)
    {
        ...
        //管理员查询的权限 -- 店铺查询 end
        $where .= !empty($filter['seller_list']) ? " AND ggt.ru_id > 0 " : " AND ggt.ru_id = 0 "; //区分商家和自营
...
{ 
    ...
    //管理员查询的权限 -- 店铺查询 end
    $where .= !empty($filter['seller_list']) ? " AND bt.ru_id > 0 " : " AND bt.ru_id = 0 "; //区分商家和自营
