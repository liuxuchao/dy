...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $row = $db->getRow("SELECT ctype,cid,ifshow,type, pic FROM " . $GLOBALS['ecs']->table('touch_nav') . " WHERE id = '$id'");

        /* 如果有上传导航图片，删除原来的导航图 */
        if ($pic && $row['pic'])
        {
            @unlink(ROOT_PATH . DATA_DIR . '/attached/nav/'. $row['pic']);
        }
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type'".$where_pic."  WHERE id='$id'";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type'".$where_pic."  WHERE id='$id'";
        }
...
{
    $id = (int)$_GET['id'];
    $row = $db->getRow("SELECT ctype,cid,type, pic FROM " . $GLOBALS['ecs']->table('touch_nav') . " WHERE id = '$id' LIMIT 1");
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type".
               " FROM ".$GLOBALS['ecs']->table('touch_nav').
...
function get_sysnav()
{
    global $_LANG;
    $sysmain = array(
        array($_LANG['index_cart'],'index.php?r=cart'),
        array($_LANG['index_store'],'index.php?r=store'),
        array($_LANG['index_brand'],'index.php?r=brand'),
        array($_LANG['index_activity'],'index.php?r=activity'),
        array($_LANG['index_groupbuy'],'index.php?r=groupbuy'),
        array($_LANG['index_exchange'],'index.php?r=exchange'),
        array($_LANG['index_community'],'index.php?r=community'),
        array($_LANG['index_crowd_funding'],'index.php?r=crowd_funding'),
        array($_LANG['index_auction'],'index.php?r=auction'),
        array($_LANG['index_package'],'index.php?r=package'),
        array($_LANG['index_topic'],'index.php?r=topic'),
        array($_LANG['index_presale'],'index.php?r=presale'),
        array($_LANG['index_article'],'index.php?r=article'),
        array($_LANG['index_user'],'index.php?r=user'),
    );
