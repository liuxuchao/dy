...
elseif (in_array($_REQUEST['act'], array('add', 'edit'))) {
    ...
    $sql = "SELECT credit_money FROM " . $GLOBALS['ecs']->table('seller_shopinfo') .
            "WHERE ru_id = '$id' LIMIT 1 ";
...
elseif (in_array($_REQUEST['act'], array('insert', 'update'))) 
{
    ...
    $sql = "UPDATE " . $GLOBALS['ecs']->table('seller_shopinfo') . " SET credit_money = '$credit_money'" .
            "WHERE ru_id = '$user_id' LIMIT 1 ";
    $db->query($sql);
    /* 商家信用额度 end */
...
elseif ($_REQUEST['act'] == 'toggle_on_settlement') {
    ...
    if ($order_exc['is_frozen']) {
        make_json_error($_LANG['toggle_on_settlement_prompt_one']);
    }
...
elseif ($_REQUEST['act'] == 'bill_remove')
{
    ...
    /* 记日志 */
    admin_log($_LANG['delete_bill'], 'remove', 'seller_commission_bill');
...
function order_download_list($result) {
    ...
    for ($i = 0; $i < $count; $i++) {
		...
		$is_frozen = i($result[$i]['is_frozen']);
...
function merchants_order_list($page = 0) {
    ...
    if ($result === false) {
        ...
        /* 查询 */
        $sql = "SELECT o.is_frozen,o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete, o.is_settlement, chargeoff_status," .
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if($account_log && $account_log['percent_value']){
            ...
            if($bill_id && $row[$i]['chargeoff_status'] > 0){
                
                $sql = "SELECT bill_sn, commission_model, proportion FROM " . $GLOBALS['ecs']->table('seller_commission_bill') . " WHERE id = '$bill_id' LIMIT 1";
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            if ($row[$i]['total_fee'] > 0 && $row[$i]['total_fee'] > $row[$i]['return_amount']) {
...
function commission_download_list($result) {
    ...
    $data = i($GLOBALS['_LANG']['commission_download_title_not'] . "\n");
    $count = count($result);
