...
elseif ($_REQUEST['act'] == 'add')
{
	...
	$smarty->assign('is_need',     $_CFG['template'] == 'ecmoban_dsc2017' ? 1:0); //品牌专区大图 by wu
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_letter, b.brand_first_char, b.site_url, b.brand_logo, b.index_img, b.brand_bg, b.brand_desc, b.brand_logo, b.is_show, b.sort_order,be.is_recommend " .
...
elseif ($_REQUEST['act'] == 'brand_separate_initial')
{
    ...
    if($result['list']){
        ...
        if($result['list']['user_id']){
            ...
            /* 更新商家优惠活动品牌类型 start */
            $sql = "SELECT GROUP_CONCAT(act_id) AS act_id FROM " . $GLOBALS['ecs']->table('favourable_activity') . " WHERE act_range = 2 AND user_id = '" . $result['list']['user_id'] . "' " .
                    "AND FIND_IN_SET('" . $result['list']['bid'] . "', act_range_ext) AND " .  db_create_in($result['list']['bid'], 'user_range_ext', 'NOT'). " LIMIT 1";
            $favourable = $GLOBALS['db']->getRow($sql);
            
            if ($favourable && $favourable['act_id']) {

                $sql = "UPDATE " . $GLOBALS['ecs']->table('favourable_activity') .
                        " SET user_range_ext = act_range_ext " .
                        " WHERE act_id " . db_create_in($favourable['act_id']) . " AND is_user_brand = 0";
                $GLOBALS['db']->query($sql);

                $sql = "UPDATE " . $GLOBALS['ecs']->table('favourable_activity') .
                        " SET act_range_ext =  REPLACE ( act_range_ext, '" . $result['list']['bid'] . "', '" . $result['list']['brand_id'] . "' ), is_user_brand = 1 " .
                        " WHERE act_id " . db_create_in($favourable['act_id']);
                $GLOBALS['db']->query($sql);
            }
            /* 更新商家优惠活动品牌类型 end */

            $result['status_lang'] = '<span style="color: red;">'. $_LANG['update_data_success'] .'</span>';
        }else{
            $result['status_lang'] = '<span style="color: red;">'. $_LANG['update_data_fail'] .'</span>';
        }
    }
