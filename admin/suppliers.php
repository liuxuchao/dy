...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('suppliers_list.dwt'), '',
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($suppliers['suppliers_id'])
    {
        ...
        if ($order_exists > 0)
        {
            make_json_error($_LANG['is_suppliers_order_not_remove']);
        }
        ...
        if ($goods_exists > 0)
        {
            make_json_error($_LANG['is_suppliers_goods_not_remove']);
        }
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        if (isset($_POST['remove']))
        {
            ...
            /* 清除缓存 */
            clear_cache_files();
            $link[] = array('text' => $_LANG['back'], 'href' => 'suppliers.php?act=list');
