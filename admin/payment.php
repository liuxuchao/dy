...
elseif (isset($_POST['Submit']))
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('payment') .
            " WHERE pay_code = '$_POST[pay_code]' AND pay_id <> '" .$_POST['pay_id']. "' AND enabled = 1";
    if ($db->GetOne($sql) > 0)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online)" .
                   "VALUES ('$_POST[pay_code]', '$pay_name', '$_POST[pay_desc]', '$pay_config', '$_POST[is_cod]', '$pay_fee', 1, '$_POST[is_online]')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'edit_pay_order')
{
    /* 检查权限 */
    check_authz_json('payment');
