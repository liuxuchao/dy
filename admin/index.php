...
elseif($_REQUEST['act'] == 'set_clear_cache')
{
    ...
    {
        sys_msg($_LANG['select_dump_target']);
    }
}
...
/* ------------------------------------------------------ */ 
 elseif ($_REQUEST['act'] == 'set_statistical_chart') {
    ...
    if ($type == 'order') {
        ...
        $yAxis['formatter'] = '{value}' . $_LANG['ge'];
        ksort($orders_series_data);
        $series[0]['name'] = $_LANG['order_num'];
    ...
    if ($type == 'sale') {
        ...
        $yAxis['formatter'] = '{value}' . $_LANG['yuan'];
        ksort($sales_series_data);
        $series[0]['name'] = $_LANG['sale_money'];
...
elseif ($_REQUEST['act'] == 'main') {
    ...
    $order['finished'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . " AS oi " .
            " WHERE 1 AND oi.shipping_status = 2 " . $where_og);/* . order_query_sql('finished')*/
    $status['finished'] = CS_FINISHED;

    /* 待发货的订单： */
    $order['await_ship'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . " AS oi JOIN " .$ecs->table('order_goods')." AS og ON og.order_id = oi.order_id WHERE 1 AND (SELECT og.ru_id FROM " . $GLOBALS['ecs']->table('order_goods') .' as og' . " WHERE og.order_id = oi.order_id LIMIT 1) = 0 AND oi.shipping_status = 0 AND oi.pay_status = 2 AND oi.order_status = 1 AND (SELECT ore.ret_id FROM ".$GLOBALS['ecs']->table('order_return')." as ore WHERE ore.order_id = oi.order_id LIMIT 1) IS NULL " . $where_og);// . order_query_sql('await_ship')
    $status['await_ship'] = CS_AWAIT_SHIP;

    /* 待付款的订单： */
    $order['await_pay'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . " AS oi " .
            " WHERE 1 AND oi.pay_status = 0 AND oi.order_status = 1 " . $where_og);// . order_query_sql('await_pay')
    $status['await_pay'] = CS_AWAIT_PAY;

    /* “未确认”的订单 */
    $order['unconfirmed'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . " AS oi " .
            " WHERE 1 AND oi.order_status = 0 " . $where_og);// . order_query_sql('unconfirmed')
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $ecs->table('stats') .' WHERE '. $today_visit_where;
    ...
    $comment_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('comment') .  ' WHERE '. $comment_number_where);
    ...
    $today_comment_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('comment') .' WHERE '. $today_comment_number_where);
    $smarty->assign('today_comment_number', $today_comment_number);

    // 自营实体商品数量
    $platform_real_where = " is_delete= 0 AND user_id = 0 AND is_real = 1 ";
    $platform_real_goods_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE '. $platform_real_where);
    ...
    $platform_virtual_goods_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE '. $platform_virtual_where);
    ...
    $merchants_real_goods_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE ' .$merchants_real_where);
    ...
    $merchants_virtual_goods_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('goods') . ' WHERE ' .$merchants_virtual_where);
    ...
    $today_user_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('users') . ' WHERE ' .$today_user_number_where);
    ...
    $yesterday_user_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('users') . ' WHERE ' .$yesterday_user_number_where);
    ...
    $month_user_number = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('users') . ' WHERE ' .$month_user_number_where);
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $ecs->table('booking_goods') . ' WHERE ' .$booking_goods_where;
    ...
    $new_repay = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('user_account') . ' WHERE '.$new_repay_where);
    ...
    $orders_yAxis = array(
                        'type'=>'value',
                        'axisLabel'=>array(
                                        'formatter'=>'{value}'.$_LANG['ge']));
    $orders_series = array(
                        array(
                            'name'=>$_LANG['order_num'],
                            'type'=>'line',
                            'data'=>array(),
                            'markPoint'=>array(
                                            'data'=>array(
    ...
    $sales_yAxis = array(
                        'type'=>'value',
                        'axisLabel'=>array(
                                        'formatter'=>'{value}' . $_LANG['yuan']));
    $sales_series = array(
                        array(
                            'name'=>$_LANG['sale_money'],
                            'type'=>'line',
                            'data'=>array(),
                            'markPoint'=>array(
                                            'data'=>array(
...
elseif($_REQUEST['act'] == 'shop_top')
{
	...
	$smarty->assign('ur_here',$_LANG['set_store_tou']);
	...
	{
		$lnk[] = array('text' => $_LANG['set_store_info'],'href'=>'index.php?act=first');
		sys_msg($_LANG['set_store_info_alt'], 0, $lnk);
	}
...
elseif($_REQUEST['act'] == 'shop_top_edit')
{
    ...
    $lnk[] = array('text' => $_LANG['return_to_superior'], 'href' => 'index.php?act=shop_top');

    sys_msg($_LANG['set_store_tou_success'], 0, $lnk);
}
 ...
 elseif ($_REQUEST['act'] == 'merchants_second') 
{
    ...
    if (!empty($domain_name)) {
        ...
        if ($db->getOne($sql) > 0) {
            $lnk[] = array('text' => $_LANG['back_home'], 'href' => 'index.php?act=main');
            sys_msg($_LANG['domain_existed'], 0, $lnk);
        }
    }
    ...
    $shop_logo = '';
    if (isset($shop_info['shop_logo'])) {
        $shop_logo = str_replace('../', '', $shop_info['shop_logo']);
    }

    $add_logo_thumb = '';
    if (isset($shop_info['logo_thumb'])) {
    ...
    if ($data_op == 'add') {
        ...
        $lnk[] = array('text' => $_LANG['go_back_step'], 'href' => 'index.php?act=merchants_first');
        sys_msg($_LANG['add_store_info_success'], 0, $lnk);
    } else {
        ...
        $lnk[] = array('text' => $_LANG['go_back_step'], 'href' => 'index.php?act=merchants_first');
        sys_msg($_LANG['update_store_info_success'], 0, $lnk);
    }
}
...
elseif ($_REQUEST['act'] == 'check_order') {
    ...
    //ecmoban模板堂 --zhuo
    $where = "";
    $where_common = " AND (SELECT og.ru_id FROM " . $GLOBALS['ecs']->table('order_goods') .' as og' . " WHERE og.order_id = o.order_id limit 0, 1) = '" .$adminru['ru_id']. "' ";
    $where .= " AND (select count(*) from " .$GLOBALS['ecs']->table('order_info'). " as oi2 WHERE oi2.main_order_id = o.order_id) = 0 ".$where_common;  //主订单下有子订单时，则主订单不显示
    ...
    if(admin_priv('order_view', '', false)){
        ...
        $where_og = "";
        $where_og .= " AND (SELECT count(*) FROM " .$GLOBALS['ecs']->table('order_info'). " AS oi2 WHERE oi2.main_order_id = oi.order_id) = 0 ";  //主订单下有子订单时，则主订单不显示
        $where_og = " AND (SELECT og.ru_id FROM " . $GLOBALS['ecs']->table('order_goods') .' as og' . " WHERE og.order_id = oi.order_id limit 0, 1) = '" .$adminru['ru_id']. "' ";
        $sql = 'SELECT count(*) FROM ' . $ecs->table('order_info') . " AS oi " .
                " WHERE 1 " . order_query_sql('await_ship') . $where_og;		
        $arr['await_ship'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . " AS oi ".
            " WHERE 1 AND (oi.shipping_status = '" .SS_UNSHIPPED. "' OR oi.shipping_status = '" .SS_PREPARING. "') AND oi.pay_status = '" .PS_PAYED. "' AND oi.order_status = '" .OS_CONFIRMED. "' AND (SELECT ore.ret_id FROM " .$GLOBALS['ecs']->table('order_return'). " AS ore WHERE ore.order_id = oi.order_id LIMIT 1) IS NULL " . $where_og);
        /* 新付款的订单 */
        // $sql = 'SELECT COUNT(*) FROM ' . $ecs->table('order_info') . " as o " .
    ...
    if(admin_priv('order_back_apply', '', false)){
        /* 待处理退换货订单 */
        $sql = "SELECT count(ore.ret_id) FROM " .
                $GLOBALS['ecs']->table('order_return')." AS ore LEFT JOIN ".$GLOBALS['ecs']->table('order_info')." AS o ON o.order_id = ore.order_id WHERE o.order_status NOT IN (2,3,4,7,8) $where_common "; //. " WHERE is_check = '0' AND agree_apply = '0' AND return_status = '0' AND refound_status = '0' AND return_type NOT IN('0')";
        $arr['no_change'] = $db->getOne($sql);
    }
	
	if(admin_priv('complaint', '', false)){
        //待处理投诉订单（未完成仲裁）
        $sql = " SELECT COUNT(*) FROM".$ecs->table('complaint')." AS c WHERE c.complaint_state != 4 AND (SELECT og.ru_id FROM " . $GLOBALS['ecs']->table('order_goods') .' as og' . " WHERE og.order_id = c.order_id limit 0, 1) = '" .$adminru['ru_id']. "'  ";
        $arr['complaint'] = $db->getOne($sql);
    }
	
	if(admin_priv('booking', '', false)){
		//待处理缺货商品
		$sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('booking_goods') . " AS b WHERE is_dispose = 0 AND (SELECT g.user_id FROM " . $GLOBALS['ecs']->table('goods') .' AS g' . " WHERE g.goods_id = b.goods_id limit 0, 1) = '" .$adminru['ru_id']. "'  ";
    ...
    if(admin_priv('discuss_circle', '', false)){
        //未处理网友讨论圈审核
        $sql = " SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('discuss_circle') ." AS dc, ".  $GLOBALS['ecs']->table('goods') . " g " . " WHERE dc.review_status = 1 AND dc.goods_id = g.goods_id ";
	...
	if(admin_priv('snatch_manage', '', false)){
		...
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods_activity') .
               " WHERE act_type ='" . GAT_SNATCH . "' AND review_status = 1";
    ...
    if(admin_priv('seckill_manage', '', false)){
        //未审核秒杀
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('seckill') . " WHERE review_status = 1";
        $arr['seckill'] = $db->getOne($sql);
    }
	//促销活动提醒 end
...
elseif ($_REQUEST['act'] == 'platform_recommend') {
    ...
    $http = $ecs->http();
    if(strpos($http, 'https://') === false){
        $Loaction = "http://www.dscmall.cn/cloud/platform_rec.html";
    }else{
        $Loaction = "https://www.dscmall.cn/cloud/platform_rec.html";
    }
...
elseif ($_REQUEST['act'] == 'best_recommend') {
    ...
    $http = $ecs->http();
    if(strpos($http, 'https://') === false){
        $Loaction = "http://www.dscmall.cn/cloud/best_rec.html";
    }else{
        $Loaction = "https://www.dscmall.cn/cloud/best_rec.html";
    }
...
function get_menu_list($menus, $ru_id = 0) {
    ...
    foreach ($menus as $key => $row) {
        ...
        if ($row['label'] == $GLOBALS['_LANG']['02_cat_and_goods']) {
            if ($ru_id > 0) {
        ...
        } elseif ($row['label'] == $GLOBALS['_LANG']['03_goods_storage']) {
            $arr[$key]['href'] = "goods_inventory_logs.php?act=list&step=put";
        } elseif ($row['label'] == $GLOBALS['_LANG']['05_banner']) {
            if ($ru_id > 0) {
        ...
        } elseif ($row['label'] == $GLOBALS['_LANG']['04_order']) {
        ...
        } elseif ($row['label'] == $GLOBALS['_LANG']['20_suppliers_promotion']) {
            $arr[$key]['href'] = "bonus.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['report_form_stats']) {
            $arr[$key]['href'] = "order_stats.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['07_content']) {
            $arr[$key]['href'] = "articlecat.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['user_manage']) {
            $arr[$key]['href'] = "users.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['10_priv_admin']) {
            $arr[$key]['href'] = "privilege.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['11_system']) {
            if($ru_id > 0){
                    $arr[$key]['href'] = "warehouse.php?act=list";
            }else{
                    $arr[$key]['href'] = "shop_config.php?act=list_edit";
            }
        } elseif ($row['label'] == $GLOBALS['_LANG']['03_template']) {
            $arr[$key]['href'] = "template.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['13_backup']) {
            $arr[$key]['href'] = "sql.php?act=main";
        } elseif ($row['label'] == $GLOBALS['_LANG']['14_sms']) {
            $arr[$key]['href'] = "sms.php?act=display_send_ui";
        } elseif ($row['label'] == $GLOBALS['_LANG']['15_rec']) {
            $arr[$key]['href'] = "affiliate.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['16_email_manage']) {
            $arr[$key]['href'] = "view_sendlist.php?act=list";
        } elseif ($row['label'] == $GLOBALS['_LANG']['seller_apply_manage']) {   
            if($ru_id > 0){
                    $arr[$key]['href'] = "merchants_commission.php?act=list";
            }else{
                    $arr[$key]['href'] = "merchants_users_list.php?act=list";
            }    
        } elseif ($row['label'] == $GLOBALS['_LANG']['goods_batch_manage']) {
            $arr[$key]['href'] = "goods_warehouse_batch.php?act=add";
        } elseif ($row['label'] == $GLOBALS['_LANG']['store_set_manage']) {
