...
elseif ($_REQUEST['act'] == 'rollback')
{
    ...
    $links[] = array('text' => $_LANG['affiliate_ck'], 'href' => 'affiliate_ck.php?act=list&uselastfilter=1');
    sys_msg($_LANG['edit_ok'], 0 ,$links);
}
/*
    分成
...
function get_affiliate_ck()
{
    ...
    if ($result === false)
    {
        ...
        $status = is_numeric($_REQUEST['status']) && !empty($_REQUEST['status']) ? intval($_REQUEST['status']) : 0;
        ...
        $sqladd .= " AND (SELECT og.ru_id FROM " .$GLOBALS['ecs']->table('order_goods'). " AS og WHERE og.order_id = o.order_id AND og.ru_id = 0 LIMIT 1) = 0"; //只显示平台分成订单
