...
function return_order_list()
{
    ...
    if ($result === false) {
        ...
        switch ($filter['composite_status']) {
            ...
            case CS_ORDER_BACK :
                ...
                //$where .= " AND o.order_status NOT IN (2,3,4,5,8)";//退货                    
                break;
            default:
                if ($filter['composite_status'] != -1) {
        ...
        $sql = "SELECT DISTINCT r.ret_id ,o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.goods_amount, o.discount," .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('users') . " AS u ON u.user_id=o.user_id  " . $leftJoin . $where . $where_store .
    ...
    foreach ($row AS $key => $value) {
        ...
        if ($return_number == $all_goods_number) {
            $row[$key]['discount_amount'] = number_format($value['discount']);
        } else {
        ...
        if ($is_ajax == 1) {
            if ($filter['order_id_list']) {
                $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') .
                    " SET `order_status` = '" . OS_SPLITED . "', `shipping_status` = '" . SS_RECEIVED . "',`pay_status` = '" . PS_PAYED . "',confirm_take_time = '" . gmtime() . "' " .
...
function get_cat_level($parent_id = 0, $level = 0, $table = 'category', $goods_table = 'goods')
{
    ...
    $sql = "SELECT c.cat_id, c.cat_name, c.parent_id, c.is_show, c.sort_order $select " .
    ...
    foreach ($res as $k => $row) {
        if ($table == 'category') {
            ...
            if ($extension_goods_array) {
                $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table($goods_table) . " WHERE is_delete = 0 AND user_id = '" . $adminru['ru_id'] . "'" .
                    " AND review_status >= 3 AND is_real = 1 " .
                    " AND (cat_id " . db_create_in($cat_keys) . " OR goods_id " . db_create_in($cat_keys) . ")";
            } else {
                $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table($goods_table) . " WHERE is_delete = 0 AND user_id = '" . $adminru['ru_id'] . "'" .
                    " AND review_status >= 3 AND is_real = 1 " .
                    " AND cat_id " . db_create_in($cat_keys);
            }
