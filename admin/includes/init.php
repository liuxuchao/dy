...
{
    @mkdir('../temp/compiled/' . ADMIN_PATH, 0777);
    @chmod('../temp/compiled/' . ADMIN_PATH, 0777);
}
        ...
        else
        {
            ...
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['admin_pass'])
            {
                ...
                $db->query('UPDATE ' . $ecs->table('admin_user') .
                            ...
                            " WHERE user_id = '" . intval($_SESSION['admin_id']) . "'");
            }
            else
            {
                setcookie($_COOKIE['ECSCP']['admin_id'],   '', 1);
...
if(isset($_SESSION['admin_name'])){
    ...
    $uname = '';
    if($_SESSION['admin_id'] > 0 && $_SESSION['admin_id'] != $uid){
