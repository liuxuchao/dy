...
$page_libs = array(
    ...
    'goods' => array(
		...
		'/library/history_goods.lbi' => 0,
        '/library/recommend_best_goods.lbi' => 3, 
		'/library/recommend_new_goods.lbi' => 3,
        '/library/recommend_hot_goods.lbi' => 3,
		//ecmoban模板堂 --zhuo end
    ...
    'exchange_list' => array(
        ...
        '/library/history.lbi' => 0,
        '/library/pages.lbi' => 0,
        '/library/exchange_hot.lbi' => 3,
        '/library/exchange_best.lbi' => 5, //ecmoban模板堂 --zhuo
...
function get_template_info($template_name, $template_style='')
{
    ...
    if (file_exists($css_path) && !empty($template_name))
    {
		...
		$business_logo_filename = explode(': ', $arr[8]);
        $template_type      = explode(': ', $arr[9]);
...
function get_template_region($tmp_name, $tmp_file, $lib=true)
{
	...
	if(!file_exists($file))
	{
		sys_msg($GLOBALS['_LANG']['have_no_file']);
	}
