    ...
    if ($search_type == "goods") {
        ...
        foreach ($arr AS $key => $val) {
            $opt[] = array('value' => $val['goods_id'],
                'text' => $val['goods_name']."--".price_format($val['shop_price']),
                'data' => $val['shop_price']);
        }
        $filter_list = $opt;
    }
...
//-- 将文章商品添加关联
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'add_link_artic_goods')
{
...
//-- 将文章商品删除关联
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_link_artic_goods')
{
    $drop_goods = empty($_REQUEST['value']) ? array() : explode(",", $_REQUEST['value']);
    $article_id = empty($_REQUEST['article_id']) ? 0 : intval($_REQUEST['article_id']);
    $result = array('error' => 0, 'message' => '', 'content' => '');
...
elseif ($_REQUEST['act'] == 'add_group_goods') {
    ...
    if ($groupCount < 1000) {
			...
			foreach($goods_ids as $key=>$val){
				...
				if (!$db->getOne($sql)) {
					...
					$sql = "INSERT INTO " . $ecs->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id, group_id) " .
							"VALUES ('$goods_id', '$val', '$price_goods', '$_SESSION[admin_id]', '$group_id')"; //by mike add
					$db->query($sql, 'SILENT');
				}
			}
		}
        ...
        $error = 1;
        $message = $_LANG['add_group_goods_notic'];
    }
	//获取全部的配件
...
//-- 添加秒杀商品
/* ------------------------------------------------------ */ 
elseif ($_REQUEST['act'] == 'add_seckill_goods') {
	...
	if($goods_ids){
		...
		foreach($goods_ids_arr as $val){
			...
			if (!$db->getOne($sql)) {
				$sql = "INSERT INTO " . $ecs->table('seckill_goods') . " (sec_id, tb_id, goods_id) " .
...
//-- 删除秒杀商品
/* ------------------------------------------------------ */ 
elseif ($_REQUEST['act'] == 'drop_seckill_goods') {
    $fittings  = empty($_REQUEST['value'])  ? array() : explode(",", $_REQUEST['value']);
    $sec_id    = empty($_REQUEST['sec_id']) ? 0 	  : intval($_REQUEST['sec_id']);
	$tb_id 	   = empty($_REQUEST['tb_id'])  ? 0 	  : intval($_REQUEST['tb_id']);
    $result = array('error' => 0, 'message' => '', 'content' => '');

    $sql = "DELETE FROM " . $ecs->table('seckill_goods') .
            " WHERE " . db_create_in($fittings, 'id');
    ...
    foreach ($arr AS $val) {
        $opt[] = array(
			'value' => $val['id'],
            'text' => $_LANG['sec_price_alt'] . $val['sec_price'] . $_LANG['sec_number_alt'] .$val['sec_num']. $_LANG['sec_restrictions_alt'] .$val['sec_limit'].'】' .$val['goods_name'],
            'data' => '');
    }
...
elseif ($_REQUEST['act'] == 'upload_img') {
    ...
    if (isset($_POST)) {
        ...
        if (empty($name)) {
            $result['error'] = $_LANG['not_select_img'];
        }
        $type = strtolower(substr(strrchr($name, '.'), 1)); //获取文件类型		
        if (!in_array($type, $typeArr)) {
            $result['error'] = $_LANG['cat_prompt_file_type'];
        }
    }
    ...
    if ($act_type == 'goods_img') {
        ...
        if ($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') {
            if (empty($is_url_goods_img)) {
                $original_img = $image->upload_image($_FILES['goods_img'], array('type' => 1)); // 原始图片
            }
        ...
        if ($img) {
            ...
            $sql = "INSERT INTO " . $ecs->table($table) . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
    ...
    if ($upload_status) { //临时文件转移到目标文件夹
        ...
        $result['error'] = $_LANG['upload_error_server_set'];
    }
    die(json_encode($result));
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'add_albun_pic')
{
    ...
    if (!$is_only) {
        $result['error'] = 0;
        $result['content'] = $_LANG['album'] . "’" . $album_mame . "‘" . $_LANG['existence'];
        die(json_encode($result));
    }
    /* 取得文件地址 */
    $file_url = '';
    if ((isset($_FILES['album_cover']['error']) && $_FILES['album_cover']['error'] == 0) || (!isset($_FILES['album_cover']['error']) && isset($_FILES['album_cover']['tmp_name']) && $_FILES['album_cover']['tmp_name'] != 'none')) {
        ...
        if (!check_file_type($_FILES['album_cover']['tmp_name'], $_FILES['album_cover']['name'], $allow_file_types)) {
            $result['error'] = 0;
            $result['content'] = $_LANG['album_prompt_file_type'];
            die(json_encode($result));
        }
    ...
    $time = gmtime();
    $sql = "INSERT INTO" . $ecs->table("gallery_album") . "(`album_mame`,`album_cover`,`album_desc`,`sort_order`,`add_time`,`ru_id`)"
            . " VALUES ('$album_mame','$file_url','$album_desc','$sort_order','$time','" . $adminru['ru_id'] . "')";
    $db->query($sql);
    $result['error'] = 1;
    ...
    $html = '<li><a href="javascript:;" data-value="0" class="ftx-01">'.$_LANG['select_please'].'</a></li>';
    if (!empty($album_list)) {
        ...
        if($result['hierarchy'] == 3){
            ...
            // end
                ...
                $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img $search FROM " .
                        $GLOBALS['ecs']->table('goods') . " AS g " . $leftjoin . $where . $limit;
...
/*获取图库列表*/
elseif($_REQUEST['act'] == 'gallery_album_list'){
    $json = new JSON;
    ...
    if($gallery_album_list || $album_id > 0){
       ...
       $sql = "SELECT album_mame FROM " . $ecs->table('gallery_album') . " "
...
//信息栏
elseif($_REQUEST['act'] == 'insertVipEdit'){
    $json = new JSON;
    $attr = array();
    $result = array('error' => 0, 'message' => '', 'content' => '');
    $result['mode'] = !empty($_REQUEST['mode'])  ?  trim($_REQUEST['mode'])  :  '';
...
//广告栏信息处理
elseif($_REQUEST['act'] == 'homeAdvInsert'){
    $json = new JSON;
    $attr = array();
    $allow_file_types = '|GIF|JPG|PNG|';
    $result = array('error' => 0, 'message' => '', 'content' => '');
...
//获取指定分类的下级分类
elseif($_REQUEST['act'] == 'getChildCat'){
    $json = new JSON;
...
elseif($_REQUEST['act'] == 'homeFloor'){
    ...
    $result['sub_title'] = !empty($_REQUEST['sub_title']) ? trim($_REQUEST['sub_title']) : ''; //楼层标题
    ...
    $result['leftBannerTitle'] = !empty($_REQUEST['leftBannerTitle']) ? $_REQUEST['leftBannerTitle'] : ''; //左侧轮播住标题
    $result['leftBannerSubtitle'] = !empty($_REQUEST['leftBannerSubtitle']) ? $_REQUEST['leftBannerSubtitle'] : ''; //左侧轮播副标题
    ...
    if ($result['moded'] == 'homeFloorFour' || $result['moded'] == 'homeFloorFive' || $result['moded'] == 'homeFloorSeven' || 
	 ...
	 //  start 获取卖场筛选条件
        ...
        $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img  FROM " .
                $GLOBALS['ecs']->table('goods') . " AS g " . $where_goods . " ORDER BY g.sort_order, g.goods_id DESC " . $limit;
        $goods_list = $db->getAll($sql);
        if (!empty($goods_list)) {
            foreach ($goods_list as $key => $val) {
                if ($val['promote_price'] > 0 && $time >= $val['promote_start_date'] && $time <= $val['promote_end_date']) {
...
elseif($_REQUEST['act'] == 'edit_cmsarti') {
    ...
    $result = array('content' => '');
    $spec_attr = '';
    $result['sort'] = !empty($_REQUEST['sort']) ? $_REQUEST['sort'] : array(); //分类排序
    $result['article_id'] = !empty($_REQUEST['article_id']) ? $_REQUEST['article_id'] : array(); //文章id
    $result['moded'] = !empty($_REQUEST['mode']) ? trim($_REQUEST['mode']) : '';
    $result['def_article_id'] = !empty($_REQUEST['def_article_id']) ? $_REQUEST['def_article_id'] : array(); //默认首位文章id数组
    $arr = '';
    $sort_vals = '';
    //处理数组
    if (!empty($result['sort'])) {
        foreach ($result['sort'] as $k => $v) {
            ...
            $arr[$k]['first_article_list'] = array();//默认首位文章
            ...
            $sql = 'SELECT a.article_id, a.content, a.title, a.add_time, a.description, a.file_url, a.open_type ' .
                    ' FROM ' . $GLOBALS['ecs']->table('article') . ' AS a ' .
                    ' WHERE a.is_open = 1 ' . $where .
                    ' ORDER BY a.article_type DESC, a.sort_order ASC, a.article_id DESC LIMIT 5';
...
elseif ($_REQUEST['act'] == 'edit_cmsgoods') {
    ...
    $spec_attr['hierarchy'] = !empty($_REQUEST['hierarchy']) ? intval($_REQUEST['hierarchy']) : '';
    $result['moded'] = !empty($_REQUEST['mode']) ? trim($_REQUEST['mode']) : '';
    $result['goods_ids'] = !empty($_REQUEST['goods_ids']) ? trim($_REQUEST['goods_ids']) : ''; //商品id
    $result['article_ids'] = !empty($_REQUEST['select_article_ids']) ? trim($_REQUEST['select_article_ids']) : ''; //文章id
    $result['article_title'] = !empty($_REQUEST['article_title']) ? trim($_REQUEST['article_title']) : $_LANG['recent_hot']; //文章标题
    $result['goods_title'] = !empty($_REQUEST['goods_title']) ? trim($_REQUEST['goods_title']) : $_LANG['best_recommend']; //商品标题
    $result['cat_id'] = !empty($_REQUEST['cat_id']) ? intval($_REQUEST['cat_id']) : '0'; //商品分类
    $result['articat_id'] = !empty($_REQUEST['articat_id']) ? intval($_REQUEST['articat_id']) : '0'; //商品分类
    $result['def_article'] = !empty($_REQUEST['def_article']) ? intval($_REQUEST['def_article']) : '0'; //商品分类
    //获取选中商品
    $where_goods = 'WHERE 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.review_status > 2 ';
    if ($result['goods_ids']) {
        $where_goods .= " AND g.goods_id in (" . $result['goods_ids'] . ")";
    }
    $sql = "SELECT  g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.original_img  FROM " .
            $GLOBALS['ecs']->table('goods') . " AS g " . $where_goods . " ORDER BY g.sort_order, g.goods_id DESC LIMIT 4";
    $goods_list = $db->getAll($sql);
    if (!empty($goods_list)) {
        foreach ($goods_list as $key => $val) {
            $goods_list[$key]['goods_thumb'] = get_image_path($val['goods_id'], $val['goods_thumb']);
    ...
    $sql = 'SELECT a.article_id, a.content, a.title, a.add_time, a.description, a.file_url, a.open_type ' .
            ' FROM ' . $GLOBALS['ecs']->table('article') . ' AS a ' .
            ' WHERE a.is_open = 1 AND article_type=0 ' . $where .
            ' ORDER BY a.article_type DESC, a.sort_order ASC LIMIT 5';
    $article_list = $db->getAll($sql);
...
//CMS频道头部广告
elseif($_REQUEST['act'] == 'edit_cmsIn'){
    $json = new JSON;
    $result = array('content' => '');
    $spec_attr = '';
    $spec_attr['hierarchy'] = !empty($_REQUEST['hierarchy']) ? intval($_REQUEST['hierarchy']) : '';
    ...
    $result['leftBannerLink'] = !empty($_REQUEST['leftBannerLink']) ? $_REQUEST['leftBannerLink'] : ''; //左侧轮播链接
    $result['leftBannerSort'] = !empty($_REQUEST['leftBannerSort']) ? $_REQUEST['leftBannerSort'] : ''; //左侧轮播排序
    $result['leftBannerTitle'] = !empty($_REQUEST['leftBannerTitle']) ? $_REQUEST['leftBannerTitle'] : ''; //左侧轮播，描述

    $result['leftAdv'] = !empty($_REQUEST['leftAdv']) ? $_REQUEST['leftAdv'] : ''; //左侧广告
    $result['leftAdvLink'] = !empty($_REQUEST['leftAdvLink']) ? $_REQUEST['leftAdvLink'] : ''; //左侧广告链接
    $result['leftAdvSort'] = !empty($_REQUEST['leftAdvSort']) ? $_REQUEST['leftAdvSort'] : ''; //左侧广告排序
    $result['leftAdvTitle'] = !empty($_REQUEST['leftAdvTitle']) ? $_REQUEST['leftAdvTitle'] : ''; //左侧广告描述
...
elseif($_REQUEST['act'] == 'get_childcat'){
    ...
    $smarty->assign('cat_select',  $article_cat);//获取文章分类
    $smarty->assign("temp", $_REQUEST['act']);
...
//获取指定文章分类下所有分类的文章
elseif($_REQUEST['act'] == 'getcat_atr'){
     $json = new JSON;
    $result = array('content' => '');
    ...
    $where = '';
    $article_list = array('list'=>array(),'filter'=>array());
    if($type == 1){
    ...
    if(!empty($spec_attr['goods_ids'])){
        if(!empty($PromotionType)){
            ...
            if($PromotionType == 'exchange')
            {
                if(empty($spec_attr['title'])){
                    $spec_attr['title'] = $_LANG['integral_mall'];
                }
                $search = ', ga.exchange_integral';
                $leftjoin = " LEFT JOIN ".$GLOBALS['ecs']->table('exchange_goods')." AS ga ON ga.goods_id=g.goods_id ";
            ...
            {
                if(empty($spec_attr['title'])){
                    $spec_attr['title'] = $_LANG['16_presale'];
                }
                $search = ',ga.act_id, ga.act_name, ga.end_time, ga.start_time ';
                $leftjoin = " LEFT JOIN ".$GLOBALS['ecs']->table('presale_activity')." AS ga ON ga.goods_id=g.goods_id ";
                $where .= " AND ga.review_status = 3 AND ga.start_time <= '$time' AND ga.end_time >= '$time' AND g.goods_id <> '' AND ga.is_finished =0"; 
            ...
            else
            {
                if($PromotionType == 'snatch'){
                    if(empty($spec_attr['title'])){
                        $spec_attr['title'] = $_LANG['02_snatch_list'];
                    }
                    $act_type = GAT_SNATCH;
                }elseif($PromotionType == 'auction'){
                    if(empty($spec_attr['title'])){
                        $spec_attr['title'] = $_LANG['10_auction'];
                    }
                    $act_type = GAT_AUCTION;
                }elseif($PromotionType == 'group_buy'){
                    if(empty($spec_attr['title'])){
                        $spec_attr['title'] = $_LANG['08_group_buy'];
                    }
                    $act_type = GAT_GROUP_BUY;
                }
                $search = ',ga.act_id, ga.act_name, ga.end_time, ga.start_time, ga.ext_info';
        ...
        elseif($spec_attr['moded'] == 'h-seckill'){
            ...
            $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb,"
                    . " g.shop_price, g.market_price, g.original_img , sg.sec_price,sg.id FROM".$ecs->table('goods')." AS g "
                    . "LEFT JOIN ".$ecs->table('seckill_goods')." AS sg ON sg.goods_id=g.goods_id "
                    . "LEFT JOIN ".$ecs->table('seckill')."AS s ON s.sec_id=sg.sec_id".$where;
            
        }
        else
        {
            if(empty($spec_attr['title'])){
                $spec_attr['title'] = $_LANG['time_limit_promotion'];
            }
             $where .= " AND g.is_on_sale=1 AND g.is_delete=0 ";
            if($GLOBALS['_CFG']['review_goods'] == 1){
                $where .= ' AND g.review_status > 2 ';
            ...
            $where .= " AND g.goods_id".db_create_in($spec_attr['goods_ids']);
            $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.original_img FROM " . 
...
//-- 图片库商品上传图片
/* ------------------------------------------------------ */ 
elseif ($_REQUEST['act'] == 'lib_upload_img') {
    include_once(ROOT_PATH . '/includes/cls_image.php');
    $image = new cls_image($_CFG['bgcolor']);
    require_once(ROOT_PATH . '/' . ADMIN_PATH . '/includes/lib_goods.php');
    ...
    if ($act_type == 'goods_img') {
        ...
        if ($img) {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods_lib_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('$goods_id', '$gallery_img', $img_desc, '$gallery_thumb', '$img')";
            $db->query($sql);
            $thumb_img_id[]=$GLOBALS['db']->insert_id();
            get_oss_add_file(array($gallery_img, $gallery_thumb, $img));
...
//-- 获取选择商品内容模板
/* ------------------------------------------------------ */ 
elseif($_REQUEST['act'] == 'getsearchgoodsDiv'){
    $json = new JSON;
    $result = array();
    $goods_ids = !empty($_REQUEST['goods_ids'])  ?  trim($_REQUEST['goods_ids'])  :  '';
...
//-- 商品类型入库
/* ------------------------------------------------------ */ 
elseif($_REQUEST['act'] == 'add_goods_type'){
    $json = new JSON;
    $result = array('error' => 0, 'message' => '', 'content' => '');
    $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
    $parent_id = !empty($_REQUEST['attr_parent_id']) ? intval($_REQUEST['attr_parent_id']) : 0;
...
//-- 商品类型入库
/* ------------------------------------------------------ */ 
elseif($_REQUEST['act'] == 'attribute_add'){
    $json = new JSON;
    $result = array('error' => 0, 'message' => '', 'content' => '');
    ...
    {
        $result['error'] = 1;
        $result['message'] = $_LANG['attr_name_repeat'];
        die(json_encode($result));
    }
    /* 取得属性信息 */
    $attr = array(
