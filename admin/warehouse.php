...
/*------------------------------------------------------ */
 ...
 elseif ($_REQUEST['act'] == 'ship_list') {
    ...
    $action_link = array('text' => $_LANG['warehouse_list'], 'href' => 'warehouse.php?act=list');
    $smarty->assign('action_link', $action_link);
    $action_link2 = array('text' => $_LANG['warehouse_freight_template'], 'href' => 'warehouse.php?act=ship_list');
    $smarty->assign('action_link2', $action_link2);
    $smarty->assign('ur_here', $_LANG['warehouse_freight_template']);
...
elseif($_REQUEST['act'] == 'multi_remove')
{
    ...
    if ($db->query($sql)) {
        $data = $_LANG['remove_success'];
    } else {
        $data = $_LANG['remove_fail'];
    }
...
if ($_REQUEST['act'] == 'tpl_list')
{
    ...
    $smarty->assign('ur_here', $_LANG['warehouse_freight_formwork_list'] . $shipping['shipping_name']);
    $smarty->assign('action_link2', array('href' => 'warehouse.php?act=ship_tpl&shipping_id=' . $shipping_id,
        'text' => $_LANG['add_freight_formwork']));
    $smarty->assign('action_link', array('href' => 'warehouse.php?act=ship_list', 'text' => $_LANG['back_dispatching_llist']));
    $smarty->assign('full_page', 1);
...
elseif ($_REQUEST['act'] == 'ship_tpl')
{
    ...
    if (!empty($row)) {
        //插入一条记录
        $smarty->assign('ur_here', $_LANG['edit_warehouse_freight_formwork'] . $row['shipping_name']);
        /* 自提点信息 */
        ...
        $smarty->assign('ur_here', $_LANG['warehouse_freight_formwork'] . $shipping['shipping_name']);
        $set_modules = 1;
    ...
    /* 仓库运费模板 by wu */
    $smarty->assign('action_link2', array('href' => 'warehouse.php?act=tpl_list&shipping_id=' . $shipping_id, 'text' => $_LANG['back_template_llist']));
...
{	
    ...
    if ($shipping_id == 0 || empty($tpl_name) || empty($warehouse_id) || empty($regionId)) {
        $add_to_mess = $_LANG['info_fillin_complete'];
        ...
        $add_to_mess = $_LANG['add_freight_success'];
    }
    ...
    foreach ($res as $key => $val) {
        ...
        if ($warehouse_state && $region_state) {
            $add_to_mess = $_LANG['template_arrive_region'];
...
/*------------------------------------------------------ */
 ...
 elseif ($_REQUEST['act'] == 'add_area') {
    ...
    $sql = "INSERT INTO " . $ecs->table('region_warehouse') . " (regionId, parent_id, region_name, region_code, region_type) " .
...
elseif ($_REQUEST['act'] == 'edit_region_code')
{
    check_authz_json('warehouse_manage');
...
{	
    ...
    if($shipping_id == 0){
            $add_to_mess = $_LANG['select_dispatching_mode'];
    ...
    if ($id) {
        ...
        $add_to_mess = $_LANG['freight_edit_success'];
    } else {
        $db->autoExecute($ecs->table('warehouse_freight'), $other, 'INSERT');
        $add_to_mess = $_LANG['freight_add_success'];
    }
