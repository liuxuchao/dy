...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = 'SELECT pay_name FROM ' . $GLOBALS['ecs']->table('payment') .
                " WHERE pay_id = '$pay_id'";
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if ($account['is_paid'] == 0)
    {
        //获取申请会员信息
        $user_info = array();
        if($is_paid == 1 && intval($account['user_id']) > 0){
        ...
        if ($is_paid == 1 && $account['process_type'] == 1)
        {
            ...
            if ($order_note && isset($order_note[1]) && $order_note[1]) {
                ...
                if ($order['ru_id']) {
                    ...
                    $change_desc = $_LANG['operator'] . $adminru['user_name'] . $_LANG['operator_two'] . $order['order_sn'] . "】" . $refund_note;
                    $log = array(
                        'user_id' => $order['ru_id'],
                        'user_money' => $amount,
                        'change_time' => gmtime(),
            ...
            if ($GLOBALS['_CFG']['user_account_code'] == '1' && $user_info['mobile_phone'] != '') { //添加条件 by wu
                if ($GLOBALS['_CFG']['sms_type'] == 0) {
                    ...
                    $result = sms_ali($smsParams, 'user_account_code'); //阿里大鱼短信变量传值，发送时机传值
        ...
        elseif ($is_paid == 0 || $is_paid == 2)
        {
            ...
            $sql = "UPDATE " .$ecs->table('user_account'). " SET ".
                   ...
                   "admin_note    = '$admin_note', ".
                    $set .
                   " WHERE id = '$id'";
            if($db->query($sql) && $is_paid == 2){
                $sql = "SELECT u.user_name, u.user_id, ua.amount FROM " . $ecs->table('users') . " AS u, " .
                       $ecs->table('user_account') . " AS ua " .
                       " WHERE u.user_id = ua.user_id AND ua.id = '$id' ";
                $row = $db->getRow($sql);
                /* 保存 */
                log_account_change($row['user_id'], abs($row['amount']), $row['amount'], 0, 0, "【" . $_LANG['Application_withdrawal'] . $_LANG['dismiss_application'] . "】" , ACT_ADJUSTING, 2);                
            }
        }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    {
        $sql = "DELETE FROM " . $ecs->table('user_account_fields') . " WHERE account_id = '$id'"; 
        $db->query($sql);
...
function get_user_surplus($user_id) {
    $sql = "SELECT user_money, frozen_money FROM " . $GLOBALS['ecs']->table('users') .
