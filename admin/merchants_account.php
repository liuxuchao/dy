...
elseif ($_REQUEST['act'] == 'update_check')
{
    ...
    if ($log_status > 0 && $log_status <= 2) {
        ...
        $msg_type = 1;
        $text = $_LANG['go_back'];
        if ($log_info['payment_info']['pay_name'] == $_LANG['bank_remittance']) {
...
elseif ($_REQUEST['act'] == 'change_amount')
{
    ...
    if($GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_account_log'), $result, 'UPDATE', 'log_id ='.$log_id)){
        $result['error'] = 0;
    }else{
        $result['error'] = 1;
    }
