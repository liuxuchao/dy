...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',     $_LANG['ad_list']);
     $smarty->assign('action_link', array('text' => $_LANG['ads_add'], 'href' => 'touch_ads.php?act=add&pid='.$pid));
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
        'FROM ' . $GLOBALS['ecs']->table('touch_ad_position') . $ruCat . ' ORDER BY position_id DESC';
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link',   array('href' => 'touch_ads.php?act=list', 'text' => $_LANG['ad_list']));
    $smarty->assign('position_list', get_touch_position_list());
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('touch_ad')." AS a, ".
            $ecs->table('touch_ad_position') ." AS p ".
    ...
    $link[1]['href'] = 'touch_ads.php?act=list';
...
elseif ($_REQUEST['act'] == 'update')
{
   ...
   /* 提示信息 */
   $href[] = array('text' => $_LANG['back_ads_list'], 'href' => 'touch_ads.php?act=list');
 ...
 * @return
 */
function get_touch_position_list()
{
    //ecmoban模板堂 --zhuo start
    $adminru = get_admin_ru_id();
    $ruCat = '';
