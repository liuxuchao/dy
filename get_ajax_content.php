        ...
        case 'mpbtn_yhq':
            ...
            foreach ($goods_coupons as $k => $v) {
                ...
                if (!in_array($v['cou_id'], $arr)) {
                    ...
                    $v['cou_start_time'] = local_date('Y-m-d', $v['cou_start_time']);
                    $v['cou_end_time'] = local_date('Y-m-d', $v['cou_end_time']);
                    $v['shop_name'] = get_shop_name($v['ru_id'], 1);
                    if($ru_id > 0){
...
elseif($act=='changeShow')
 {
    ...
    if ($type == 1) {
        ...
        $havealook = get_category_recommend_goods('rand', $child_children, 0, 0, 0, '', $region_id, $area_id, $area_city, 6);
        ...
        if ($cat_id > 0) {
            if ($tpl == 2) {
                $child_children = get_children($cat_id);
                $goods_list = get_category_recommend_goods('rand', $child_children, 0, 0, 0, '', $region_id, $area_id, $area_city, 8);
                $smarty->assign('goods_list', $goods_list);
                $result['page'] = html_entity_decode($smarty->fetch('library/cat_goods_change.lbi'));
            } elseif ($tpl == 3) {
                $child_children = get_children($cat_id);
                $goods_list = get_category_recommend_goods('rand', $child_children, 0, 0, 0, '', $region_id, $area_id, $area_city, 6);
                $smarty->assign('goods_list', $goods_list);
                $result['page'] = html_entity_decode($smarty->fetch('library/cat_goods_change.lbi'));
            }
        } else {
...
elseif($act=='floor_cat_content')
 {
    ...
    $floorcat = isset($_REQUEST['floorcat']) && !empty($_REQUEST['floorcat']) ? intval($_REQUEST['floorcat']) : 0;//模板标识
