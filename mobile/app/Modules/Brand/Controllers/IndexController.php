...
class IndexController extends FrontendController
{
    ...
    public function __construct()
    {
        ...
        $this->other_area_id = $warehouse_area_info['city_id'];
        cookie('other_area_id ', $this->other_area_id);
        /* 获得请求的平台品牌ID */
        $this->brand_id = I('get.id', 0, 'intval');

        $this->is_ship = I('request.ship', 0, 'intval');//by wang是否包邮
        $this->sort = I('request.sort', 'goods_id', ['htmlspecialchars','trim']);
        $this->order = I('request.order', 'DESC', ['htmlspecialchars','trim']);
        /* 获得请求的商家品牌ID */
            ...
            if ($list) {
                if ($list['top']) {
                    ...
                    foreach ($list['top'] as $key => $val) {
                        $list['top'][$key]['goods'] = brand_get_goods($val['brand_id'], $this->mbid, $this->cate, $this->size, $this->page, $this->sort, $this->order, $this->region_id, $this->area_id, $act, $this->is_ship, $this->price_min, $this->price_max,$this->other_area_id);
                        $arr_tmp = [];
                ...
                if ($list['center']) {
                    ...
                    foreach ($list['center'] as $key => $val) {
                        ...
                        $list['center'][$key]['goods'] = brand_get_goods($val['brand_id'], $this->mbid, $this->cate, $this->size, $this->page, $this->sort, $this->order, $this->region_id, $this->area_id, $act, $this->is_ship, $this->price_min, $this->price_max,$this->other_area_id);
                        $arr_tmp = [];
    ...
    {
        $brand_num = dao('brand')->where(['is_show' => 1])->count();
        if ($brand_num > 1000) {
            $this->actionNavList();
        } else {
            $list = S('brand_list'.$brand_num);
            if ($list === false) {
                $list = get_brands_letter('brand', 1000, 1);
                S('brand_list'.$brand_num, $list);
            }
    ...
    {
        $cat_id = input('request.cat_id', 0, 'intval');
        $page = input('request.page', 1, 'intval');
        $size = input('request.size', 10);
        $keyword = input('request.keyword', '', ['html_in', 'trim']);

        if (IS_AJAX) {
            $brand_cache = md5(serialize($_REQUEST));
            $list = S($brand_cache);
            if ($list == false) {
                $list = get_brands_list($cat_id, $keyword, $size, $page);
                S($brand_cache, $list);
            }
            exit(json_encode(['list' => $list['list'], 'totalPage' => $list['totalpage']]));
        }

        //获得品牌的分类
        $brand_cat = S('cat_brand_cache');
        if ($brand_cat === false) {
            $brand_cat = get_categories_tree(0, 1);
            S('cat_brand_cache', $brand_cat);
        }
        $this->assign('brand_cat', $brand_cat);
    ...
    public function actionDetail()
    {
        ...
        // 用于异步请求品牌商品列表
        $type = I('request.type', '', ['htmlspecialchars','trim']);  //商品属性
        ...
        $count = goods_count_by_brand($brand_id, $mbid, $this->cate, $act, $this->is_ship, $this->price_min, $this->price_max, $this->region_id, $this->area_id, $this->self,$this->other_area_id);
        $goods_list = brand_get_goods($brand_id, $mbid, $this->cate, $this->size, $this->page, $this->sort, $this->order, $this->region_id, $this->area_id, '', $this->is_ship, $this->price_min, $this->price_max, $this->type,$this->other_area_id);

        // 新品 精品 推荐
        $new_count = goods_count_by_brand($brand_id, $mbid, $this->cate, $act, $this->is_ship, $this->price_min, $this->price_max, $this->region_id, $this->area_id, $this->self, 'new',$this->other_area_id);
        $hot_count = goods_count_by_brand($brand_id, $mbid, $this->cate, $act, $this->is_ship, $this->price_min, $this->price_max, $this->region_id, $this->area_id, $this->self, 'hot',$this->other_area_id);
        $promote_count = goods_count_by_brand($brand_id, $mbid, $this->cate, $act, $this->is_ship, $this->price_min, $this->price_max, $this->region_id, $this->area_id, $this->self, 'promote',$this->other_area_id);
        $best_count = goods_count_by_brand($brand_id, $mbid, $this->cate, $act, $this->is_ship, $this->price_min, $this->price_max, $this->region_id, $this->area_id, $this->self, 'best',$this->other_area_id);
        ...
        foreach ($seo as $key => $value) {
            $seo[$key] = html_in(str_replace(['{sitename}', '{description}', '{name}'], [C('shop.shop_name'), $brand_info['brand_desc'],$brand_info['brand_name']], $value));
        }
    ...
    public function actionGoods()
    {
        if (IS_AJAX) {
            ...
            $size = 10;
            $act = '';
            $type = I('get.type', '', ['htmlspecialchars','trim']);
            $goods_list = brand_get_goods_ajax($brand_id, $this->mbid, $this->cate, $size, $page, $this->sort, $this->order, $this->region_id, $this->area_id, $act, $this->is_ship, $this->price_min, $this->price_max, $type,$this->other_area_id);
