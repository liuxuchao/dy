...
{
    $start = ($page - 1) * $size;
    $sql = "SELECT brand_id, brand_name, brand_logo, brand_first_char FROM {pre}brand WHERE is_show = 1 GROUP BY brand_id , sort_order  LIMIT $start , $size";
    $res = $GLOBALS['db']->query($sql);
    $arr = [];
    foreach ($res as $row) {
        $brand['brand_id'] = $row['brand_id'];
        $brand['brand_name'] = trim($row['brand_name']);
        $brand['url'] = url('brand/index/detail', ['id' => $row['brand_id']]);
        $brand['brand_logo'] = get_data_path($row['brand_logo'], 'brandlogo/');
        // $brand['goods_num'] = goods_count_by_brand($row['brand_id']);
        $first = !empty($row['brand_first_char']) ? $row['brand_first_char'] : getLetter($brand['brand_name']);
 ...
 * @return  integer
 */
function goods_count_by_brand($brand_id, $mbid = 0, $cate = 0, $act = '', $ship = 0, $price_min = 0, $price_max = 0, $warehouse_id = 0, $area_id = 0, $self = 0, $type = '',$area_city = 0)
{
    ...
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        "WHERE " . $type_sql . "  g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $cate_where $tag_where ";
 ...
 * @return array
 */
function brand_get_goods($brand_id, $mbid = 0, $cate, $size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $act = '', $ship = '', $price_min, $price_max, $type = '',$area_city = 0)
{
    $cate_where = ($cate > 0) ? 'AND ' . get_children($cate) : '';
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, g.market_price, g.is_new,g.is_promote,g.model_attr, g.is_best,g.is_hot,g.shop_price AS org_price,g.sales_volume, ' .
        $shop_price .
        "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
        "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, " .
        'g.promote_start_date, g.promote_end_date, g.is_promote, g.goods_brief,g.product_price,g.product_promote_price, g.goods_thumb , g.goods_img ' .
        'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        $leftJoin .
        'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
...
function brand_get_goods_ajax($brand_id, $mbid = 0, $cate, $size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $act = '', $ship = '', $price_min = 0, $price_max = 0, $type = '',$area_city = 0)
{
    $cate_where = ($cate > 0) ? 'AND ' . get_children($cate) : '';
...
function brand_related_cat($brand)
{
    $arr[] = [
        'cat_id' => 0,
        'cat_name' => L('all_category'),
        'url' => url('brand/index/detail', ['id' => $brand])
    ];
    ...
    foreach ($res as $key => $row) {
        $row['url'] = url('brand/index/detail', ['cat' => $row['cat_id'], 'id' => $brand]);
        $arr[] = $row;
    }
 ...
 * @return
 */
function get_brands_list($cat = 0, $keyword = '',$size = 10, $page = 0)
{
    /* 获得品牌列表 */
    $where = '';
    if (!empty($keyword)) {
        $val = mysql_like_quote(trim($keyword));
        $where .= " (b.brand_name LIKE '%$val%') ";
    } else {
        $children = ($cat > 0) ? '1 AND ' . get_children($cat) : 1;
        $where .= $children;
    }

    $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table('brand') . " as b ".
            ...
            " WHERE " . $where . " AND b.is_show = 1 ORDER BY b.sort_order ASC";
    $total = $GLOBALS['db']->getOne($sql);

    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.index_img, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
    ...
    $list = [];
    foreach ($res as $row) {
        $list[$row['brand_id']]['brand_id'] = $row['brand_id'];
        $list[$row['brand_id']]['brand_name'] = trim($row['brand_name']);
        $list[$row['brand_id']]['url'] = url('brand/index/detail', ['id' => $row['brand_id']]);
        $list[$row['brand_id']]['brand_logo'] = get_data_path($row['brand_logo'], 'brandlogo/');
        $list[$row['brand_id']]['index_img'] = get_data_path($row['index_img'], 'indeximg/');
    }

    return ['list' => array_values($list), 'totalpage' => ceil($total / $size)];
}