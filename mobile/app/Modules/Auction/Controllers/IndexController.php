...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_AJAX) {
            ...
            die(json_encode(['list' => $auction_list, 'totalPage' => $page_count]));
        }
    ...
    public function actionBid()
    {
        ...
        if ($user_id <= 0) {
            $url = url('auction/index/detail', ['id' => $id]);
            show_message(L('au_bid_after_login'), '登录', url('user/login/index', ['back_act' => urlencode($url)]));
        }
        ...
        if ($auction['deposit'] > 0) {
            ...
            if ($auction['bid_user_count'] > 0) {
                log_account_change(
                    ...
                    0,
                    0,
                    sprintf(L('au_unfreeze_deposit'), $auction['act_name'])
                );
            }
            ...
            log_account_change(
                ...
                0,
                0,
                sprintf(L('au_freeze_deposit'), $auction['act_name'])
            );
        }
