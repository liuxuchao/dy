...
function auction_list($keywords, $sort, $order, $size, $page)
{
    ...
    $sql = "SELECT a.*, IFNULL(g.goods_thumb, '') AS goods_thumb " .
        ...
        "AND a.start_time <= '$now' AND a.end_time >= '$now' AND a.is_finished < 2 AND a.review_status = 3 ORDER BY $sort $order";
...
function get_exchange_recommend_goods($type = '', $cats = '', $min = 0, $max = 0, $ext = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, ' .
        ...
        "AND ga.start_time <= '$now' AND ga.end_time >= '$now' AND ga.is_finished < 2 AND ga.review_status = 3" . $price_where . $ext;
    ...
    foreach ($res as $row) {
        ...
        $auction[$idx]['url'] = build_uri('auction', ['auid' => $row['act_id'], $row['act_name']]);
