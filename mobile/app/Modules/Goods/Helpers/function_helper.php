...
function get_also_bought($goods_id)
{
    ...
    foreach ($res as $key => $row) {
        ...
        $arr[$key]['url'] = build_uri('goods', ['gid' => $row['goods_id']]);
...
function get_linked_goods($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    //ecmoban模板堂 --zhuo end
    //g.setting_sales,
    $sql = 'SELECT g.goods_id, g.goods_name,g.model_attr, g.goods_thumb, g.goods_img, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' .
        ...
        ' g.market_price, g.sales_volume, g.model_attr,g.promote_start_date,g.product_price,g.product_promote_price, g.promote_end_date ' .
...
/**
 * 查询第一张商品相册图（排序同商品详情页） 用于生成分享二维码图片
 * @param  int $goods_id
 * @return
 */
function get_one_goods_gallery($goods_id)
{
    $row = dao('goods_gallery')->field('img_url')->where(['goods_id' => $goods_id])->order('img_desc ASC')->find();
    $row['img_url'] = get_image_path($row['img_url']);
    return $row;
}

// 评分等级
function comment_font($key)
{
    if ($key > 4) {
        return L('height');
    } elseif ($key > 3) {
