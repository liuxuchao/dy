...
class Admin extends PluginController
{
    ...
    public function creatQrpayCode($id = 0)
    {
        ...
        if (!file_exists($filename)) {
            ...
            $img = new Image();
            // 生成水印缩略图
            $img->open($logo)->thumb(80, 80, Image::IMAGE_THUMB_FILLED)->save($logo_out);
            // 生成原图+水印
    ...
    public function marketQrpayLogList()
    {
        ...
        $handler = input('get.handler', '', ['htmlspecialchars','trim']);
        $function = input('get.function', '', ['htmlspecialchars','trim']);
    ...
    public function marketExportQrpayLog()
    {
        if (IS_POST) {
            ...
            if ($list) {
                ...
                $this->message('该时间段没有要导出的数据', null, 2, $this->ru_id);
            }
        }
