...
class Ddcx extends PluginController
{
    ...
    public function returnData($fromusername, $info)
    {
        ...
        $articles = ['type' => 'text', 'content' => '暂无订单信息'];
        $users = get_wechat_user_id($fromusername);
        if (empty($users) || empty($users['mobile_phone'])) {
            $articles = ['type' => 'text', 'content' => '无法查询订单，请先 <a href="'.url('oauth/index/index', ['type' => 'wechat', 'refer' => 'user'], false, true).'">绑定手机号</a>'];
            return $articles;
        }
        if (!empty($users)) {
            //订单ID
            ...
            if (isset($order_id_arr[0]) && !empty($order_id_arr[0]['order_id'])) {
                ...
                $articles['content'][0]['Url'] = __HOST__ . url('user/order/detail', ['order_id' => $order['order_id']]);
                // 积分赠送
