...
class IndexController extends FrontendController
{
    ...
    {
        $userinfo = $this->weObj->getUserInfo($fromusername);
        if (!empty($userinfo) && !empty($userinfo['unionid'])) {
            $uid = dao('wechat_user')->where(['openid' => $fromusername, 'unionid' => $userinfo['unionid'], 'subscribe' => 1, 'wechat_id' => $this->wechat_id])->getField('uid');
            if ($uid) {
                $data['uid'] = $uid;
                $data['msg'] = $keywords;
                $data['wechat_id'] = $this->wechat_id;
    ...
    public function send_custom_message($openid = 0, $msgtype = '', $data)
    {
        ...
        if ($msgtype == 'text') {
             ...
             *  )
             */
            array_key_case($data); // 将多维数组key转换为小写
            $newsData = $data;
            $msg = [
                'touser' => $openid,
                'msgtype' => 'news',
