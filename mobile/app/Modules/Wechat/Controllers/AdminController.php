...
class AdminController extends BackendController
{
    ...
    public function __construct()
    {
        ...
        $condition['default_wx'] = 1;
        $wechatinfo = $this->model->table('wechat')->field('*')->where($condition)->find();
        if (empty($wechatinfo)) {
            $data = [
                'id' => $this->wechat_id,
                'time' => gmtime(),
    ...
    public function actionSubscribeList()
    {
        ...
        foreach ($list as $key => $value) {
            ...
            $users = get_connect_user($value['unionid']);
            $list[$key]['from'] = get_wechat_user_from($value['from']);
            if (!empty($users)) {
                $list[$key]['user_name'] = $users['user_name'];
                $list[$key]['look_user_url'] = '../' . ADMIN_PATH . '/users.php?act=edit&id=' . $users['user_id']; // 查看商城会员
            }
        }
        // 标签
    ...
    public function actionSubscribeSearch()
    {
        ...
        $keywords = I('request.keywords', '', ['htmlspecialchars','trim']);
        ...
        if (isset($tag_id) && $tag_id > 0) {
            $rightjoin = "RIGHT JOIN {pre}wechat_user_tag t ON u.openid = t.openid";
    ...
    public function actionSendCustomMessage()
    {
        ...
        if (IS_POST) {
            ...
            $msgtype = I('msgtype', '', ['htmlspecialchars','trim']);

            $form = new Form();
            if (!$form->isEmpty($openid, 1)) {
                exit(json_encode(['status' => 1, 'msg' => L('select_openid')]));
            }

            if ($msgtype == 'text' && !$form->isEmpty($data['msg'], 1)) {
                exit(json_encode(['status' => 1, 'msg' => L('message_content') . L('empty')]));
            }
            ...
            if (empty($rs)) {
                ...
                exit(json_encode(['status' => 1, 'msg' => $errmsg]));
            }
            // 添加数据
            dao('wechat_custom_message')->data($data)->add();
            exit(json_encode(['status' => 0, 'msg' => '发送成功！']));
        }
        $uid = I('get.uid');
        $openid = I('get.openid');
        if ($openid) {
        ...
        // 最新发送的消息6条
        $condition = ['uid' => $uid, 'wechat_id' => $this->wechat_id];
        if ($openid && $info) {
            $condition = ['uid' => $info['uid'], 'is_wechat_admin' => 1, 'wechat_id' => $this->wechat_id];
        }
    ...
    public function actionBatchTagging()
    {
        ...
        if (IS_POST) {
            ...
            if (is_array($openlist)) {
                ...
                $num = count($openlist);
                if ($num >= 50) {
                    $this->message(L('批量加入标签数量一次不能超过50'), url('subscribe_list'), 2);
                }
                // 微信端打标签
    ...
    public function actionEditUserRemark()
    {
        ...
        if (IS_POST) {
            $remark = input('remark', '', ['htmlspecialchars','trim']);
        ...
        if (IS_POST) {
            ...
            if ($data['type'] == 0) {
                if (!empty($data['expire_seconds']) && $data['expire_seconds'] > 2592000) {
                    exit(json_encode(['status' => 0, 'msg' => '临时二维码最大不能超过30天']));
                }
                if ($data['scene_id'] < 100001 || $data['scene_id'] > 4294967295) {
                    exit(json_encode(['status' => 0, 'msg' => '临时二维码取值区间 100001 - 4294967295']));
                }
            }
            // 永久二维码
            if ($data['type'] == 1) {
                if ($data['scene_id'] < 1 || $data['scene_id'] > 100000) {
                    exit(json_encode(['status' => 0, 'msg' => '永久二维码取值区间 1- 100000']));
                }
            }
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                $list[$key]['share_account'] = $this->model->table('affiliate_log')
                    ...
                    ->getField('sum(money)');
                $list[$key]['status'] = return_qrcode_status($val['id'], $val['scene_id']);
            }
        }
        $this->assign('page', $this->pageShow($total[0]['count']));
        $this->assign('list', $list);
    ...
    public function actionShareEdit()
    {
        ...
        if (IS_POST) {
            ...
            if ($id) {
                ...
                $status = dao('wechat_qrcode')->where($where)->getField('status');
                if ($status == 0) {
                    exit(json_encode(['status' => 0, 'msg' => L('qrcode_islosed')]));
                }
    ...
    public function actionVoice()
    {
        ...
        if (IS_POST) {
            if ($_FILES['voice']['name']) {
                $type = ['audio/amr', 'audio/x-mpeg', 'audio/mp3']; // amr,mp3
        ...
        if (IS_POST) {
            ...
            if (!empty($article_info['article_id'])) {
                ...
                foreach ($articles as $key => $val) {
                    $artinfo = $this->model->table('wechat_media')
                        ->field('id, title, command, author, file, is_show, digest, content, link')
                    ...
                    $article[$key]['content_source_url'] = empty($artinfo['link']) ? __HOST__ . url('article/index/wechat', ['id' => $artinfo['id']]) : strip_tags(html_out($artinfo['link']));
                ...
                $article[0]['content_source_url'] = empty($article_info['link']) ? __HOST__ . url('article/index/wechat', ['id' => $article_info['id']]) : strip_tags(html_out($article_info['link']));
    ...
    public function actionRuleEdit()
    {
        if (IS_POST) {
            ...
            // 主表数据
            $data['rule_name'] = I('post.rule_name', '', ['htmlspecialchars','trim']);
    ...
    public function actionTemplate()
    {
        ...
        if (IS_POST) {
            ...
            if (empty($data['primary_industry']) || empty($data['secondary_industry'])) {
                $this->message('请选择主行业和副行业！', null, 2);
            }
            ...
            F('TMIndustry', null, ROOT_PATH . 'storage/app/wechat/');
            $this->message('设置' . L('success'), null);
        }
        ...
        // 显示所在行业
        $industry = F('TMIndustry', '', ROOT_PATH . 'storage/app/wechat/');
        if ($industry === false) {
            $industry = $this->weObj->getTMIndustry();
            if (empty($industry)) {
                $error = L('errcode') . $this->weObj->errCode . L('errmsg') . $this->weObj->errMsg;
                $this->assign('error', '请登录微信公众号平台，申请开通模板消息');
            } else {
                F('TMIndustry', $industry, ROOT_PATH . 'storage/app/wechat/');
            }
        }
    ...
    public function actionResetTemplate()
    {
        ...
        if (IS_AJAX) {
            ...
            if (!empty($id)) {
                $condition['id'] = $id;
                $condition['wechat_id'] = $this->wechat_id;
                $template = dao('wechat_template')->field('template_id')->where($condition)->find();
                if (!empty($template['template_id'])) {
                    ...
                    dao('wechat_template')->data(['template_id' => '', 'status' => 0])->where(['id' => $id])->save();
    ...
    public function actionExtendEdit()
    {
        ...
        if (IS_POST) {
                    ...
                    if (empty($cfg_value['media_id'])) {
                        ...
                        if ($media_id) {
                            ...
                            if (file_exists($sql_file)) {
                                ...
                                $sql = str_replace(['ecs_wechat_media', '(0', 'http://', 'views/images'], ['{pre}wechat_media', '(' . $this->wechat_id, __HOST__ . url('wechat/index/plugin_show', ['name' => $this->plugin_name]), 'public/assets/wechat/' . $this->plugin_name . '/images'], $sql);
            ...
            $this->message('安装编辑成功', url('extend_index'));
        }
    ...
    public function actionExtendUninstall()
    {
        ...
        $this->message('卸载成功', url('extend_index'));
    }
    ...
    public function actionWinnerList()
    {
        ...
        $ks = I('get.ks', '', ['htmlspecialchars','trim']);
        if (empty($ks)) {
            $this->message('请选择插件', null, 2);
        }
        // 初始化 每页分页数量
        $page_num = isset($_COOKIE['ECSCP']['page_size']) && !empty($_COOKIE['ECSCP']['page_size']) ? $_COOKIE['ECSCP']['page_size'] : 10;
        $this->assign('page_num', $page_num);

        $keywords = I('request.keywords', '', ['htmlspecialchars','trim']); // 搜索关键词
        $where = '';
        if (!empty($keywords)) {
            $where .= ' and (u.nickname like "%' . $keywords . '%" )'; // 支持昵称搜索
        }
    ...
    public function actionWinnerIssue()
    {
        ...
        $id = I('get.id', 0, 'intval');
        $cancel = I('get.cancel');
        $activity_type = I('get.ks', '', ['htmlspecialchars','trim']);
        if (empty($id)) {
        ...
        if (!empty($cancel)) {
            ...
            $this->message('取消成功', url('winner_list', ['ks' => $activity_type]));
        } else {
            $data['issue_status'] = 1;
            $this->model->table('wechat_prize')->data($data)->where(['id' => $id, 'wechat_id' => $this->wechat_id])->save();

            $this->message('发放成功', url('winner_list', ['ks' => $activity_type]));
        }
    }
    ...
    public function actionWinnerDel()
    {
        ...
        $this->message('删除成功', url('winner_list', ['ks' => $activity_type]));
    }
    ...
    public function actionExportWinner()
    {
        ...
        if (IS_POST) {
            ...
            $sql = "SELECT p.id, p.prize_name, p.issue_status, p.winner, p.dateline, p.openid, u.nickname FROM {pre}wechat_prize p LEFT JOIN {pre}wechat_user u ON p.openid = u.openid WHERE p.activity_type = '" . $ks . "' and u.wechat_id = '" . $this->wechat_id . "' and p.prize_type = 1 and u.subscribe = 1 AND " . $where . " ORDER BY dateline desc ";
            $list = $this->model->query($sql);
            if ($list) {
                ...
                //设置标题
                $rowVal = ['编号', '微信昵称', '奖品', '是否发放', '中奖用户信息', '中奖时间'];
                ...
                foreach ($list as $k => $v) {
                    ...
                    $excel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('E' . $num, $v['winner'])
                        ->setCellValue('F' . $num, local_date("Y-m-d H:i:s", $v['dateline']));
                }
                ...
                $this->message('没有可导出数据', null, 2);
            }
        }

        $this->redirect('winner_list', ['ks' => $ks]);
    }
