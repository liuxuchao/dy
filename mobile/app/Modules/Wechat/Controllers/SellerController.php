...
class SellerController extends BackendSellerController
{
    ...
    public function actionMenuEdit()
    {
        ...
        if (IS_POST) {
            ...
            if ('click' == $data['type']) {
                ...
                $data['url'] = add_url_suffix($data['url'], ['wechat_ru_id' => $this->ru_id]);
                $data['key'] = '';
            }
            // 编辑
            if (!empty($id)) {
    ...
    public function actionBatchTagging()
    {
        ...
        if (IS_POST) {
            ...
            if (is_array($openlist)) {
                ...
                $num = count($openlist);
                if ($num >= 50) {
                    $this->message(L('批量加入标签数量一次不能超过50'), url('subscribe_list'), 2, true);
                }
                // 微信端打标签
    ...
    public function actionTemplate()
    {
        ...
        if (IS_POST) {
            ...
            if (empty($data['primary_industry']) || empty($data['secondary_industry'])) {
                $this->message('请选择主行业和副行业！', null, 2, true);
            }
            ...
            $this->message('设置' . L('success'), null, 1, true);
        }
                    ...
                    if (empty($cfg_value['media_id'])) {
                        ...
                        if ($media_id) {
                            ...
                            if (file_exists($sql_file)) {
                                ...
                                $sql = str_replace(['ecs_wechat_media', '(0', 'http://', 'views/images'], ['{pre}wechat_media', '(' . $this->wechat_id, __HOST__ . url('wechat/index/plugin_show', ['name' => $this->plugin_name, 'wechat_ru_id' => $this->ru_id]), 'public/assets/wechat/' . $this->plugin_name . '/images'], $sql);
    ...
    public function actionMarketList()
    {
        ...
        if (class_exists($plugin)) {
            ...
            $config['wechat_ru_id'] = $this->ru_id;
