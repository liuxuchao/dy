...
class IndexController extends FrontendController
{
    ...
    public function actionList()
    {
        ...
        if (IS_AJAX) {
            ...
            $sort = (isset($_REQUEST['sort']) && in_array(trim(strtolower(I('sort'))), ['shop_price', 'start_time', 'act_id'])) ? I('sort', '', ['htmlspecialchars','trim']) : $default_sort_order_type;
            $order = (isset($_REQUEST['order']) && in_array(trim(strtoupper(I('order'))), ['ASC', 'DESC'])) ? I('order', '', ['htmlspecialchars','trim']) : $default_sort_order_method;
    ...
    public function actionNew()
    {
        ...
        if (IS_AJAX) {
            ...
            foreach($res as $key =>$val){
                ...
                $res[$key]['url'] = build_uri('presale', ['r' => 'index/detail', 'id' => $val['act_id']]);
            }
            $pre_goods = $this->get_pre_goods($cat_id, $status, $sort, $order, $page, $size, $keyword);
            die(json_encode(['list' => $res, 'totalPage' => ceil($pre_goods['total'] / $size)]));
        }
        // 预售分类
        $cat_id = I('get.id', 0);
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('presale_cat') . " ORDER BY sort_order ASC ";
    ...
    public function actionDetail()
    {
        ...
        $goods = get_goods_info($this->goods_id, $this->region_id, $this->area_info['region_id'],$this->area_city);
        if (empty($goods)) {
            ecs_header("Location: ./\n");
            exit;
        }
        ...
        $this->assign('best_goods', get_recommend_goods('best', '', $this->region_id, $this->area_info['region_id'],$this->area_city, $goods['user_id'], 1, 'presale'));    // 推荐商品
        $this->assign('new_goods', get_recommend_goods('new', '', $this->region_id, $this->area_info['region_id'],$this->area_city, $goods['user_id'], 1, 'presale'));     // 最新商品
        $this->assign('hot_goods', get_recommend_goods('hot', '', $this->region_id, $this->area_info['region_id'],$this->area_city, $goods['user_id'], 1, 'presale'));     // 最新商品
    ...
    public function actionPrice()
    {
        ...
        $goods = get_goods_info($this->goods_id, $warehouse_id, $area_id,$this->area_city);
        if ($this->goods_id == 0) {
            ...
            //ecmoban模板堂 --zhuo start
            $products = get_warehouse_id_attr_number($this->goods_id, $_REQUEST['attr'], $goods['user_id'], $warehouse_id, $area_id, $this->area_city);
            ...
            $shop_price = get_final_price($this->goods_id, $number, true, $attr_id, $warehouse_id, $area_id, $this->area_city);
            //ecmoban模板堂 --zhuo end
            ...
            //属性价格
            $spec_price = get_final_price($this->goods_id, $number, true, $attr_id, $warehouse_id, $area_id, $this->area_city, 1, 0, 0, $res['show_goods'], $product_promote_price);
    ...
    public function actionBuy()
    {
        ...
        if ($specs) {
            ...
            $product_info = get_products_info($goods['goods_id'], $_specs, $warehouse_id, $area_id, $this->area_city);
        }
