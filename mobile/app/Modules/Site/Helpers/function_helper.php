...
function goods_list($type, $page, $size, $warehouse_id = 0, $area_id = 0,$area_city)
{
    ...
    $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, ' . $shop_price . ' g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, ' .
        ...
        'g.promote_start_date, g.promote_end_date, g.is_promote, g.goods_brief, g.goods_thumb , g.goods_img, g.cat_id ' .
        'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        $leftJoin .
        'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        "WHERE $where ORDER BY g.sort_order ASC , g.goods_id DESC LIMIT $page , $size";
    ...
    foreach ($goods_list as $key => $val) {
        ...
        $goods_list[$key]['url'] = url('goods/index/index', ['id' => $val['goods_id'], 'u' => $_SESSION['user_id']]);
        ...
        //添加相似商品链接
        $goods_list[$key]["link_url"] = url('category/index/search', ['id' => $val['cat_id']]);
    }
 ...
 *
 */
function limit_grab($warehouse_id = 0, $area_id = 0,$area_city = 0)
{
    if(C('shop.area_pricetype') == 1){
        $where_area = "AND wag.city_id = '$area_city'";
    }
    ...
    $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, ' . $shop_price . ' g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, ' .
        ...
        " AND g.promote_start_date < $time AND g.promote_end_date > $time " . $review_goods . " GROUP BY g.goods_id ORDER BY g.sort_order ASC, g.goods_id DESC LIMIT 5";
    $goods_list = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($goods_list as $key => $val) {
        ...
        $goods_list[$key]['url'] = build_uri('goods', ['gid' => $val['goods_id']], $val['goods_name']);
...
function get_store()
{
    ...
    foreach ($store_list as $key => $val) {
        ...
        $store_list[$key]['url'] = build_uri('store', ['stid' => $val['user_id']]);
        $sql = "SELECT goods_id,goods_name,goods_thumb,goods_img FROM {pre}goods WHERE user_id = " . $val['user_id'] . " AND " .
            " is_on_sale = 1 AND is_alone_sale = 1 AND " . "is_delete = 0 $review_goods ORDER BY sort_order,goods_id desc LIMIT 4";
        $goods_list = $GLOBALS['db']->getAll($sql);
