...
class IndexController extends FrontendController
{
    ...
    public function actionSend()
    {
        ...
        if (!$this->check_verify($this->sms_code)) {
            exit(json_encode(['msg' => '图片验证码不正确']));
        }

        if (empty($this->mobile)) {
            exit(json_encode(['msg' => L('mobile_notnull')]));
        }

        // 验证手机号格式
        if (is_mobile($this->mobile) == false) {
            exit(json_encode(['msg' => L('mobile_format_error')]));
        }
        if ($_SESSION['sms_mobile']) {
            if (strtotime($this->read_file($this->mobile)) > (time() - 60)) {
                exit(json_encode(['msg' => L('msg_wait_auth_code')]));
            }
        }
        ...
        if ($this->flag == 'register') {
            //手机注册
            if (!empty($user_id)) {
                exit(json_encode(['msg' => L('change_mobile')]));
            }
        } elseif ($this->flag == 'forget') {
            //找回密码
            if (empty($user_id)) {
                exit(json_encode(['msg' => L('mobile_number_Unknown')]));
            }
        }
        ...
        if ($send_result === true) {
            ...
            exit(json_encode(['code' => 2, 'msg' => L('send_auth_code')]));
        } else {
            // exit(json_encode(array('msg' => $send_result)));
            exit(json_encode(['msg' => '短信发送失败']));
        }
    }
    ...
    public function actionCheck()
    {
        if ($this->mobile != $_SESSION['sms_mobile'] or $this->mobile_code != $_SESSION['sms_mobile_code']) {
            exit(json_encode(['msg' => L('mobile_auth_code_error'), 'code' => 1]));
        } else {
