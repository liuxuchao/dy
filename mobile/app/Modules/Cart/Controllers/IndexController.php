...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_AJAX && C('shop.wap_category') == '1') {
            $this->response(['error' => 0, 'goods_list' => $cart_goods['goods_list'], 'total' => $cart_goods['total']]);
        } else {
            $this->assign('goods_list', $merchant_goods_list);
            $this->assign('total', $cart_goods['total']); // 购物车商品数&购物车总价
    ...
    public function actionHeart()
    {
        if (IS_AJAX) {
            if ($_SESSION['user_id'] > 0) {
                $goods_id = input('id', '', ['html_in', 'trim']);
                $status = input('status', 0, 'intval');

                $goods_arr = explode(',', substr($goods_id, 0, str_len($goods_id) - 1));
                ...
                if (count($goods_arr) > 0) {
                    if ($status == 0) {
                        foreach ($goods_arr as $val) {
                            ...
                            $map['goods_id'] = $val;
                            $rs = dao('collect_goods')->where($map)->count();
                            //dd($map);
                            if ($rs < 1) {
                                $data['user_id'] = $_SESSION['user_id'];
                                $data['goods_id'] = $val;
                                $data['add_time'] = gmtime();
                                dao('collect_goods')->data($data)->add();
                            }
                        }
                        die(json_encode(['msg' => L('already_attention_check_shop'), 'status' => 1, 'error' => 0]));
                    } else {
                        $map['user_id'] = $_SESSION['user_id'];
                        $map['goods_id'] = array('in', $goods_arr);
                        dao('collect_goods')->where($map)->delete();

                        die(json_encode(['msg' => L('cancel_attention'), 'status' => 0, 'error' => 0]));
                    }

                } else {
                    die(json_encode(['msg' => L('error_information'), 'error' => 1]));
                }
            }
            die(json_encode(['msg' => L('no_login_attention'), 'error' => 2]));
        }
    }
    ...
    public function actionDropGoods()
    {
        if (IS_AJAX) {
            ...
            exit(json_encode(['error' => 1, 'msg' => '删除失败']));
        }
    }
    ...
    public function actionDeleteCart()
    {
        if (IS_AJAX) {
            ...
            if ($rec_id) {
                ...
                if ($result) {
                    exit(json_encode(['error' => 0, 'msg' => '已删除']));
                } else {
                    exit(json_encode(['error' => 1, 'msg' => '删除失败']));
                }
            }
    ...
    {
        if (IS_AJAX) {
            $cart_id = input('cart_id', '', ['trim', 'html_in']); // 当前rec_id 1
            $goods_number = input('number', 0, 'intval');
            $none = input('none', '', ['trim', 'html_in']);
            $rec_id = input('rec_id', '', ['trim', 'html_in']); // 已选rec_id 1,2
            $act_id = input('act_id', '', ['trim', 'html_in']);
            ...
            $sql = "SELECT g.goods_name, wg.region_number as wg_number, wag.region_number as wag_number, g.model_price, g.model_inventory, g.model_attr, g.goods_number, g.group_number, " .
                    ...
                    $leftJoin .
                    "WHERE c.rec_id = '$cart_id' LIMIT 1";
            ...
            //订货数量大于0
            if ($goods_number > 0) {
                if ($row['group_number'] > 0 && $goods_number > $row['group_number'] && !empty($row['group_id'])) {
                    $result['error'] = 1;
                ...
                if ($row['extension_code'] == 'package_buy') {
                    ...
                    $goods_price = get_final_price($row['goods_id'], $goods_number, true, $attr_id, $warehouse_id, $area_id, $area_city,0, 0, $add_tocart); //ecmoban模板堂 --zhuo
                    //更新购物车中的商品数量
                    $set = "";
                    if ($goods_price > 0) {
            ...
            // 优惠活动
            if ($act_id) {
                // $act_sel_id = input('sel_id', '', ['trim', 'addslashes']);
                $sel_flag = input('sel_flag', '', ['trim', 'addslashes']); // 标志flag
                $act_sel = ['act_sel_id' => $cart_id, 'act_sel' => $sel_flag];
    ...
    {
        if (IS_POST) {
            $type = input('type', 0, 'intval'); // 单选 0;多选 1;全选 2
            $rec_id = input('rec_id', '', ['trim', 'addslashes']); // 所有的选中的购物车rec_id
            $cart_id = input('cart_id', '', ['trim', 'addslashes']); // 当前点击（单选）后的购物车的id, 全选时与rec_id 相同
            $act_id = input('act_id', '', ['trim', 'addslashes']); // 当前点击商品相关的活动id
            $status = input('status', 1, 'intval');  // 未选中 0; 选中 1;  默认 1
            ...
            $cart_goods = cart_goods(CART_GENERAL_GOODS, $rec_id, 0, $this->region_id, $this->area_id, $this->area_city); // 取得商品列表
            ...
            if ($type > 0) {
                /* 取得商品列表，计算合计 */
                $cart_goods = get_cart_goods(CART_GENERAL_GOODS, 1, $this->region_id, $this->area_id, $this->area_city);
                // 对同一商家商品按照活动分组
                $merchant_goods_list = cart_by_favourable($cart_goods['goods_list'], $cart_goods['total']['cart_value']);
    ...
    public function actionCartValue()
    {
        if (IS_AJAX) {
            /* 取得购物车选中商品列表，计算合计 */
            $rec_id = input('rec_id', '', ['trim', 'html_in']);
            $act_id = input('act_id', '', ['trim', 'html_in']);
            $result = ['error' => 0, 'cart_number' => 0, 'goods_amount' => 0, 'rec_id' => $rec_id , 'act_id' => $act_id];
     ...
     * @param   string  $select_gift      赠品id
     * @return
     */
    public function actionAddGiftToCart()
    {
        $act_id = input('act_id', 0, 'intval');
        $ru_id = input('ru_id', 0, 'intval'); // 商家ID
        $select_gift = input('select_gift', '', ['trim', 'html_in']);
        ...
        if ($favourable['act_type'] == FAT_GOODS) {
            ...
            /* 检查是否已在购物车 */
            $sql = "SELECT goods_name FROM {pre}cart" .
                " WHERE " . $this->sess_id .
            ...
            if ($success == true) {
                ...
                $result['goods_amount'] = $cart_fav_box['goods_amount'];
                $result['act_id'] = $act_id;
                $result['ru_id'] = $ru_id;
    ...
    public function actionAddToCartCombo()
    {
        if (IS_POST) {
            ...
            $attr = trim($goods->spec); // 当前商品属性
            $goods->spec = !empty($attr) ? explode(',', $attr) : [];

            $parent_attr = trim($goods->parent_attr); // 主件商品属性
            // $parent_attr_id = !empty($parent_attr) ? explode(',', $parent_attr) : [];
            ...
            $group_id = trim($goods->group_id); // group_id
            ...
            $fittings = get_goods_fittings([$parent_id], $warehouse_id, $area_id, $area_city, $goods->group_id, 1);
            if ($fittings) {
            ...
            $result['confirm_type'] = !empty(C('shop.cart_confirm')) ? C('shop.cart_confirm') : 2;
    ...
    public function actionDelInCartCombo()
    {
        if (IS_POST) {
            ...
            //更新临时购物车（删除配件）
            $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE ". $this->sess_id .
                    " AND goods_id = '" . $goods->goods_id . "' AND group_id = '" . $group_id . "'";
            $GLOBALS['db']->query($sql);
    ...
    public function actionAddToCartGroup()
    {
        if (IS_POST) {
            ...
            $group_id = $group_name . "_" . $goods->goods_id; // group_id

            //批量加入购物车
            $sql = "SELECT rec_id FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE " . $this->sess_id .
                    " AND group_id = '" . $group_id . "' ORDER BY parent_id ASC limit 1";
            ...
            if ($res) {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('cart') . "(" .
                        ...
                        "user_id, session_id, goods_id, goods_sn, product_id, group_id, goods_name, market_price, goods_price, goods_number, goods_attr, is_real, " .
                        "extension_code, parent_id, rec_type, is_gift, is_shipping, can_handsel, model_attr, goods_attr_id, warehouse_id, area_id, area_city, add_time" .
                        " FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE " . $this->sess_id . " AND group_id = '" . $group_id . "' ORDER BY parent_id ASC ";
                $GLOBALS['db']->query($sql);
        ...
        if (IS_AJAX) {
            ...
            if ($coupons) {
                foreach ($coupons as $key => $value) {
                    ...
                    $coupons[$key]['enable_ling'] = (!empty($cou_num) && $cou_num >= $value['cou_total']) ? 1 : 0;
                }
