...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_AJAX) {
            ...
            // 主营分类筛选店铺
            if ($cat_id > 0 && empty($store_user)) {
                exit(json_encode(['list' => [], 'totalPage' => 0]));
            }
            // 总数
            $counts = $this->db->getOne($count);
            // 缓存
            $user_store_count = dao('collect_store')->where(['user_id' => $_SESSION['user_id']])->count();
            ...
            if ($store === false) {
                ...
                if ($store_list) {
                    foreach ($store_list as $key) {
                        ...
                        //评分 end
                        //店铺关注人数
                        $gaze = $this->model->table('collect_store')->where(['ru_id' => $key['user_id']])->count('user_id');
                        //取出店铺商品
                        ...
                        if ($goods) {
                            foreach ($goods as $gkey) {
                                $goodsarr[] = get_goods_info($gkey['goods_id'], $this->region_id, $this->area_info['region_id'],$this->area_city);
                            }
                        } else {
                            $goodsarr = 0;
                        }
                        ...
                        $store[] = [
                            'shop_id' => $key['shop_id'],
                            'url' => build_uri('store', ['stid' => $key['user_id']]),
                            'user_id' => $key['user_id'],
                            'shop_name' => get_shop_name($key['user_id'], 1),
                            'self_run' => $key['self_run'],
            ...
            $list = ['list' => $store, 'totalPage' => ceil($counts / $offset)];
        ...
        // 一级地区
        $province = $this->model->table('region')->where(['parent_id' => 1])->select();
    ...
    {
        $id = I('city');
        $city = $this->model->table('region')->where(['parent_id' => $id])->select();
        die(json_encode(['list' => $city, 'html' => 1]));
    }
    ...
    public function actionShopInfo()
    {
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                $list[$key] = get_goods_info($val['goods_id'], $this->region_id, $this->area_info['region_id'],$this->area_city);
            }
        }
        ...
        foreach ($seo as $key => $value) {
            $seo[$key] = html_in(str_replace(['{sitename}', '{key}', '{shopname}', '{description}'], [C('shop.shop_name'), $data['shop_keyword'], $data['shop_title'], $data['street_desc']], $value));
        }
        ...
        $share_data = [
            ...
            'desc' => $description,
            'link' => '',
            'img' => get_wechat_image_path(ltrim($data['logo_thumb'], '../')), // 获得当前店铺logo缩略图
        ];
    ...
    public function actionProList()
    {
        ...
        if (IS_AJAX) {
            ...
            $list = store_get_goods($children, $bid, 0, 0, '', $size, $page, $sort, $order, $ru_id, $this->region_id, $this->area_info['region_id'], $keyword, $type,$this->area_city);

            $maxpage = store_get_goods($children, $bid, 0, 0, '', 0, 0, $sort, $order, $ru_id, $this->region_id, $this->area_info['region_id'], $keyword, $type,$this->area_city);
            ...
            $show = empty($list) ? 0 : 1;
            die(json_encode(['list' => $list, 'totalPage' => ceil($maxpage / $size), 'show' => $show]));
        }
    ...
    protected function shopdata($data = [])
    {
        ...
        $info['count_goods_new'] = $this->sql('store_new = 1 AND user_id=' . $user_id . "   AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0" . $this->review_goods);//所有新品
        $info['count_goods_promote'] = $this->sql('is_promote = 1 AND user_id=' . $user_id . "   AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 AND promote_start_date < '".$time."' AND promote_end_date > '".$time."' " . $this->review_goods);//促销品
    ...
    public function actionShopAbout()
    {
        ...
        //二维码内容
        $url = url('shop_info', ['id' => $ru_id], true, true);
        ...
        // 水印logo
        //        $qrcode_logo = ROOT_PATH . 'public/img/shop_app_icon.png';
