...
class MapController extends FrontendController
{
    ...
    public function actionIndex()
    {
        if (IS_POST) {
            ...
            $store = implode('|', $list);
            if (empty($store)) {
                exit(json_encode(['error' => 1, 'message' => '您的附近暂无商家哦']));
            }
    ...
    public function actionTest()
    {
        ...
        foreach ($seller_shopinfo as $key => $vo) {
            ...
            $result = Http::doGet('http://apis.map.qq.com/ws/geocoder/v1/?key=EVNBZ-UYICR-6DSW4-WSHCK-VEHYH-HWB66&address=' . $address);
