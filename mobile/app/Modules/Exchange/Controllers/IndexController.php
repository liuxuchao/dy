...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_AJAX) {
            $this->sort = (isset($_REQUEST ['sort']) && in_array(trim(strtolower($_REQUEST ['sort'])), [
                    'goods_id',
                    'amount',
                    'popularity',
                    'integral'
            ...
            die(json_encode(['goodslist' => $goodslist, 'totalPage' => ceil($count / $this->size)]));
        }
        // SEO标题
    ...
    public function actionDetail()
    {
        ...
        //客服end
        //商品详情start
        $info = dao('goods')->field('goods_desc')->where(['goods_id' => $this->goods_id])->find();
        if ($info['goods_desc']) {
    ...
    public function actionExprice()
    {
        ...
        $goods_id = (isset($_REQUEST['id'])) ? intval($_REQUEST['id']) : 0; //商品ID
        ...
        $goods = get_goods_info($goods_id, $warehouse_id, $area_id,$this->area_city);
        if ($goods_id == 0) {
        ...
        $goods = get_exchange_goods_info($goods_id, $this->region_id, $this->area_info['region_id'],$this->area_city);
    ...
    public function actionExbuy()
    {
        ...
        $goods = get_exchange_goods_info($good_id, $this->region_id, $this->area_info['region_id'], $this->area_city);
        if ($goods['is_exchange'] == 0) {
            die(json_encode(['msg' => L('eg_error_status')]));
        }
        $user_info = get_user_info($_SESSION['user_id']);
        $user_points = $user_info['pay_points']; // 用户的积分总数
        if ($goods['exchange_integral'] > $user_points) {
            die(json_encode(['msg' => L('eg_error_integral')]));
        }
        /* 查询：取得规格 */
        $specs = '';
        ...
        if (!empty($specs)) {
            ...
            $product_info = get_products_info($good_id, $_specs, $this->region_id, $this->area_info['region_id'], $this->area_city);
        }
        ...
        if ($GLOBALS['_CFG']['use_storage'] == 1) {
            ...
            if (is_spec($_specs) && (!empty($prod))) {
                if (($product_info['product_number'] == 0)) {
                    die(json_encode(['msg' => L('eg_error_number')]));
                }
            } else {
                $is_product = 1;
            }
