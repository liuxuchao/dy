 ...
 * @return array
 */
function category_get_goods($children, $intro = '', $brand, $user_id, $sort = '', $warehouse_id = 0, $area_id = 0, $limit = 10,$area_city = 0)
{
    ...
    if ($intro) {
        switch ($intro) {
            case 'best':
                $where .= ($user_id > 0) ? " AND  g.store_best = '1' " : (($user_id > 0) ? " AND 1 " : " AND is_best = '1' ");
                break;
            case 'new':
                $where .= ($user_id > 0) ? " AND  g.store_new = '1' " : " AND  g.is_new = '1' ";
                break;
            case 'hot':
                $where .= ($user_id > 0) ? " AND  g.store_hot = '1' " : " AND  g.is_hot = '1' ";
                break;
            case 'promotion':
                $time = gmtime();
    ...
    /* 获得商品列表 */
    // 前端与后台
    $discount = isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0 ? $_SESSION['discount'] : 1;
    $user_rank = isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0 ? $_SESSION['user_rank'] : 0 ;
    $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, ' . $shop_price . ' g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot,g.model_attr,' .
        ...
        "WHERE $where  ORDER BY $sort LIMIT $limit";
    ...
    foreach ($res as $row) {
        ...
        $arr[$row['goods_id']]['price'] = ($promote_price > 0) ? price_format($promote_price) : '';
...
function get_thumb_tree($tree_id = 0)
{
    $three_arr = [];
    $where = "";
        $child_sql = 'SELECT album_id, album_name ' .
            ' FROM ' . $GLOBALS['ecs']->table('gallery_album') .
            " WHERE parent_id = '$tree_id' AND is_show = 1 " . $where . " ORDER BY sort_order ASC";
...
function get_article_tree($tree_id = 0){
    ...
    $child_sql = 'SELECT cat_id, cat_name ' .
        ' FROM ' . $GLOBALS['ecs']->table('article_cat') .
        " WHERE parent_id = '$tree_id' ORDER BY sort_order ASC";
    $res = $GLOBALS['db']->getAll($child_sql);
...
function article_tree($tree_id = 0){
    $three_arr = [];
    $res = dao('article_cat')->field('cat_id')->where(['parent_id' => $tree_id])->order('sort_order ASC')->select();
    foreach ($res as $k => $row) {
        $three_arr[$k]['cat_id'] = $row['cat_id'];
        if (isset($row['cat_id'])) {
