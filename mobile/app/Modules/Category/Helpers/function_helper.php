...
{
    $is_show = dao('category')->field('is_show, parent_id')->where(['cat_id' => $cat_id])->find();
 ...
 * @return array
 */
function category_get_goods($keywords = '', $children, $intro = '', $brand, $min, $max, $ext, $size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $ubrand = 0, $hasgoods = 0, $promotion = 0, $cou_id = 0, $area_city)
{
    ...
    // 优惠券商品条件
    if ($cou_id > 0) {
        $cou_data = dao('coupons')->field('cou_type, ru_id, cou_goods, spec_cat, cou_ok_goods, cou_ok_cat')->where(['cou_id' => $cou_id])->find();
        //如果指定了使用的商品
        $where .= get_con_where($cou_data['ru_id'], $cou_data['cou_goods'], $cou_data['spec_cat']);
    }

    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.cat_id, g.cloud_id, g.user_id, g.goods_name, ' . $shop_price . ' g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot,g.model_attr,' .
...
function add_highlight($goods_name, $keyword = '')
{
    if (!empty($keyword)) {
        ...
        $goods_name = str_replace($keyword, $string, $goods_name); //加粗加颜色;
        // $goods_name = preg_replace("/($keyword)/i", "<b style=\"color:red\">\\1</b>", $goods_name);
    }
    return $goods_name;
}
