...
function merge_user($from_user_id = 0, $to_user_id = 0)
{
    if ($from_user_id > 0 && $to_user_id > 0 && $from_user_id != $to_user_id) {
        // users表
        $from_user_info = dao('users')->field('*')->where(['user_id' => $from_user_id])->find();
        if (!empty($from_user_info)) {
            ...
            dao('users')->data($from_data)->where(['user_id' => $to_user_id])->save();
            ...
            // 同步更新老用户的下线用户
            dao('users')->data(['parent_id' => $to_user_id])->where(['parent_id' => $from_user_id])->save();
        }
        // 用户订单
        $from_order_info = dao('order_info')->field('order_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户缺货登记
        $from_booking_goods = dao('booking_goods')->field('rec_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 会员收藏商品
        $from_collect_goods = dao('collect_goods')->field('rec_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 会员留言
        $from_feedback = dao('feedback')->field('msg_id')->where(['user_id' => $from_user_id])->select();
        if (!empty($from_feedback)) {
            $to_user_info = dao('users')->field('user_name,email')->where(['user_id' => $to_user_id])->find();
            foreach ($from_feedback as $key => $value) {
                $setdata = ['user_id' => $to_user_id, 'user_name' => $to_user_info['user_name'], 'email' => $to_user_info['email']];
                dao('feedback')->where('msg_id = ' . $value['msg_id'])->setField($setdata);
            }
        }
        // 会员地址
        $from_user_address = dao('user_address')->field('address_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 会员红包
        $from_user_bonus = dao('user_bonus')->field('bonus_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户帐号金额
        $from_user_account = dao('user_account')->field('id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户标记
        $from_tag = dao('tag')->field('tag_id')->where(['user_id' => $from_user_id])->select();
        if (!empty($from_tag)) {
        ...
        // 用户日志
        $from_account_log = dao('account_log')->field('log_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户原退货订单
        $from_back_order = dao('back_order')->field('back_id')->where(['user_id' => $from_user_id])->select();
        ...
        /*
        多商户新增表
        用户评论:comment,评论图片:comment_img, comment_seller, 收藏关注品牌:collect_brand, 默认卡片:card, 用户白条:baitiao, baitiao_log, 用户退换货:order_return
        默认退货订单表: back_order

        礼品卡:user_gift_gard, 用户银行卡:user_bank, 用户支付密码:users_paypwd, 用户实名认证:users_real,
        关注店铺:collect_store, 用户优惠券:coupons_user, 储值卡: value_card  储值卡中的充值卡表: pay_card,
        订单发票:order_invoice

        关注众筹： zc_focus, 众筹话题: zc_topic
         */

        // 用户评论
        $from_comment = dao('comment')->field('comment_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 评论图片
        $from_comment_img = dao('comment_img')->field('id')->where(['user_id' => $from_user_id])->select();
        ...
        // comment_seller
        $from_comment_seller = dao('comment_seller')->field('sid')->where(['user_id' => $from_user_id])->select();
        ...
        // 收藏品牌
        $from_collect_brand = dao('collect_brand')->field('rec_id')->where(['user_id' => $from_user_id])->select();
        ...
        // card
        $from_card = dao('card')->field('card_id')->where(['user_id' => $from_user_id])->select();
        if (!empty($from_card)) {
        ...
        // 用户银行卡 后续会废弃该表
        $from_user_bank = dao('user_bank')->field('id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户白条
        $from_baitiao = dao('baitiao')->field('baitiao_id')->where(['user_id' => $from_user_id])->select();
        ...
        $from_baitiao_log = dao('baitiao_log')->field('log_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户退换货
        $from_order_return = dao('order_return')->field('ret_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 礼品卡
        $from_user_gift_gard = dao('user_gift_gard')->field('gift_gard_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户支付密码
        $from_users_paypwd = dao('users_paypwd')->field('paypwd_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户实名认证
        $from_users_real = dao('users_real')->field('real_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户关注店铺
        $from_collect_store = dao('collect_store')->field('rec_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户优惠券
        $from_coupons_user = dao('coupons_user')->field('uc_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户储值卡
        $from_value_card = dao('value_card')->field('vid')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户储值卡中的充值卡
        $from_pay_card = dao('pay_card')->field('id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户订单发票
        $from_order_invoice = dao('order_invoice')->field('invoice_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户关注众筹
        $from_zc_focus = dao('zc_focus')->field('rec_id')->where(['user_id' => $from_user_id])->select();
        ...
        // 用户发表的众筹话题
        $from_zc_topic = dao('zc_topic')->field('topic_id')->where(['user_id' => $from_user_id])->select();
