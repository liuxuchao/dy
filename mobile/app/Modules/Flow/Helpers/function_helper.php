...
function get_cart_value($flow_type = 0, $store_id = 0)
{
    ...
    $sql = "SELECT c.rec_id FROM " . $GLOBALS['ecs']->table('cart') .
        ...
        " AND c.is_checked = 1 AND c.is_invalid = 0 AND c.rec_type = '$flow_type' order by c.rec_id asc";
...
function flow_cart_stock($arr, $store_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    foreach ($arr as $key => $val) {
        ...
        if ($store_id > 0) {
            ...
            $sql = "SELECT IF(g.model_inventory < 1, g.goods_number, IF(g.model_inventory < 2, wg.region_number, wag.region_number)) AS goods_number  FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
                $leftJoin .
