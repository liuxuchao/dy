...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if ((!isset($use_integral) || $use_integral == '1') && $_SESSION['user_id'] > 0 && $user_info['pay_points'] > 0 && ($flow_type == CART_GENERAL_GOODS || $flow_type == CART_ONESTEP_GOODS)) {
            // 能使用积分
        ...
        if (!empty($users_paypwd)) {
            if ($allow_use_surplus == 1 && $users_paypwd['user_surplus'] == 1 && $user_info['user_money'] > 0) {
                // 可使用余额，且用户有余额
                $allow_users_paypwd = 1;
            }
            if ($allow_use_value_card == 1) {
                $allow_users_paypwd = 1;
            }
        }
        ...
        $is_user_invoices_id = dao('users_vat_invoices_info')->where(['user_id' => $_SESSION[user_id]])->find();
    ...
    public function actionDone()
    {
        ...
        $pay_type = input('post.pay_type', 0, 'intval'); //取得支付类型
        ...
        if ($done_cart_value && $GLOBALS['_CFG']['stock_dec_time'] == SDT_PLACE) {
            ...
            if ($solve_info) {
                if ($solve_info['solve_num'] > 1) {
                    ...
                    $sql = "SELECT rec_id, goods_number, user_id, goods_id, CONCAT(goods_id, '|', model_attr, '|', goods_attr_id) AS cart_info, CONCAT(goods_id, '|', model_attr, '|', goods_attr_id, '|', warehouse_id, '|', area_id) AS warehouse_area FROM {pre}cart WHERE rec_id " . db_create_in($solve_info['orec_id']) . " AND is_real = 1" .
        ...
        $msg = input('post.postscript', '', ['htmlspecialchars','trim']);
        $ru_id_arr = input('post.ru_id', '', ['htmlspecialchars','trim']); // 商家id数组
        $shipping_arr = input('post.shipping', '', ['htmlspecialchars','trim']); // 配送方式数组
        $shipping_type = input('post.shipping_type', '', ['htmlspecialchars','trim']);
        $shipping_code = input('post.shipping_code', '', ['htmlspecialchars','trim']);

        $lang_oos = L('oos');
        $how_oos = $lang_oos[$_POST['how_oos']];

        $point_id = I('post.point_id');
        $shipping_dateStr = input('post.shipping_dateStr', '', ['htmlspecialchars','trim']);
        ...
        $order = [
            ...
            'is_surplus' => isset($_POST['is_surplus']) ? intval($_POST['is_surplus']) : 0,
            ...
            'how_oos' => isset($how_oos) ? addslashes($how_oos) : '',
        ...
        if ($user_id > 0) {
            ...
            // 查询用户有多少积分
            $flow_points = flow_available_points($_SESSION['cart_value'], $flow_type, $this->region_id, $this->area_id, $this->area_city);  // 该订单允许使用的积分
        ...
        if ($order['bonus_id'] > 0) {
            ...
            if (empty($bonus) || $bonus['user_id'] != $user_id || $bonus['order_id'] > 0 || $bonus['min_goods_amount'] > cart_amount(true, $flow_type, $_SESSION['cart_value'])) {
                $order['bonus_id'] = 0;
            }
            ...
            $now = gmtime();
            if (empty($bonus) || $bonus['user_id'] > 0 || $bonus['order_id'] > 0 || $bonus['min_goods_amount'] > cart_amount(true, $flow_type, $_SESSION['cart_value']) || $now > $bonus['use_end_date']) {
            } else {
                if ($user_id > 0) {
        ...
        if ($order['uc_id'] > 0) {
            ...
            if (empty($coupons) || $coupons['user_id'] != $user_id || $coupons['is_use'] == 1 || $coupons['cou_man'] > cart_amount(true, $flow_type, $_SESSION['cart_value'])) {
                $order['uc_id'] = 0;
            }
        }

        $cart_goods_list = cart_goods($flow_type, $_SESSION['cart_value'], 1, $this->region_id, $this->area_id, $this->area_city, $consignee, $store_id);// 取得商品列表，计算合计
        if (empty($cart_goods_list)) {
        ...
        // 验证支付密码
        $pay_paypwd = input('pay_paypwd', '', ['htmlspecialchars','trim']);
        $users_paypwd = dao('users_paypwd')->field('pay_online, pay_password, ec_salt, user_surplus')->where(['user_id' => $_SESSION['user_id']])->find();
        // 启用并使用余额
        if ($users_paypwd && $order['is_surplus'] == 1) {
            if (empty($pay_paypwd)) {
                show_message("请输入支付密码！", '', '', 'warning');
            } else {
                $new_password = md5(md5($pay_paypwd) . $users_paypwd['ec_salt']);
                if ($users_paypwd['user_surplus'] == 1){
        ...
        $order['use_value_card'] = $total['use_value_card'] ? $total['use_value_card'] : 0; //储值卡使用金额
        ...
        /* 如果全部使用余额支付，检查余额是否足够 */
        if ($order['is_surplus'] == 1 && $order['order_amount'] > 0) {
        ...
        if ($flow_type == CART_TEAM_GOODS) {
            if ($_SESSION['team_id'] > 0) {
                $team_info = $this->db->table('team_log')->where(['team_id' => $_SESSION['team_id']])->find();
        ...
        do {
            ...
            //$this->db->autoExecute($GLOBALS['ecs']->table('order_info'), $order, 'INSERT');
        ...
        if (is_dir(APP_DRP_PATH) && $order['extension_code'] != 'bargain_buy' && $order['extension_code'] != 'team_buy') {
            ...
            $sql = "INSERT INTO " . $this->ecs->table('order_goods') . "( " .
                "user_id, order_id, goods_id, goods_name, goods_sn, product_id,is_reality,is_return,is_fast, goods_number, market_price, goods_price, commission_rate, " .
                ...
                " WHERE ca." . $this->sess_id . " AND ca.is_checked = 1 AND ca.rec_type = '$flow_type'" . $goodsIn;
            $this->db->query($sql);
            //微分销end
            ...
            /* 插入订单商品 */
            $sql = "INSERT INTO " . $this->ecs->table('order_goods') . "( " .
                "user_id, order_id, goods_id, goods_name, goods_sn, product_id,is_reality,is_return,is_fast, goods_number, market_price, commission_rate, " .
                ...
                " SELECT '$user_id', '$new_order_id', ca.goods_id, ca.goods_name, ca.goods_sn, ca.product_id,(SELECT is_reality FROM ".$GLOBALS['ecs']->table('goods_extend')." AS ge WHERE ge.goods_id = ca.goods_id),(SELECT is_return FROM ".$GLOBALS['ecs']->table('goods_extend')." AS ge WHERE ge.goods_id = ca.goods_id),(SELECT is_fast FROM ".$GLOBALS['ecs']->table('goods_extend')." AS ge WHERE ge.goods_id = ca.goods_id), ca.goods_number, ca.market_price, ca.commission_rate, " .
                ...
                " FROM " . $this->ecs->table('cart') ." AS ca".
                " WHERE " . $this->sess_id . " AND ca.rec_type = '$flow_type'" . $goodsIn;
        ...
        if ($new_order_id > 0 && $store_id > 0 && $ru_id_arr) {
            foreach ($ru_id_arr as $v) {
                ...
                $take_time = input('take_time', '', ['htmlspecialchars','trim']);
                $store_order = [
        ...
        if ($order['extension_code'] == 'auction') {
            ...
            $sql = "SELECT ext_info FROM {pre}goods_activity WHERE act_id = '" . $order['extension_id']."' LIMIT 1";//获取拍卖活动保证金
        ...
        if ($new_order_id > 0 && C('shop.use_storage') == '1' && C('shop.stock_dec_time') == SDT_PAID && $order['order_amount'] <= 0) {
            ...
            if ($virtual_goods && $flow_type != CART_GROUP_BUY_GOODS) {
                ...
                if (virtual_goods_ship($virtual_goods, $error_msg, $order['order_sn'], true)) {
                    ...
                    if ($this->db->getOne($sql) <= 0) {
                        ...
                        update_order($order['order_id'], ['order_status' => OS_CONFIRMED, 'shipping_status' => SS_SHIPPED, 'shipping_time' => gmtime()]);
        ...
        // 微信通模板消息 订单通知
        if (is_dir(APP_WECHAT_PATH)) {
            // $new_order_amount = ($order['is_surplus'] == 1) ? $order['surplus'] : $order['order_amount'];
            $pushData = [
                'orderID' => ['value' => $order['order_sn'], 'color' => '#173177'], //订单号
                'orderMoneySum' => ['value' => $order['order_amount'], 'color' => '#ec5151'], //订单金额
        ...
        if ($order['is_surplus'] == 1 && $order['order_amount'] <= 0 && $order['surplus'] > 0) {
            /* 记录主订单操作记录 */
            order_action($order['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, PS_PAYED, L('flow_surplus_pay'), L('buyer'));

            if ($child_order > 1 && $child_order_info) {
                /* 记录子订单操作记录 */
            ...
            if (is_dir(APP_WECHAT_PATH)) {
                ...
                $pushData = [
                    ...
                    'keyword4' => ['value' => $users['user_money'], 'color' => '#ec5151'], // 当前余额
        ...
        if ($order['order_amount'] > 0 && $payment['pay_code'] == 'onlinepay' && $payment['pay_code'] !='cod') {
    ...
    {
        $is_integral = input('points', 0 ,'intval'); // 是否使用积分 0 否 1 是
        ...
        if (empty($cart_goods_list) || !check_consignee_info($consignee, $flow_type)) {
            ...
            if ($is_integral > 0) {
                ...
                if ($user_points == 0 || $user_points < $flow_points) {
                    $integral = 0;
                    $result['error'] = L('integral_not_enough'); // 积分不足以抵扣
                } else {
    ...
    {
        $result = ['error' => 0, 'err_msg' => '', 'point_id' => 0, 'shipping_dateStr' => ''];
    ...
    {
        $sql = "UPDATE {pre}coupons_user SET order_id = '$order_id', is_use_time = '" . gmtime() . "', is_use =1 " .
            "WHERE uc_id = '$cou_id'";
    ...
    public function use_value_card($vc_id, $order_id, $use_val)
    {
        ...
        $sql = " UPDATE {pre}value_card SET card_money = '$card_money' " .
            " WHERE vid = '$vc_id' ";

        if (!$this->db->query($sql)) {
            return false;
        }
        $sql = "INSERT INTO {pre}value_card_record (vc_id, order_id, use_val, record_time) " .
