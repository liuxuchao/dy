...
class LoginController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_POST) {
            ...
            $back_act = input('back_act', '', ['htmlspecialchars','urldecode']);
    ...
    public function actionRegister()
    {
        ...
        // 注册协议文章id
        $register_article_id = !empty(C('shop.register_article_id')) ? intval(C('shop.register_article_id')) : 6; // 6 默认注册协议文章

        $this->assign('flag', 'register');
        $this->assign('back_act', $back_act);
        $this->assign('register_article_id', $register_article_id);
        $this->assign('sms_signin', C('shop.sms_signin'));
    ...
    public function actionArticleDetail()
    {
        if (IS_AJAX) {
            ...
            $result = [];
            if ($article_id) {
                $result = dao('article')->field('title, content')->where(['article_id' => $article_id])->find();
            }
            exit(json_encode(['status' => 0, 'data' => $result]));
        }
    }
