...
class CrowdController extends FrontendController
{
    ...
    public function actionOrder()
    {
        ...
        if (IS_AJAX) {
            ...
            exit(json_encode(['list' => $order_list['list'], 'totalPage' => $order_list['totalpage']]));
        }
    ...
    public function actionDetail()
    {
        ...
        if ($order['order_status'] == OS_UNCONFIRMED) {
            $order['handler'] = "<span class=\"box-flex text-right\"></span><a class=\"btn-default box-flex\" type=\"button\" href=\"" . url('user/crowd/cancel', ['order_id' => $order['order_id']]) . "\" onclick=\"if (!confirm('" . L('confirm_cancel') . "')) return false;\">" . L('cancel') . "</a>";
        } elseif ($order['order_status'] == OS_SPLITED) {

            /* 对配送状态的处理 */
            if ($order['shipping_status'] == SS_SHIPPED) {
                @$order['handler'] = "<a class=\"btn-submit\" href=\"" . url('user/crowd/affirmreceived', ['order_id' => $order['order_id']]) . "\" onclick=\"if (!confirm('" . L('confirm_received') . "')) return false;\">" . L('received') . "</a>";
    ...
    public function actionCrowdbuy()
    {
        ...
        if (IS_AJAX) {
            ...
            exit(json_encode(['list' => $crowd_buy['list'], 'totalPage' => $crowd_buy['totalpage']]));
        }
