...
class RefoundController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_AJAX) {
            $order_list = get_all_return_order($order_id, $size, $page);  // 售后申请订单列表
            exit(json_encode(['order_list' => $order_list['list'], 'totalPage' => $order_list['totalpage']]));
        }

        $this->assign('order_id', $order_id);
        $this->assign('page_title', L('return_index'));
        $this->display();
    }
    ...
    public function actionList()
    {
        ...
        if (IS_AJAX) {
            $refound_list = return_order($order_id, $size, $page);  // 退货进度查询列表
            exit(json_encode(['refound_list' => $refound_list['list'], 'totalPage' => $refound_list['totalpage']]));
        }

        $this->assign('order_id', $order_id);
        $this->assign('page_title', L('return_progres'));
        $this->display();
    }
    ...
    public function actionApplyReturn()
    {
        ...
        if (empty($return_rec_id)) {
            show_message(L('return_exist'), '', '', 'error', true);
        }
        // 判断退换货是否已申请
        $is_refound = get_is_refound($return_rec_id);
        if ($is_refound == 1) {
            show_message(L('return_is_apply'), '', '', 'warning', true);
        }
        /* 退货权限：订单状态 已发货、未退货 */
        $order_id = I('order_id', 0, 'intval');
        $order = dao('order_info')->field('shipping_status, order_status, chargeoff_status, is_settlement')->where(['order_id' => $order_id])->find();
    ...
    public function actionSubmitReturn()
    {
        ...
        if (empty($rec_id)) {
            show_message(L('Apply_Abnormal'), '', '', 'error', true);
        }

        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_return') . " WHERE rec_id = " . $rec_id;
        $num = $GLOBALS['db']->getOne($sql);
        if ($num > 0) {
            show_message(L('Repeated_Submission'), '', '', 'error', true);
        }

        if (empty($last_option)) {
            show_message(L('cause_id_empty'), '', '', 'error', true);
        }
        ...
        $sql = "SELECT g.goods_name, g.goods_sn,g.brand_id, og.order_id, og.goods_id, og.product_id, og.goods_attr, og.warehouse_id, og.area_id, o.order_sn, " .
            ...
            " , o.consignee, o.mobile, o.country, o.province, o.city , o.district " .
        ...
        if (!empty($order_cloud)) {
            ...
            if ($return_type == 0 || $return_type == 2) {
                show_message(L('return_error'), '', '', 'error', true);
            }
            $isRefund = 1;
            if ($return_type == 3) {
                $isRefund = 2;
            ...
            if ($requ) {
                ...
                show_message(L('process_false'), '', '', 'error', true);
            }
        }

        $attr_val = isset($_REQUEST['attr_val']) ? $_REQUEST['attr_val'] : []; //获取属性ID数组
        ...
        $order_return = [
            ...
            'addressee' => empty($_REQUEST['addressee']) ? $order_goods['consignee'] : htmlspecialchars(trim($_REQUEST['addressee'])),
            'phone' => empty($_REQUEST['mobile']) ? $order_goods['mobile'] : htmlspecialchars(trim($_REQUEST['mobile'])),
        ...
        if ($query) {
            $return = dao('order_return')->field('ret_id, refound_status')->where(['return_sn' => $order_return['return_sn']])->find();
             ...
             */
            $new_order = dao('order_info')->field('pay_id, pay_status, money_paid')->where(['order_id' => $order_goods['order_id']])->find();
            ...
            show_message(L('Apply_Success_Prompt'), L('See_returnlist'), url('detail', ['ret_id' => $return['ret_id']]), 'success', true);
    ...
    public function actionImgReturn()
    {
        ...
        if ($res >= 5) {
            echo json_encode(['error' => 1, 'content' => '图片不能超过5张']);
            return;
        }
        //判断文件类型
        if (empty($type)) {
        ...
        //上传图片并 获得路径
        $result = $this->upload('data/return_images', false, 20, [C('shop.thumb_width'), C('shop.thumb_height')]);
    ...
    public function actionDetail()
    {
        ...
        if (!empty($order['out_invoice_no'])) {
            $shipping_code = dao('shipping')->where(['shipping_id' => $order['out_shipping_name']])->getField('shipping_code');
        ...
        if (!empty($order['back_invoice_no'])) {
            $shipping_code = dao('shipping')->where(['shipping_id' => $order['back_shipping_name']])->getField('shipping_code');
            $plugin = ADDONS_PATH . 'shipping/' . $shipping_code . '.php';
            if (file_exists($plugin)) {
                include_once($plugin);
                $shipping = new $shipping_code;
        ...
        //     $requ = $cloud->getAfterSalesAddress($store_addres);
        //     $requ = json_decode($requ, true);
        //     if ($requ['code'] = 10000) {
        //         $order = $requ['data'];
        //     }
         ...
         * 如果成功，查询官方接口退款状态，更新网站订单退款状态
         */
        $new_order = dao('order_info')->field('pay_id, pay_status, money_paid')->where(['order_id' => $order['order_id']])->find();
    ...
    {
        if (IS_AJAX) {
            $result = ['error' => 0, 'message' => '', 'attr_val' => ''];
                        ...
                        if ($value['values']) {
                            foreach ($value['values'] as $k => $v) {
                                ...
                                if ($arr_class['class'] == 'cattsel') {
                                    $v['checked'] = 1; // 选中状态
                                    $spec[$key]['attr_val'] = $arr_class['attr_val']; // 默认属性赋值
                                }
                                $spec[$key]['values'][$k] = $v;
                            }
                        }
    ...
    public function actionEditExpress()
    {
        if (IS_POST) {
            ...
            if (empty($ret_id)) {
                $json_res = ['error' => 1, 'msg' => L('return_exist')];
                exit(json_encode($json_res));
            }
            if (empty($back_shipping_name)) {
                $json_res = ['error' => 1, 'msg' => L('shipping_name_empty')];
                exit(json_encode($json_res));
            }
            if (empty($back_invoice_no)) {
                $json_res = ['error' => 1, 'msg' => L('invoice_no_empty')];
                exit(json_encode($json_res));
            }
            ...
            if ($ret_id) {
                ...
                dao('order_return')->data($data)->where(['ret_id' => $ret_id])->save();
                $json_res = ['error' => 0, 'msg' => L('edit_shipping_success')];
                exit(json_encode($json_res));
            }
        }
    }
    ...
    public function actionActivationReturnOrder()
    {
        if (IS_POST) {
            ...
            $activation_number = dao('order_return')->where(['ret_id' => $ret_id])->getField('activation_number');

            if ($activation_number_type > $activation_number) {
                $Order = dao('order_return');
                $Order->return_status = 0;
                $Order->activation_number = ['exp', 'activation_number+1'];
                $Order->where(['ret_id' => $ret_id])->save();
    ...
    public function actionDeleteReturnOrder()
    {
        if (IS_POST) {
            ...
            if ($ret_id > 0) {
                ...
                dao('order_return')->where(['ret_id' => $ret_id, 'user_id' => $user_id])->delete();
