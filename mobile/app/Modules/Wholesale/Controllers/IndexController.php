...
class IndexController extends FrontendController
{
    ...
    public function actionWholeList()
    {
        ...
        $search_keywords = I('search_keywords', '', ['htmlspecialchars','trim']);
        $sort = I('sort', '', ['htmlspecialchars','trim']);
        $sort = ($sort == 'ASC') ? 'ASC' : 'DESC';
        $order = I('order', '', ['htmlspecialchars','trim']);
        $param = []; // 翻页链接所带参数列表
    ...
    public function actionAddToCart()
    {
        ...
        if (isset($_SESSION['wholesale_goods'])) {
            foreach ($_SESSION['wholesale_goods'] as $goods) {
                if ($goods['goods_id'] == $wholesale['goods_id']) {
                    if (empty($goods_attr)) {
                        $this->ajaxReturn(['msg' => L('goods_have_in_cart')]);
        ...
        foreach ($wholeattr as $klist => $list) {
            ...
            $goods_list[] = ['number' => $number[$klist], 'goods_attr' => $goods_attr];
        }
        ...
        foreach ($goods_list as $goods_key => $goods) {
            if ($goods['number'] < $goods['qp_list'][0]['quantity']) {
                $this->ajaxReturn(['msg' => L('dont_match_min_num')]);
            } elseif (strlen(intval($goods['number'])) > 6) {
                $this->ajaxReturn(['msg' => L('number_is_to_large')]);
            } else {
                $goods_price = 0;
        ...
        foreach ($goods_list as $goods_key => $goods) {
            ...
            $_SESSION['wholesale_goods'][] = [
                ...
                'goods_thumb' => get_image_path($goods['goods_thumb']),
                'subtotal' => $total,
                'formated_goods_price' => price_format($goods['goods_price'], false),
                'formated_subtotal' => price_format($total, false),
                'goods_url' => url('goods/index/index', ['id' => $wholesale['goods_id']]),
            ];
        }
        $this->ajaxReturn(['error' => '0', 'msg' => L('success_add_to_cart')]);
    }
