<?php

/**
 * ECTouch Open Source Project
 * ============================================================================
 * Copyright (c) 2012-2014 http://ectouch.cn All rights reserved.
 * ----------------------------------------------------------------------------
 * 文件名称：coupont.php
 * ----------------------------------------------------------------------------
 * 功能描述：优惠券语言
 * ----------------------------------------------------------------------------
 * Licensed ( http://www.ectouch.cn/docs/license.txt )
 * ----------------------------------------------------------------------------
 */

$_LANG['Coupon_redemption_task'] = "领券中心-任务集市";
$_LANG['Coupon_redemption_failure'] = "领取失败,券已经被领完了";
$_LANG['Coupon_redemption_limit'] = "领取失败,您已经领取过该券了!每人限领取 %s 张";
$_LANG['Coupon_redemption_failure_two'] = '领取失败,仅限会员等级为: %s 领取'; //wang
$_LANG['receive_success'] = "领取成功！感谢您的参与，祝您购物愉快~";
$_LANG['use_limit'] = "限 %s 可用";
$_LANG['platform_use'] = "全平台可用";
$_LANG['Top_up_coupons'] = "满额返现";
$_LANG['day'] = '天';
$_LANG['hour'] = '小时';
$_LANG['hour_two'] = '时';
$_LANG['minute'] = '分';
$_LANG['seconds'] = '秒';
$_LANG['remaining'] = '剩余';
$_LANG['Coupon_kill'] = '优惠券秒杀';
$_LANG['pic_kill_goods'] = '秒杀商品图';
$_LANG['page_title_Coupon'] = "首页-领券中心";
$_LANG['consumption_full'] = "消费满"; 
$_LANG['available_full'] = "元可用";
$_LANG['Activities_end'] = "活动已抢完";
$_LANG['receive_now'] = "立即领取";
$_LANG['redemption_task'] = "任务集市";
$_LANG['coupons_prompt'] = "买好货送实惠";
$_LANG['always_you'] = "总有你想要的";
$_LANG['full'] = "满"; 
$_LANG['permissions_buy'] = "仅可购买指定商品";
$_LANG['category'] = '全品类';
$_LANG['Take_up'] = "已抢完";
$_LANG['Immediate_use'] = "立即使用";
$_LANG['receive_hove'] = "已领取";
$_LANG['default'] = "默认";
$_LANG['Will_expire'] = "即将过期";
$_LANG['max_Face'] = "最大面值";
$_LANG['type_all'] = '所有类型';
$_LANG['vouchers_all'] = "全场券";
$_LANG['vouchers_user'] = "会员券";
$_LANG['vouchers_shoping'] = "购物券";
$_LANG['vouchers_login'] = "注册券";
$_LANG['unknown'] = "未知";
$_LANG['vouchers_shipping'] = "免邮券";
$_LANG['unlimited'] = '不限';
$_LANG['js_languages']['receive_coupons'] = '领取优惠券';
$_LANG['js_languages']['Immediate_use'] = "立即使用";



return $_LANG;
