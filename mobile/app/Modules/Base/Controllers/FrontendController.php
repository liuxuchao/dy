...
abstract class FrontendController extends FoundationController
{
    ...
    protected function init_oauth()
    {
        ...
        if (is_wechat_browser() && empty($_SESSION['unionid']) && $module_name != 'oauth') {
            ...
            if ($auth_config) {
                ...
                $this->redirect('oauth/index/index', ['type' => 'wechat', 'back_url' => urlencode(html_in($back_url))]);
            }
        }
    }
    ...
    public function get_wechat_share_content($share_data = [])
    {
        if (!empty($share_data['img'])) {
            $share_img = (strtolower(substr($share_data['img'], 0, 4)) == 'http') ? $share_data['img'] : __STATIC__ . $share_data['img'];
        } else {
            $share_img = !empty(C('shop.wap_logo')) ? __STATIC__ . '/'. ltrim(C('shop.wap_logo'), '../') : elixir('img/wxsdk.png', true);
        }
