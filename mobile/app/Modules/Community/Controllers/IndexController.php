...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        $tao = ['num' => community_num(1), 'has_new' => community_has_new(1)];//讨论贴
        $wen = ['num' => community_num(2), 'has_new' => community_has_new(2)];//问答帖
        $quan = ['num' => community_num(3), 'has_new' => community_has_new(3)];//圈子帖
        $sun = ['num' => sd_count(), 'has_new' => community_has_new(4, 1)];//晒单贴
        $this->assign('tao', $tao);
        $this->assign('wen', $wen);
        ...
        if (!empty($dis_type) && !empty($dis_id)) {
            ...
            if ($res) {
                ...
                if (!empty($link_good)) {
                    ...
                    $link_good['url'] = url('goods/index/index', ['id' => $link_good['goods_id']]);
                }
    ...
    public function actionCommentList()
    {
        if (IS_AJAX) {
            ...
            $discuss = dao('discuss_circle')->where(['dis_id' => $id, 'parent_id' => 0])->find();
            ...
            if ($dis_type == 4) {
                ...
                foreach ($dis_comment as $k => $v) {
                    ...
                    $dis_comment[$k]['quote'] = $id > 0 ? get_comment_reply($v['dis_id'], $id, $goods_id) : [];
                ...
                foreach ($dis_comment as $k => $v) {
                    ...
                    $dis_comment[$k]['user_picture'] = get_image_path($usersnick['user_picture'], '', elixir('img/user_default.png'));
                    $dis_comment[$k]['quote'] = $dis_id > 0 ? get_quote_reply($v['dis_id'], $dis_id) : [];
                    $dis_comment[$k]['quote_count'] = count($dis_comment[$k]['quote']);
            ...
            exit(json_encode(['commentlist' => $dis_comment, 'totalPage' => ceil($total / $this->size)]));
        }
    }
    ...
    public function actionCommnet()
    {
        if (IS_POST) {
            ...
            $dis_text = I('dis_text', '', ['htmlspecialchars','trim']);
            $user_id = I('user_id', 0, 'intval');
            $goods_id = I('goods_id', 0, 'intval');

            $reply_type = I('reply_type', '', ['htmlspecialchars','trim']); // 回复类型 回复他人
            if (empty($_SESSION['user_id'])) {
                $back_act = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : __HOST__ . $_SERVER['REQUEST_URI'];
                $url = url('user/login/index', ['back_act' => urlencode($back_act)]);
                exit(json_encode(['error' => 1, 'msg' => '请登录', 'url' => $url]));
            }
            if (checkDistype($dis_type) == false) {
                exit(json_encode(['error' => 1, 'msg' => '发帖类型不允许！']));
            }
            if (empty($dis_text)) {
                exit(json_encode(['error' => 1, 'msg' => L('write_answer')]));
            }
            // 回复晒单
            if ($dis_type == 4) {
                // 回复其他
                if ($reply_type == 'reply_other') {
                    ...
                    if ($user_id == $_SESSION['user_id']) {
                        exit(json_encode(['error' => 1, 'msg' => '不能回复自己的帖子']));
                    }
                }
                $data = [
                    'comment_type' => 2,
                    ...
                    'status' => 1,
                    'user_id' => $_SESSION['user_id'],
                    'user_name' => $_SESSION['nickname'] ? $_SESSION['nickname'] : $_SESSION['user_name'],
                    'add_time' => gmtime()
                ];
            ...
            if ($res) {
                $json_res = ['error' => 0, 'msg' => '发帖成功！', 'url' => url('community/index/detail', ['id' => $parent_id, 'type' => $dis_type])];
    ...
    public function actionReply()
    {
        ...
        if (IS_AJAX) {
            ...
            foreach ($list as $k => $v) {
                ...
                $list[$k]['url'] = url('detail', ['id' => $v['parent_id'], 'type' => $v['dis_type']]);
            }
            exit(json_encode(['list' => $list, 'totalPage' => ceil($total / $this->size)]));
        }
        ...
        if (IS_AJAX) {
            ...
            if (!empty($dis_type) && !empty($dis_id)) {
                ...
                $like_num = $this->db->getOne(
                    "SELECT like_num FROM {pre}$table  WHERE $field =  $dis_id  "
                );
                if ($like_num === null) {
                    $like_num = '0';
                }
                ...
                echo json_encode(['like_num' => $like_num, 'is_like' => $islike, 'dis_id' => $dis_id]);
            }
        }
    }
