...
function zc_goods_info($goods_id = 0)
{
    ...
    //项目状态
    $time = gmtime();
    if ($time > $zhongchou['end_time'] && $zhongchou['amount'] >= $zhongchou['join_money']) {
        $zhongchou['zc_status'] = '项目失败';
        $zhongchou['result'] = 2;
        $zhongchou['shenyu_time'] = 0;
    }
    if ($time > $zhongchou['end_time'] && $zhongchou['join_money'] >= $zhongchou['amount']) {
...
{
    //支持者列表
    $sql = " SELECT oi.user_id,oi.add_time,zg.price,zg.content " .
...
function zc_comment_list($id, $size, $page)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $arr[$row['topic_id']]['url'] = U('crowd_funding/index/topic', ['id' => $row['pid'], 'topic_id' => $row['topic_id'], 'action' => 'comment']);
...
function zc_cart_goods($pid, $id, $number)
{
    ...
    $zhongchou['formated_subtotal'] = price_format(($zhongchou['price'] * $number), false); //商品总价
