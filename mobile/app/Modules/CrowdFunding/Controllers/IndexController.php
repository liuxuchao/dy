...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        if (IS_AJAX) {
            ...
            $goodslist = $this->zc_goods($page);//获取众筹商品列表
    ...
    private function zc_goods($page)
    {
        ...
        foreach ($res as $k => $z_val) {
            ...
            $res[$k]['url'] = url('info', ['id' => $z_val['id']]);
        ...
        return ['list' => array_values($res), 'totalpage' => ceil($total / $this->size)];
    }
    ...
    public function actionInfo()
    {
        ...
        $this->assign('description', $goods['title']);    // 商品简单描述

        // 微信JSSDK分享
        $share_data = [
            'title' => $goods['title'],
            'desc' => msubstr_ect(strip_tags($goods['describe']), 0, 30, 'utf-8', '', 0),
            'link' => '',
            'img' => $goods['title_img'],
        ];
    ...
    public function actionProperties()
    {
        ...
        $this->assign('details', $row['details']);//项目主页
        $this->assign('row', $row);//风险说明
    ...
    public function actionComment()
    {
        if (IS_AJAX) {
            ...
            exit(json_encode(['list' => $comment_list['list'], 'totalPage' => $comment_list['totalpage']]));
        }
    ...
    public function actionTopic()
    {
        if (IS_POST) {
            ...
            if ($action == 'info') {
                show_message('发布话题成功', '返回上一页', url('crowd_funding/index/info', ['id' => $data['pid']]), 'success');
            } else {
                show_message('发布话题成功', '返回上一页', url('crowd_funding/index/comment', ['id' => $data['pid']]), 'success');
            }
        }
    ...
    public function actionCheckout()
    {
        ...
        */
        ...
        if ($shengyu == 0) {
            show_message('该产品已售罄，请选择其他产品', '', url('crowd_funding/index/info', ["id" => $_SESSION['pid']]), 'warning');
        }
        /* 取得配送列表 */
        $region = [
            $consignee ['country'],
    ...
    public function actionDone()
    {
        ...
        /*判断重复商品订单 是否支付 */
        $sql = " SELECT COUNT(order_id) FROM " . $this->ecs->table('order_info') . " WHERE user_id = '$_SESSION[user_id]' AND is_zc_order = 1 AND zc_goods_id = '$_SESSION[id]' AND is_delete = 0 AND pay_status = 0 AND order_status !=2 ";    
        ...
        $postscript = I('post.postscript', '',  ['htmlspecialchars','trim']);
        ...
        /* 配送方式  */
        $order_shipping = dao('seller_shopinfo')->field("shipping_id")->where(["ru_id" => 0])->find();
    ...
    public function actionSetAddress()
    {
        if (IS_AJAX) {
            ...
            echo json_encode(['url' => url('checkout'), 'status' => 1]);
        }
    }
