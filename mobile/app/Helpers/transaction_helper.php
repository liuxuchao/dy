...
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0)
{
    ...
    if (is_dir(APP_TEAM_PATH)) {
        $sql = "SELECT og.ru_id, oi.main_order_id, oi.consignee,oi.pay_name, oi.order_id, oi.order_sn,oi.pay_time,oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time,oi.team_id,oi.extension_code, " .
            $select .
            "(oi.goods_amount + oi.shipping_fee + oi.insure_fee + oi.pay_fee + oi.pack_fee + oi.card_fee + oi.tax - oi.discount - oi.coupons) AS total_fee, og.goods_id, " .
        ...
        $sql = "SELECT og.ru_id, oi.main_order_id, oi.consignee,oi.pay_name, oi.order_id, oi.order_sn,oi.pay_time,oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time,oi.extension_code, " .
            $select .
            "(oi.goods_amount + oi.shipping_fee + oi.insure_fee + oi.pay_fee + oi.pack_fee + oi.card_fee + oi.tax - oi.discount - oi.coupons) AS total_fee, og.goods_id, " .
            "oi.invoice_no, oi.shipping_name, oi.tel, oi.email, oi.address, oi.province, oi.city, oi.district " .
            " FROM " . $GLOBALS['ecs']->table('order_info') . " as oi" .
    ...
    foreach ($res as $key => $row) {
        ...
        $date = ['order_status', 'shipping_status', 'pay_status', 'shipping_time', 'auto_delivery_time'];
        ...
        if ($row['order_status'] == OS_UNCONFIRMED) {
            ...
            $row['online_pay'] =url('onlinepay/index/index',['order_sn'=>$row[order_sn]]);
            ...
            if ($row['shipping_status'] == SS_SHIPPED) {
                ...
                if ($row['pay_status'] == PS_UNPAYED) {
                    @$row['handler'] = "<a class=\"btn-default-new br-5\" href=\"" . url('user/order/detail', ['order_id' => $row['order_id']]) . '" >' . L('pay_money') . '</a>';
                } else {
                    @$row['handler'] = "<a  class=\"btn-default-new br-5\" href=\"" . url('user/order/detail', ['order_id' => $row['order_id']]) . '">' . L('view_order') . '</a>';
                }
            }
        } else {
        ...
        if ($row['order_status'] == OS_CONFIRMED && $row['shipping_status'] == SS_RECEIVED && $row['pay_status'] == PS_UNPAYED) {
            $row['handler_return'] = url('user/order/goodsorder', ['order_id' => $row['order_id']]);
        }
        ...
        if ($sign_time > 0) {
            ...
            if ($row['order_status'] != OS_CANCELED && $row['pay_status'] == PS_PAYED) {
                if ($day < $sign_time) {
                    $row['handler_return'] = url('user/refound/index', ['order_id' => $row['order_id']]);
                } else {
                    @$row['handler_return'] = '';
                }
            }
        ...
        if ($row['order_status'] == OS_SPLITED && $row['pay_status'] == PS_PAYED && $row['shipping_status'] == SS_SHIPPED) {
            ...
            //$a < ($b + $c) - $noTime;
            $order_delay_day = C('shop.order_delay_day') * 86400;//$a
            $auto_delivery_time = $row['auto_delivery_time'] * 86400;//$b
            $shipping_time = $row['shipping_time'];//$c
        ...
        if ($row['user_order'] == OS_SPLITED && $row['user_shipping'] == SS_RECEIVED && $row['user_pay'] == PS_PAYED) {
            ...
            $row['handler'] = '';
        } elseif ($row['user_order'] == OS_SPLITED && $row['user_shipping'] == SS_SHIPPED) { // 确认收货(包含货到付款)
        ...
        if ($store_id > 0 && $row['shipping_status'] == SS_SHIPPED && $row['pay_status'] == PS_PAYED) {
            @$row['handler'] = "<a class=\"btn-default-new br-5 min-btn\" href=\"" . url('user/order/affirmreceived', ['order_id' => $row['order_id']]) . "\" onclick=\"if (!confirm('" . L('confirm_received') . "')) return false;\">" . L('received') . "</a>";
        }
        ...
        $arr[] = [
            ...
            'order_url' => url('user/order/detail', ['order_id' => $row['order_id']]),
            'delay' => $delay
        ];
    }
...
function save_consignee($consignee, $default = false)
{
    if ($consignee['address_id'] > 0) {
        ...
        $count = dao('user_address')->where(['user_id' => $consignee[user_id]])->count();
        if ($count == 1) {
            dao('users')->data(['address_id' => $address_id])->where(['user_id' => $consignee[user_id]])->save();
        }
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if ($order['order_status'] != OS_CANCELED && $order['order_status'] != OS_INVALID && $order['order_status'] != OS_RETURNED && $order['pay_status'] == PS_UNPAYED) {
        ...
        if (file_exists($plugin)) {
            ...
            $order['log_id'] = dao('pay_log')->where(['order_id' => $order['order_id']])->getField('log_id');
    ...
    if ($order['order_status'] != OS_CANCELED && $order['pay_status'] == PS_UNPAYED || $order['pay_status'] == PS_PAYED_PART &&
      ...
      $order['pay_online'] = url('onlinepay/index/index',['order_sn'=>$order[order_sn]]);
    } else {
        $order['pay_online'] = '';
    }
    ...
    // 订单优惠券
    $sql = "SELECT cu.uc_id, c.cou_money, cu.cou_id, c.cou_type FROM " . $GLOBALS['ecs']->table('coupons_user') . " as cu"
        . " LEFT JOIN " . $GLOBALS['ecs']->table('coupons') . " as c"
        . " ON cu.cou_id = c.cou_id"
        . " WHERE cu.order_id = " . $order['order_id'] . " AND cu.user_id = '".$_SESSION['user_id']."' ";
...
function return_to_cart($order_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $sql = "SELECT goods_number FROM" . $GLOBALS['ecs']->table('cart') . " " .
            ...
            "AND rec_type = '" . CART_GENERAL_GOODS . "' AND stages_qishu='-1' AND store_id = 0 LIMIT 1";
        ...
        if (empty($cart_goods)) {
            ...
            $condition = ($return_goods['goods_price'] > 0) ? ",goods_price = '" . $return_goods['goods_price'] . "' " : '';
            // 有相同商品，修改数量
 ...
 * @access  public
 * @param   int $user_id 用户ID
 * @param   int $type 0 未使用 1 已使用 2 已过期
 * @param   int $size 列表显示条数
 * @param   int $page 页数
 * @return  array
 */
function get_user_bouns_list($user_id, $type = 0, $size = 10, $page = 1)
{
    $cur_date = gmtime();
    $where = '';
    if ($type == 0) {
    ...
    $total = get_user_conut_bonus($user_id, $type);

    $sql = "SELECT u.bonus_sn, u.order_id, b.user_id as ru_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date, b.usebonus_type " .
        " FROM " . $GLOBALS['ecs']->table('user_bonus') . " AS u ," .
        $GLOBALS['ecs']->table('bonus_type') . " AS b " .
        " WHERE  u.bonus_type_id = b.type_id " . $where . " AND u.user_id = '" . $user_id . "'";
    ...
    foreach ($res as $row) {
        ...
        if (empty($row['order_id'])) {
            ...
            if ($row['use_start_date'] > $cur_date) {
                ...
                $row['status'] = L('overdue'); // 已过期
                $row['bonus_status'] = 3;
            } else {
                $row['status'] = L('not_use'); // 未使用
                $row['bonus_status'] = 0;
            }
        } else {
            $row['status'] = L('had_use');  // 已使用
            $row['order_url'] = url('user/account/detail', ['order_id' => $row['order_id']]);
            $row['bonus_status'] = 1;
        }
        ...
        if ($row['usebonus_type'] == 0) {
            $row['shop_name'] = sprintf(L('use_limit'), get_shop_name($row['ru_id'], 1));
        } else {
            $row['shop_name'] = '全场通用';
        }
    ...
    return ['list' => $arr, 'totalpage' => ceil($total / $size)];
}

/**
 * 用户红包类型数量
 * @param  integer  $user_id
 * @param  integer $type 0 未使用 1 已使用 2 已过期
 * @return
 */
function get_user_conut_bonus($user_id, $type = 0)
{
    $cur_date = gmtime();
    $where = '';
    if ($type == 0) {
