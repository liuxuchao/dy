...
function get_user_info($id = 0)
{
    ...
    $sql = 'SELECT u.user_id, u.email, u.mobile_phone, u.user_name, u.nick_name, u.user_money, u.pay_points' .
        ' FROM ' . $GLOBALS['ecs']->table('users') . ' AS u ' .
        " WHERE u.user_id = '$id'";
    ...
    $user['integral'] = $user['pay_points'] . $GLOBALS['_CFG']['integral_name'];
...
function recalculate_price()
{
    ...
    if ($user_id > 0) {
        ...
        $sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id, c.ru_id, c.warehouse_id, c.area_id, c.area_city, g.promote_price, g.promote_start_date, c.goods_number, c.goods_price as c_price, ' .
...
/**
 * 调用购物车商品数量
 * @param   $type 购物车商品类型 默认普通商品
 * @return
 */
function cart_number($type = CART_GENERAL_GOODS)
{
    ...
    $sql = 'SELECT SUM(goods_number) AS number ' .
        ...
        " WHERE " . $sess_id . " AND rec_type = '" . $type . "' AND stages_qishu='-1' AND store_id = 0";
    $row = $GLOBALS['db']->GetRow($sql);
