...
function get_ip_area_name($ip = '', $api = 0)
{
    ...
    $area_name = str_replace(['省', '市'], '', $region);
...
function get_order_query_sql($type = 'finished', $alias = '')
{
    ...
    if ($type == 'finished') {
        ...
        return " AND   {$alias}order_status " .
            ...
            " AND ( {$alias}pay_status " . db_create_in([PS_PAYED, PS_PAYING]) . " OR {$alias}pay_id " . db_create_in(get_payment_id_list(true)) . ") ";
    } /* 待付款订单 */ elseif ($type == 'await_pay') {
        return " AND   {$alias}order_status " . db_create_in([OS_CONFIRMED, OS_SPLITED]) .
            " AND   {$alias}pay_status = '" . PS_UNPAYED . "'" .
            " AND ( {$alias}shipping_status " . db_create_in([SS_SHIPPED, SS_RECEIVED]) . " OR {$alias}pay_id " . db_create_in(get_payment_id_list(false)) . ") ";
        ...
        return " AND {$alias}order_status = '" . OS_CONFIRMED . "'" .
            " AND {$alias}shipping_status " . db_create_in([SS_SHIPPED, SS_RECEIVED]) . " ";
    } /* 待确认收货：已确认订单、已付款、已发货（待用户确认收货） */ elseif ($type == 'to_confirm') {
        return " AND {$alias}order_status " . db_create_in([OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART]) . " " .
...
function get_merchants_article_menu($cat_id)
{
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['open_type'] != 1) {
            $arr[$key]['url'] = build_uri('merchants', ['mid' => $row['article_id']], $row['title']);
        } else {
...
//查找字段数据 end
//添加类目证件标题
function get_documentTitle_insert_update($dt_list, $cat_id, $dt_id = [])
{
...
function get_root_merchants_steps_title($pid, $user_id)
{
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['steps_style'] == 1) {
            ...
            $arr[$key]['first_cate'] = get_first_cate_list(0, 0, [], $_SESSION['user_id']);
...
function get_update_temporarydate_isAdd($catId_array, $type = 0)
{
    ...
    if ($type == 0) {
        ...
        for ($i = 0; $i < count($catId_array); $i++) {
            ...
            $cat_id = isset($_POST['permanentCat_id_' . $parentChild[0]]) ? $_POST['permanentCat_id_' . $parentChild[0]] : [];
            $dt_id = isset($_POST['permanent_title_' . $parentChild[0]]) ? $_POST['permanent_title_' . $parentChild[0]] : [];
            ...
            $permanent_date = isset($_POST['categoryId_date_' . $parentChild[0]]) ? $_POST['categoryId_date_' . $parentChild[0]] : [];
...
function get_first_cate_list($parent_id = 0, $type = 0, $catarr = [], $user_id = 0)
{
    if ($type == 1) {
        ...
        $sql = "SELECT cat_id, cat_name, cat_alias_name FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$parent_id' order by sort_order asc";
...
function get_add_childCategory_info($cat_id, $user_id)
{
    ...
    $sql = "SELECT cat_id, cat_name, parent_id, cat_alias_name FROM " . $GLOBALS['ecs']->table('category') . " WHERE cat_id in ($cat_id) ORDER BY cat_id";
    $res = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($res as $key => $row) {
        ...
        $arr[$key]['cat_name'] = !empty($row['cat_alias_name']) ? $row['cat_alias_name'] : $row['cat_name'];

        $parents = dao('category')->field('cat_name, cat_alias_name')->where(['cat_id' => $row['parent_id']])->find();
        $arr[$key]['parent_name'] = !empty($parents['cat_alias_name']) ? $parents['cat_alias_name'] : $parents['cat_name'];;

        $parent = [
            'user_id' => $user_id,
            'cat_id' => $row['cat_id'],
            'parent_id' => $row['parent_id'],
            'cat_name' => $arr[$key]['cat_name'],
...
function get_fine_category_info($cat_id, $user_id)
{
    ...
    $sql = "SELECT ct_id, cat_id, cat_name, parent_name FROM " . $GLOBALS['ecs']->table('merchants_category_temporarydate') . " WHERE user_id = '$user_id' ORDER BY ct_id ASC";
    $res = $GLOBALS['db']->getAll($sql);
...
function get_insert_warehouse_goods($goods_id = 0, $warehouse_name = [], $warehouse_number = [], $warehouse_price = [], $warehouse_promote_price = [], $user_id = 0)
{
    ...
    for ($i = 0; $i < count($warehouse_name); $i++) {
        if (!empty($warehouse_name[$i])) {
            ...
            if ($w_id > 0) {
                $link[] = ['text' => '返回一页', 'href' => 'goods.php?act=edit&goods_id=' . $goods_id . '&extension_code='];
...
function get_goods_bacth_warehouse_list($goods_list)
{
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        ...
        $arr[$i]['user_id'] = get_table_date('goods', $where_goods, ['user_id'], 2);
        $arr[$i]['goods_id'] = get_table_date('goods', $where_goods, ['goods_id'], 2);
        $arr[$i]['region_id'] = get_table_date('region_warehouse', $where_region, ['region_id'], 2);
...
function get_goods_bacth_area_attr_list($goods_list)
{
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        ...
        $arr[$i]['goods_id'] = get_table_date('goods', $where_goods, ['goods_id'], 2);
        $arr[$i]['area_id'] = get_table_date('region_warehouse', $where_region, ['region_id'], 2);
        $arr[$i]['goods_attr_id'] = get_table_date('goods_attr', $where_attr, ['goods_attr_id'], 2);
...
//查询属性商品仓库库存
function get_warehouse_id_attr_number($goods_id = 0, $attr_id = '', $seller_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0,$model_attr = '', $store_id = 0)
{
    if (empty($model_attr)) {
    ...
    if (empty($attr_id)) {
        ...
        foreach ($attr_arr as $key => $val) {
            $attr_type = get_goods_attr_id(['goods_id'=> $goods_id, 'goods_attr_id' => $val], ['a.attr_type']);
...
function get_new_goods_attribute($goods_id, $_attribute = [])
{
    $arr = [];
...
function get_goods_attr_values_id($attr_values = [], $goods_attr_id = [])
{
    $arr = [];
...
//主次订单拆分新数组
function get_new_ru_goods_info($all_ruId = '', $order_goods = [])
{
    ...
    if ($type == 0) {
        ...
        foreach ($newInfo as $key => $row) {
            ...
            if ($key > 0) {
                ...
                $arr[$key]['compute_discount'] = ['discount' => 0, 'name' => []];
            }
        }
    }
...
function get_insert_order_goods_single($orderInfo, $row, $order_id, $ru_number)
{
    ...
    $use_bonus = 0;
    $discount = !empty($row['discount']) ? $row['discount'] : 0; //折扣金额
    ...
    $sql = "SELECT vc_id, use_val FROM " .$GLOBALS['ecs']->table('value_card_record'). " WHERE order_id = '$order_id' LIMIT 1";
    $cartUseVal = $GLOBALS['db']->getRow($sql);
    ...
    $point_id = empty($row['point_id']) ? '' : $row['point_id'];
    $shipping_dateStr = empty($row['shipping_datestr']) ? '' : $row['shipping_datestr']; // 注意查询出来的字段是小写的

    $postscript_value = !empty($row['postscript']) ? $row['postscript'] : '';
    ...
    foreach ($newInfo as $key => $info) {
        ...
        $row['shipping_name'] = empty($shipping['shipping_name']) ? '' : $shipping['shipping_name'];
        $row['shipping_code'] = empty($shipping['shipping_code']) ? '' : $shipping['shipping_code'];
        $row['shipping_type'] = $shipping['shipping_type'];

        // 分单上门自提（自营）
        $point_info = get_seller_cac_order($key, $point_id, $shipping_dateStr);
        $row['point_id'] = $point_info['point_id'];
        $row['shipping_dateStr'] = $point_info['shipping_dateStr'];

        // 分单留言
        $postscript = get_seller_postscript($key, $postscript_value);
        $row['postscript'] = $postscript['postscript'];
        ...
        do {
            ...
            if (isset($row['uc_id']) && !empty($row['uc_id'])) {
                $couponsInfo = get_coupons($row['uc_id'], ['c.cou_id', 'c.cou_man', 'c.cou_type', 'c.ru_id', 'c.cou_money', 'cu.uc_id'], $row['user_id'], $key);
            }
            ...
            if ($cartValOther && $new_orderId > 0) {
                ...
                $cartValOther['record_time'] = gmtime();
                $GLOBALS['db']->table('value_card_record')->data($cartValOther)->add();
            }
            ...
            if ($key == 0) {
                ...
                // 商家
                $sql = "SELECT mobile, seller_email FROM " . $GLOBALS['ecs']->table('seller_shopinfo') . " WHERE ru_id = '".$key."' LIMIT 1 ";
        ...
        $order_goods_num = count($arr[$key]);
        for ($j = 0; $j < $order_goods_num; $j++) {
            ...
            if ($virtual_goods AND $flow_type != CART_GROUP_BUY_GOODS) {
                ...
                if (virtual_goods_ship($virtual_goods, $msg, $order_sn, true)) {
                    ...
                    if ($GLOBALS['db']->getOne($sql) <= 0) {
                        ...
                        update_order($arr[$key][$j]['order_id'], ['order_status' => OS_CONFIRMED, 'shipping_status' => SS_SHIPPED, 'shipping_time' => gmtime()]);
                    }
                }
            }
        }
...
/* 获取子订单积分比例 by kong */
function get_integral_ratio($order_id = 0, $info = [])
{
    // 获取订单商品总共可用积分
...
function get_integral($order_id = 0, $goods_id = [])
{
    $where = '';
    if (!empty($goods_id)) {
...
// 商家配送方式分单分组
function get_seller_shipping_order($ru_id = 0, $shipping_id = '', $shipping_name = '', $shipping_code = '', $shipping_type = '')
{
    ...
    $shipping = ['shipping_id' => $shippingId, 'shipping_name' => $shippingName, 'shipping_code' => $shippingCode, 'shipping_type' => $shippingType];
    return $shipping;
}

// 商家买家留言分单分组
function get_seller_postscript($ru_id = 0, $postscript_arr = '')
{
    $postscript_arr = explode(',', $postscript_arr);

    $new_postscript = '';
    if (!empty($postscript_arr)) {
        foreach ($postscript_arr as $key => $row) {
            $row = explode('|', $row);
            if ($row[0] == $ru_id) {
                $new_postscript = $row[1];
            }
        }
    }

    return ['postscript' => $new_postscript];
}
...
function get_seller_cac_order($ru_id, $point_id, $shipping_dateStr)
{
    ...
    return ['point_id' => $pointId, 'shipping_dateStr' => $dateStr];
}
...
{
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, og.goods_number, og.goods_price, og.goods_price, og.extension_code, og.goods_name AS extension_name, oi.order_sn, og.parent_id, og.is_gift, ga.act_id FROM " . $GLOBALS['ecs']->table('order_goods') . " as og " .
        ...
        "WHERE og.order_id = '$order_id' GROUP BY g.goods_id ORDER BY og.parent_id ASC, og.rec_id DESC";
    $res = $GLOBALS['db']->getAll($sql);
    $arr = [];
    foreach ($res as $key => $row) {
        $arr[$key]['goods_id'] = $row['goods_id'];
        $arr[$key]['parent_id'] = $row['parent_id'];
        $arr[$key]['is_gift'] = $row['is_gift'];
        ...
        if ($row['extension_code'] == 'package_buy') {
            ...
            $activity = get_goods_activity_info($row['act_id'], ['act_id', 'activity_thumb']);
...
function get_user_store_category($ru_id)
{
    ...
    foreach ($res as $key => $row) {
        $arr[$key] = $row;
        $arr[$key]['url'] = build_uri('merchants_store', ['cid' => $row['cat_id'], 'urid' => $ru_id], $row['cat_name']);
        $arr[$key]['opennew'] = 0;
...
function get_goods_minMax_price($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $goods_price, $market_price, $type = 1)
{
    $model_attr = get_table_date("goods", "goods_id = '$goods_id'", ['model_attr'], 2);
    ...
    if ($type == 1) { //商品组合购买
        ...
        $goodsLeftJoin .= " left join " . $GLOBALS['ecs']->table('warehouse_area_goods') . " as wag on g.goods_id = wag.goods_id and wag.region_id = '$area_id' " . $where_area;
 ...
 * 2 = 二维数组
 */
function get_unset_null_array($arr = [], $type = 0)
{
 ...
 * 运算商品详情设置非按区域运费模式价格
 */
function get_order_transport($goods_list, $consignee = [], $shipping_id = 0, $shipping_code = '')
{
...
function get_goods_area_shipping($goods_list, $shipping_id = 0, $shipping_code = '',$consignee = [])
{
    ...
    if(empty($shipping_id)){
        $select = ['shipping_code' => $shipping_code];
        $shipping_info = shipping_info($select, ['shipping_id']);
        $shipping_id = $shipping_info['shipping_id'];
    }

    if(empty($shipping_code)){
        $shipping_info = shipping_info($shipping_id, ['shipping_code']);
...
function getSellerShippingFee($sellerOrderInfo = [], $cart_goods)
{
    //获取配送区域
 ...
 */
function get_goods_attr_id($where_select = [], $select = [], $attr_type = 0, $retuen_db = 0)
{
    if ($where_select) {
    }
