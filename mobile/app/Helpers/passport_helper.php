    ...
    if (!$GLOBALS['user']->add_user($username, $password, $email)) {
        ...
        if (isset($affiliate['on']) && $affiliate['on'] == 1) {
            ...
            if ($up_uid) {
                ...
                //设置推荐人
                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $up_uid . ', drp_parent_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];
        ...
        if (is_dir(APP_DRP_PATH)) {
            ...
            if (isset($affiliate['on']) && $affiliate['on'] == 1) {
                ...
                if ($up_drpid) {
                    //设置推荐人
                    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET drp_parent_id = ' . $up_drpid . ', parent_id = ' . $up_drpid . ' WHERE user_id = ' . $_SESSION['user_id'];
                    $GLOBALS['db']->query($sql);
                }
            }
            /*自动注册分销商 by han sty*/
            $register = dao('drp_config')->field('value')->where(['code' => 'register'])->find();
            if ($register['value'] == 1) {
            ...
            /*自动注册分销商 end*/
              ...
              //模板消息
            if($other['drp_parent_id'] > 0){
                $time = gmtime();
                $pushData = [
                    'keyword1' => ['value' => $_SESSION['user_id'], 'color' => '#173177'],
                    'keyword2' => ['value' => date('Y-m-d', $time), 'color' => '#173177'],
                    'remark'   => ['value' => $other['nick_name'].'新会员加入', 'color' => '#173177']
                ];
        ...
        //定义other合法的变量数组
        $other_key_array = ['aite_id', 'nick_name', 'sex', 'user_picture', 'msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'parent_id', 'drp_parent_id'];
        ...
        dao('users')->data($update_data)->where(['user_id' => $_SESSION['user_id'] ])->save();
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    /* 设置重置邮件模板所需要的内容信息 */
    //    $code=rand(1000,9999);
    session('maildata', ['email'=>$email,'code'=>md5($code)]);
