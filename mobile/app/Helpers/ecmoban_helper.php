...
function get_discount($goods)
{
    ...
    return ['jiesheng' => $jiesheng, 'zhekou' => $zhekou];
}
...
function get_goods_con_list($goods_id = 0, $table, $type = 0)
{
    ...
    foreach ($res as $key => $row) {
        $arr[$key]['id'] = $row['id'];
        if ($type == 0) {
            $arr[$key]['cfull'] = floatval($row['cfull']);
            $arr[$key]['creduce'] = floatval($row['creduce']);
        } elseif ($type == 1) {
            $arr[$key]['sfull'] = floatval($row['sfull']);
            $arr[$key]['sreduce'] = floatval($row['sreduce']);
        }
    }
...
function get_cart_combo_goods_list($goods_id = 0, $parent = 0, $group_id = '')
{
    //ecmoban模板堂 --zhuo start
    ...
    $sql = "SELECT goods_price, goods_number, goods_id FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE " . $sess_id .
        " AND (parent_id = '$parent' OR (goods_id = '$parent' AND parent_id = '0')) AND group_id = '$group_id'";
    $res = $GLOBALS['db']->getAll($sql);
...
function get_user_default($user_id = 0)
{
    ...
    $info['integral'] = $row['pay_points'] . $GLOBALS['_CFG']['integral_name']; // 积分

    $info['user_money'] = $row['user_money'];
    $info['surplus'] = price_format($row['user_money'], false); // 余额
    $info['credit_line'] = $row['credit_line']; // 信用额度
...
function get_guess_goods($user_id, $history = 0, $page = 1, $limit = 5, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    if (empty($history)) {//用户中心
        ...
        if ($order_arr) {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.user_id, g.market_price, ' .
                    "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
                    "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price," .
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                    " WHERE $link_cats_str AND g.goods_id NOT IN ($finished_goodsStr) AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $tag_where ORDER BY g.sales_volume DESC  LIMIT $start, $limit";
        ...
        if (!empty($_COOKIE['ECS']['history'])) {
            ...
            $sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, g.model_attr, g.market_price, " .
                    "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
                    "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price," .
    ...
    if ((empty($query) || count($query) < $limit) && $history == 1) {
        $sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, g.user_id, g.model_attr, g.market_price," .
                "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
                "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price," .
    ...
    if ($query) {
        foreach ($query as $row) {
            ...
            $guess_goods[$row['goods_id']]['shop_price'] = $row['shop_price'];
            $guess_goods[$row['goods_id']]['shop_price_formated'] = price_format($row['shop_price']);
            $guess_goods[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
            $guess_goods[$row['goods_id']]['market_price'] = price_format($row['market_price']);
