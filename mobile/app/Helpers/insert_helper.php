    ...
    foreach ($res as $key=>$row) {
        ...
        switch ($row['media_type']) {
            case 0: // 图片广告
                ...
                $ads[] = "<a href='" .url("affiche/index/index", ["ad_id"=>$row["ad_id"], "uri"=>urlencode($row["ad_link"])]). "' target='_blank'><img src='$src' /></a>";
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                    get_data_path($row['ad_code'], 'afficheimg') : $row['ad_code'];
            ...
            case 2: // CODE
                ...
                break;
            case 3: // TEXT
                $ads[] = "<a href='" . url('affiche/index/index', ['id'=>$row['ad_id'], 'uri'=>urlencode($row['ad_link'])]) . "' target='_blank'>" . htmlspecialchars($row['ad_code']) . '</a>';
                break;
        }
    }
...
function insert_team($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1) {
        if (!empty($arr['tc_id']) && $arr['tc_id']>0) {
            $where .=" and p.tc_id = '" . $arr['tc_id']."' and p.tc_type = '" . $arr['type']."' and p.ad_type !='wxapp' " ;
        } else {
            $where .= '';
        }
        $sql = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
