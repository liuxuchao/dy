...
function get_child_tree($tree_id = 0, $top = 0)
{
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        $child_sql = 'SELECT c.cat_id, c.cat_name, c.touch_icon, c.cat_icon, c.touch_icon,c.parent_id, c.cat_alias_name, c.is_show, (SELECT goods_thumb FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE cat_id = c.cat_id AND is_on_sale = 1 AND is_delete = 0 ORDER BY sort_order ASC, goods_id DESC limit 1 ) as goods_thumb ' .
...
function get_top10($cats = '', $presale)
{
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++) {
        ...
        if ($arr[$i]['market_price'] > 0) {
            $discount_arr = get_discount($arr[$i]); //函数get_discount参数goods_id
        }
 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '', $warehouse_id = 0, $area_id = 0, $area_city = 0, $ru_id = 0, $rec_type = 0, $presale = '', $limit = 0)
{
            ...
            foreach ($type2lib as $key => $data) {
                if (!empty($goods_data[$key])) {
                    ...
                    if ($order_type == 0) {
                        ...
                        $rand_key = array_slice($goods_data[$key], 0, $limit);
                        foreach ($rand_key as $key_data) {
                            $type_array[$key][] = $key_data['goods_id'];
                        }
                    } else {
                        $rand_key = array_rand($goods_data[$key], $limit);
                        if ($num == 1) {
 ...
 * @return bool
 */
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    $time = gmtime();
    $tag = [];
    //ecmoban模板堂 --zhuo start
...
function get_goods_properties($goods_id, $warehouse_id = 0, $area_id = 0, $goods_attr_id = '', $attr_type = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, " .
        ...
        'ORDER BY a.sort_order, a.attr_id, g.goods_attr_id ASC';
    ...
    foreach ($res as $row) {
        ...
        if ($row['attr_type'] == 0) {
            ...
            //     $arr['spe'][$row['attr_id']]['is_checked'] = $row['attr_checked'];
            // }
            ...
            if (isset($arr['spe'][$row['attr_id']]['values']) && $arr['spe'][$row['attr_id']]['values']) {
                $arr['spe'][$row['attr_id']]['values'] = get_array_sort($arr['spe'][$row['attr_id']]['values'], 'attr_sort'); // 排序
                $arr['spe'][$row['attr_id']]['is_checked'] = get_attr_values($arr['spe'][$row['attr_id']]['values']);  // 是否选中
            }
        }
 ...
 * @return  array
 */
function goods_info($goods_id, $warehouse_id, $area_id, $select = [], $attr_id = '' ,$area_city = 0)
{
    $leftJoin = '';
    $where_area = '';
    if(C('shop.area_pricetype') == 1){
    ...
    if (!empty($row)) {
        ...
        $row['goods_price'] = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $warehouse_id, $area_id, $area_city, 0, 0, $add_tocart);
 ...
 * @return  array
 */
function get_goods_fittings($goods_list = [], $warehouse_id = 0, $area_id = 0, $area_city = 0, $rev = '', $type = 0, $goods_equal = [])
{
    $fitts_goodsList = '';
    ...
    foreach ($res as $row) {
        ...
        $arr[$temp_index]['spare_price_ori'] = floatval($row['shop_price'] - $row['goods_price']);//节省的差价 by mike
        ...
        // 默认选中
        $default_spe = '';
        $properties_spe = $arr[$temp_index]['properties']['spe'];
        if ($properties_spe) {
            foreach ($properties_spe as $k => $v) {
                if ($v['attr_type'] == 1) {
                    if ($v['is_checked'] > 0) {
            ...
            $arr[$temp_index]['default_spe'] = $default_spe;
        }
 ...
 * @return array
 */
function get_goods_fittings_info($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $group_id = '', $type = 0, $fittings_goods = 0, $fittings_attr = array())
{
    if (!empty($_SESSION['user_id'])) {
        $sess_id = " cc.user_id = '" . $_SESSION['user_id'] . "' ";
    } else {
    ...
    if ($type == 0) {
        $leftJoin .= " left join " . $GLOBALS['ecs']->table('cart_combo') . " as cc on g.goods_id = cc.goods_id ";
        $where .= " and cc.group_id = '$group_id' and " . $sess_id;
    ...
    foreach ($res as $row) {
        ...
        $sql = "SELECT cc.img_flie FROM " . $GLOBALS['ecs']->table('cart_combo') . " as cc" . " WHERE cc.goods_id = '" . $row['goods_id'] . "'" . " AND cc.group_id = '$group_id' AND " . $sess_id;
        $img_flie = $GLOBALS['db']->getOne($sql);
        $arr[$temp_index]['img_flie'] = $img_flie;
