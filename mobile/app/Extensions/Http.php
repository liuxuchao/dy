...
class Http
{
     ...
     * @return string
     */
    public static function doPost($url, $post_data = [], $timeout = 5, $header = "")
    {
     ...
     * @return string
     */
    public static function socketPost($url, $post_data = [], $timeout = 5, $header = "")
    {
        $header = empty($header) ? self::defaultHeader() : $header;
        $post_string = http_build_query($post_data);
        $url2 = parse_url($url);
     ...
     * @return string
     */
    public static function phpPost($url, $post_data = [], $timeout = 5, $header = "")
    {
        $header = empty($header) ? self::defaultHeader() : $header;
        $post_string = http_build_query($post_data);
    ...
    {
        $header = "User-Agent:Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.2.12) Gecko/20181026 Firefox/3.6.12\r\n";
