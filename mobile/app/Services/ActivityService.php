...
class ActivityService
{
    ...
    {  
        ...
        foreach ($activity_list as $row) {
            ...
            $row['status'] = $this->activityRepository->getStatus($row['start_time'], $row['end_time']); // 0 未开始, 1 正在进行, 2 已结束
            ...
            if ($row['act_range'] == FAR_CATEGORY) {
                ...
                if ($cat_rows) {
                    foreach ($cat_rows as $v) { 
                        $cat_children = array_unique(array_merge([$v], $this->categoryRepository->arr_foreach($this->categoryRepository->catList($v))));
                        if ($cat_children) {
                            //只有1级分类，不需要去重
