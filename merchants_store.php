...
if(($ua == '' || preg_match($uachar, $ua))&& !strpos(strtolower($_SERVER['REQUEST_URI']),'wap'))
{
    ...
    // 优化二级域名下 PC店铺页自动跳转wap店铺页,需将二级域名换成主域名 以免微信授权登陆问题
    if (strpos(strtolower($_SERVER['HTTP_HOST']), "www") === false ) {
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_warehouse') {
    ...
    die($json->encode($res));
}elseif($_REQUEST['act'] == 'merchants_licence'){
...
//商家二维码 by wu start
    $sql = "select ss.*,sq.*, msf.license_fileImg,msf.companyName,msf.business_license_id,msf.legal_person,msf.license_comp_adress"
            . ",msf.license_adress,msf.business_term,msf.busines_scope,msf.company_located,msf.company_adress,msf.registered_capital from " . $ecs->table('seller_shopinfo') . " as ss " .
...
//print_arr($basic_info);
    $smarty->assign('grade_info', $grade_info);
    $smarty->assign('site_domain', $_CFG['site_domain']);  //网站域名
	$smarty->assign('licence_type', 1);  //店铺信息标识

    $smarty->display('merchants_licence.dwt');exit;
}
...
if (!$smarty->is_cached($dwt, $cache_id))
{
   assign_template('',array(),$merchant_id);
    ...
    if ($cat_id > 0) {
        ...
        if ($cat['grade'] > 1) {
            ...
            $sql = "SELECT min(IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price))) AS min, " .
                    ...
                    " WHERE $cat_where g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_show = 1 ";
            //获得当前分类下商品价格的最大值、最小值
            ...
            $sql = "SELECT (FLOOR((IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) - $row[min]) / $dx)) AS sn, COUNT(*) AS goods_num  " .
                    " FROM " . $ecs->table('goods') . " AS g " . $leftJoin .
                    " WHERE $cat_where g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_show = 1 AND g.is_alone_sale = 1 " .
                    " GROUP BY sn ";
        ...
        if ($cat['filter_attr'] > 0) {
            ...
            foreach ($cat_filter_attr AS $key => $value) {
                ...
                if ($attributeInfo) {
                    ...
                    $sql = "SELECT a.attr_id, MIN(a.goods_attr_id) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a " . 
    ...
    $count = get_store_goods_count($children, $brand, $price_min, $price_max, $ext, $merchant_id, $region_id, $area_info['region_id'], $area_city, $keyword);
    $max_page = ($count> 0) ? ceil($count / $size) : 1; 
    if ($page > $max_page)
    {
        $page = $max_page;
    }
    $goodslist = store_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order, $merchant_id, $region_id, $area_info['region_id'], $area_city, $keyword);
    ...
    $goods_hot = get_hot_new_best_goods($children, $goods_hot, $merchant_id, $region_id, $area_info['region_id'], $area_city);
    $smarty->assign('goods_hot',         $goods_hot); //热销商品

    $goods_new = get_hot_new_best_goods($children, $goods_new, $merchant_id, $region_id, $area_info['region_id'], $area_city);
 ...
 * @return  integer
 */
function get_store_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='', $merchant_id, $warehouse_id = 0, $area_id = 0, $area_city, $keyword)
{
...
//热门商品
function get_hot_new_best_goods($children = '', $type = '', $merchant_id, $warehouse_id = 0, $area_id = 0, $area_city = 0) {
