...
if (isset($_REQUEST['act'])) {
    ...
    if (defined('THEME_EXTENSION')) {
        ...
        if ($act == 'view') {
            ...
            if ($row['userFav_type']) {
                $row['shop_name'] = $_LANG['His_general']; //商家名称
            } else {
            ...
            if ($row['act_range'] != FAR_ALL && !empty($row['act_range_ext'])) {
                if ($row['act_range'] == FAR_CATEGORY) {
                    ...
                    $sql = "SELECT g.goods_id AS id, g.goods_name AS name, " .
                            "IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]') AS price, " .
                            " g.goods_thumb as thumb FROM " . $ecs->table('goods') . " AS g " .
                            $leftJoin .
                            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                            "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                            " WHERE g.goods_id " . db_create_in($row['act_range_ext']);
                }
			...
			$filter_array = get_filter_goods_list($filter, $size, $page, "sort_order", "ASC", $warehouse_id, $area_id, $area_city);
...
$sql = "SELECT * FROM " . $ecs->table('favourable_activity'). " WHERE review_status = 3 AND end_time > '".gmtime()."' ORDER BY `sort_order` ASC,`end_time` DESC";
$res = $db->query($sql);
