...
if($_GET['act']=='send'){
    ...
    if (($_GET['flag'] == 'register' && (intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0) || isset($_POST['captcha'])) {
    ...
    else
    {
        //阿里大鱼短信接口
        $message = array(
            'mobile_code'       => $mobile_code,    //短信验证码
            'user_name'           => $username,     //会员名称
            'sms_value'           => $sms_value      //验证码类型
        );
    } 
