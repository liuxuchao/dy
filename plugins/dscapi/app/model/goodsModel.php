...
abstract class goodsModel extends common {
    ...
    public function get_select_list($table, $select, $where, $page_size, $page, $sort_by, $sort_order) {
        ...
        if ($table == 'warehouse_area_goods') {
            ...
            if (!empty($region_where)) {
                $left_join = " LEFT JOIN " . $GLOBALS['ecs']->table('region_warehouse') . " AS r1 ON r1.region_id = wag.region_id " . $region_where;
            }

            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('warehouse_area_goods') . " AS wag " .
                    $left_join .
                    " WHERE " . $where;
        ...
        if ($table == 'warehouse_area_goods') {
            ...
            $sql = "SELECT wag.*, r1.region_name FROM " . $GLOBALS['ecs']->table('warehouse_area_goods') ." AS wag ". 
                    " LEFT JOIN " . $GLOBALS['ecs']->table('region_warehouse') . " AS r1 ON r1.region_id = wag.region_id " . $region_where . 
                    " WHERE " . $where;
            ...
            foreach($select as $k=>$v){
                ...
                if($v['goodsDetailList']){
                    ...
                    foreach($v['goodsDetailList'] as $detail_key=>$detail_val){
                        ...
                        {
                            // if(common::get_http_basename($detail_val['imagePath'], $detail_dir)){
                            //     $image_url = trim($detail_val['imagePath']);

                            //     //定义原图路径
                            //     $down_img = $detail_dir ."/". basename($image_url);
                            //     $goods_desc_arr[$detail_key]['orderNo'] = $detail_val['orderNo'];
                            //     $goods_desc_arr[$detail_key]['path'] = "/images/upload/cloud/". basename($image_url);
                            //     $goods_desc_arr[$detail_key]['basename'] = basename($image_url);
                            // }
                    ...
                    // foreach($goods_desc_arr as $val){
                    //     if($j == $val['orderNo']){
                    //         $goods_desc .= '<p><img src="'.$val['path'].'" title="'.$val['basename'].'"/></p>';
                    //     }
                    //     $j++;
                    // }
                ...
                if($v['goodsNavigateList']){
                    ...
                    foreach($v['goodsNavigateList'] as $gallery_key=>$gallery_val){
                        ...
                        {
                            if(common::get_http_basename($gallery_val['navigateImage'], $admin_temp_dir)){
                                $image_url = trim($gallery_val['navigateImage']);
                                //定义原图路径
                                $down_img = $admin_temp_dir ."/". basename($image_url);
                                    
                                $img_wh = common::get_width_to_height($down_img, $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height']);
                                ...
                                $goods_img = common::make_thumb(array('img' => $down_img, 'type' => 1), $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height']);
                                
                                // 生成缩略图
                                if ($proc_thumb) {
                                    $thumb_url = common::make_thumb(array('img' => $down_img, 'type' => 1), $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height']);
                                    $thumb_url = common::reformat_image_name('gallery_thumb', $id, $thumb_url, 'thumb');
                                }else{
                                    $thumb_url = common::make_thumb(array('img' => $down_img, 'type' => 1));
                                    $thumb_url = common::reformat_image_name('gallery_thumb', $id, $thumb_url, 'thumb');
                                }
                                
                                $img_original = common::reformat_image_name('gallery', $id, $down_img, 'source');
                                $img_url = common::reformat_image_name('gallery', $id, $goods_img, 'goods');
    ...
    public function get_goodsnotification_update($table, $select, $format){
        ...
        if(!empty($select)){
            ...
            $order = array();
            if($select['orderSn']){
                $sql = "SELECT oi.order_id , oi.order_sn , oi.user_id,oi.surplus,oi.money_paid,oi.bonus_id,oi.integral_money,oi.bonus,oi.coupons FROM".$GLOBALS['ecs']->table('order_goods')
                    ."AS og LEFT JOIN " . $GLOBALS['ecs']->table('order_cloud') 
                    . "AS oc ON oc.rec_id = og.rec_id LEFT JOIN " . $GLOBALS['ecs']->table('order_info') 
                    ...
                    if(!empty($order)){
                        foreach($order as $k=>$v){
                            ...
                            $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('order_action') .
                                ...
                                'FROM ' . $GLOBALS['ecs']->table('order_info') . " WHERE order_sn = '" . $v['order_sn'] . "'";
                            $GLOBALS['db']->query($sql);
