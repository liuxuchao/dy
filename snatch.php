...
if ($_REQUEST['act'] == 'list') {
    ...
    $snatch_list = get_snatch_list($keywords, $size, $page, $sort, $order, $region_id, $area_id, $area_city);
    ...
    $smarty->assign('hot_goods', get_exchange_recommend_goods('hot', $region_id, $area_id, $area_city));  //热门
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    $properties = get_goods_properties($goods['goods_id'], $region_id, $area_id, $area_city);  // 获得商品的规格和属性  //ecmoban模板堂 --zhuo
    
    $smarty->assign('cfg', $_CFG);
    $smarty->assign('properties', $properties['pro']);                              // 商品规格	
    $smarty->assign('specification', $properties['spe']);                              // 商品属性
    $smarty->assign('goods_id', $goods['goods_id']);                              // 商品ID
    $smarty->assign('region_id', $region_id);                              // 商品ID
    $smarty->assign('area_id', $area_id);                              // 商品ID
...
if ($_REQUEST['act'] == 'bid')
{
    ...
    $smarty->assign('snatch_goods',       $snatch_info); // 竞价商品
    
    $goods = get_goods_info($snatch_info['goods_id'], $warehouse_id, $area_id, $area_city);
    ...
    if ($goods_attr) {
        ...
        $products = get_warehouse_id_attr_number($goods['goods_id'], $goods_attr, $goods['user_id'], $warehouse_id, $area_id, $area_city);
...
if ($_REQUEST['act'] == 'buy')
{
    ...
    /* 查询：取得商品信息 */
    $goods = get_goods_info($snatch['goods_id'], $warehouse_id, $area_id, $area_city);
 ...
 * @return void
 */
function get_snatch_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id, $area_city = 0)
{
    $where_area = '';
    if ($GLOBALS['_CFG']['area_pricetype'] == 1) {
 ...
 * @return  array
 */
function get_exchange_recommend_goods($type = '', $warehouse_id, $area_id, $area_city = 0)
{
    $where_area = '';
    if ($GLOBALS['_CFG']['area_pricetype'] == 1) {
        $where_area = " AND wag.city_id = '$area_city'";
