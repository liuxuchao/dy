...
require(ROOT_PATH . 'languages/' .$_CFG['lang']. '/admin/common_stores.php');
...
{
    @mkdir('../temp/compiled/' . STORES_PATH, 0777);
    @chmod('../temp/compiled/' . STORES_PATH, 0777);
}
...
if(isset($_GET['ent_id']) && isset($_GET['ent_ac']) &&  isset($_GET['ent_sign']) && isset($_GET['ent_email']))
{
    ...
    $domain_url = $ecs->url();
    $token=$_GET['token'];
    if($token==md5(md5($_CFG['token']).$domain_url.STORES_PATH))
    {
...
/* 验证管理员身份 */
if ((!isset($_SESSION['store_user_id']) || intval($_SESSION['store_user_id']) <= 0) &&
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['store_user_id']) && !empty($_COOKIE['ECSCP']['stores_pass']))
    {
        ...
        $sql = 'SELECT id, stores_user, stores_pwd ' .
                ' FROM ' .$ecs->table('store_user') .
                " WHERE id = '" . intval($_COOKIE['ECSCP']['store_user_id']) . "'";
        $row = $db->GetRow($sql);
        ...
        {
            // 检查密码是否正确
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['stores_pass'])
            {
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    $admin_path = preg_replace('/:\d+/', '', $ecs->stores_url()) . STORES_PATH;//重置路径
...
if(isset($_SESSION['stores_name'])){
    ...
    $uname = '';
    if($_SESSION['store_user_id'] > 0 && $_SESSION['store_user_id'] != $uid){
