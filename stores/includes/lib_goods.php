...
function handle_link_goods($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[store_user_id]'";
    $GLOBALS['db']->query($sql);
...
function handle_goods_area($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('link_area_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND ru_id = (SELECT ru_id FROM " . $GLOBALS['ecs']->table('admin_user') . "  WHERE user_id = '$_SESSION[store_user_id]') ";
    $GLOBALS['db']->query($sql);
}
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $single_id = 0, $files_type = 0)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if($GLOBALS['_CFG']['open_oss'] == 1){
            ...
            $urlip = get_ip_url($GLOBALS['ecs']->stores_url());
 ...
 * @return  void
 */
function handle_gallery_image_add($goods_id, $image_files, $image_descs, $image_urls, $single_id = 0, $files_type = 0,$is_ajax)
{
	if($files_type == 0){
		$files_type = 'single_id';
	}elseif($files_type = 1){
	...
	$max_image_descs=$GLOBALS['db']->getOne(" SELECT MAX(img_desc) FROM".$GLOBALS['ecs']->table('goods_gallery')." WHERE goods_id = '$goods_id'");//获取最大排序  by kong
	$i=0;
	foreach ($image_descs as $k=>$v){
		$i+=1;
		if(empty($v)){
			$image_descs[$k]=$max_image_descs+$i;
		}
	}
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if(!empty($_SESSION['thumb_img_id'.$_SESSION['store_user_id']])){
            $_SESSION['thumb_img_id'.$_SESSION['store_user_id']] = array_merge($thumb_img_id , $_SESSION['thumb_img_id'.$_SESSION['store_user_id']]);
        }else{
            $_SESSION['thumb_img_id'.$_SESSION['store_user_id']] = $thumb_img_id;
        }
}
...
function delete_goods($goods_id)
{
    ...
    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('store_products') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
...
function get_linked_goods($goods_id)
{
    ...
    if ($goods_id == 0)
    {
        $sql .= " AND lg.admin_id = '$_SESSION[store_user_id]'";
    }
    $row = $GLOBALS['db']->getAll($sql);
...
function get_group_goods($goods_id)
{
    ...
    if ($goods_id == 0)
    {
        $sql .= " AND gg.admin_id = '$_SESSION[store_user_id]'";
    }
...
function get_goods_articles($goods_id)
{
    ...
    if ($goods_id == 0)
    {
        $sql .= " AND g.admin_id = '$_SESSION[store_user_id]'";
    }
    $row = $GLOBALS['db']->getAll($sql);
...
function check_goods_product_exist($goods_id, $conditions = '')
{
    ...
    $sql = "SELECT goods_id
            FROM " . $GLOBALS['ecs']->table('store_products') . "
            WHERE goods_id = '$goods_id'
            " . $conditions . "
            LIMIT 0, 1";
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('store_products'). " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . $GLOBALS['ecs']->table('store_products') . " AS g
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['ecs']->table('store_products') . " WHERE product_id = '$product_id'";
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0)
{
	...
	if($GLOBALS['store_id'])
	{
		$where .= " AND store_id = '$GLOBALS[store_id]' ";
	}
    ...
    if (empty($product_id))
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['ecs']->table('store_products') ."
...
function check_product_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $store_id = 0)
{
    ...
    $sql="SELECT goods_id FROM ". $GLOBALS['ecs']->table('goods')." WHERE goods_sn='$product_sn' AND user_id = '$ru_id'";
