...
else
{
    ...
    if (!empty($_POST['action']) && $_POST['action'] == 'get_pwd')
    {
        ...
        $sql = 'SELECT id, stores_pwd FROM ' .$ecs->table('store_user').
               " WHERE stores_user = '$admin_username' AND email = '$admin_email'";
    ...
    elseif (!empty($_POST['action']) && $_POST['action'] == 'reset_pwd')
    {
        ...
        //更新管理员的密码
		$ec_salt=rand(1,9999);
        $sql = "UPDATE " .$ecs->table('store_user'). "SET stores_pwd = '".md5(md5($new_password).$ec_salt)."',`ec_salt`='$ec_salt' ".
               "WHERE id = '$adminid'";
        $result = $db->query($sql);
        if ($result)
        {
