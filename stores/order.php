    ...
    if ($pick_code) {
        ...
        if ($order_info['pay_status'] != 2 && !in_array($payment['pay_code'], array('bank', 'cod'))) {
            ...
            $sql = " UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET order_status = '" . OS_SPLITED . "', pay_status = '" . PS_PAYED . "', shipping_status = '" . SS_RECEIVED . "' " .
                    ", shipping_time = '" . gmtime() . "', confirm_take_time = '" . gmtime() . "' " .
...
function store_order_list($ru_id = 0, $store_id = 0) {
	...
	switch($filter['order_type']){
		...
		break;
		case 3:
		$where .= " AND store_id = $store_id  AND shipping_status = 1 AND (order_status < 2 OR order_status = 5) ";//已发货
		break;
    }
    ...
    $sql = 'SELECT so.*, o.order_id, o.order_sn, o.add_time, o.consignee, o.order_status, o.shipping_status, o.pay_status, so.store_id, ' .
            "(" . order_amount_field('o.') . ") AS total_fee,o.shipping_fee,o.shipping_name,o.invoice_no " .
...
function get_order_goods_list($order_id = 0) {
    $sql = " SELECT og.order_id, og.goods_id, og.goods_name, og.goods_price, og.goods_number, g.goods_thumb, og.goods_sn, og.goods_attr,og.product_id FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og " .
