...
$search_type = isset($_REQUEST['store_search_cmt']) && !empty($_REQUEST['store_search_cmt']) ? intval($_REQUEST['store_search_cmt']) : 0; //搜索类型
...
if ($_REQUEST['act'] == 'advanced_search')
{
    ...
    if ($search_type == 0) {
        $smarty->assign('best_goods', get_recommend_goods('best', '', $region_id, $area_info['region_id'], $area_city, 0, 1));
...
else
{
    ...
    if ($search_type == 0) { //搜索商品
        ...
        if (!empty($_REQUEST['keywords'])) {
            ...
            foreach ($arr AS $key => $val) {
                ...
                if ($val) {
                    ...
                    show_message($_LANG['not_search_goods']);
                }
            }
            $keywords .= ')';
        ...
        if ($cat['grade'] > 1) {
            ...
            if ($children) {
                $row_children = "($children OR " . get_extension_goods($children) . ") AND ";
            } else {
                $row_children = '';
            }
            ...
            $sql = "SELECT min(IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price))) AS min, " .
                    ...
                    $leftJoin .
                    " WHERE " .$row_children. ' g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_show = 1 AND g.is_alone_sale = 1 ' . $keywords . $tag_where;
            //获得当前分类下商品价格的最大值、最小值
            ...
            if ($children) {
                $price_grade_children = "($children OR " . get_extension_goods($children) . ") AND ";
            } else {
                $price_grade_children = '';
            }
            ...
            $sql = "SELECT (FLOOR((IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) - $row[min]) / $dx)) AS sn, COUNT(*) AS goods_num  " .
                    ...
                    " LEFT JOIN " . $GLOBALS['ecs']->table('warehouse_area_goods') . " AS wag ON g.goods_id = wag.goods_id AND wag.region_id = '$area_id' $where_area " .
                    $leftJoin .
                    " WHERE " .$price_grade_children. ' g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_show = 1 AND g.is_alone_sale = 1 ' . $keywords . $price_grade_where .
                    $tag_where .
                    " GROUP BY sn ";
        ...
        if ($children) {
            $brands_list_children = "($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), $cat_keys))) . ") AND";
        } else {
            $brands_list_children = '';
        }
        ...
        $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num " . $brand_select .
                ...
                " WHERE " .$brands_list_children. " b.is_show = 1 " .
            ...
            foreach ($cat_filter_attr AS $key => $value) {
                ...
                if ($attributeInfo) {
                    ...
                    $sql = "SELECT a.attr_id, MIN(a.goods_attr_id) AS goods_id, a.attr_value AS attr_value, a.color_value FROM " .
                            ...
                            " WHERE a.attr_id = '$value' GROUP BY a.attr_value";
        ...
        $sql = "SELECT pa.act_id, pa.act_name, pa.start_time, pa.end_time, " .
                ...
                "WHERE (g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_show = 1 AND g.is_alone_sale = 1 $attr_in $cou_goods_where " . //优惠券商品条件 bylu
                ...
                " GROUP BY g.goods_id ORDER BY goods_fen, $goods_sort $order"; //增加表别名 g. by wanganlin
        ...
        while ($row = $db->FetchRow($res)) {
            ...
            if (!defined('THEME_EXTENSION')) {
                //商品运费by wu 
                $shippingFee = goodsShippingFee($row['goods_id'], $region_id, $area_info['region_id'], $area_city, $region);
        ...
        $smarty->assign('best_goods', get_recommend_goods('best', '', $region_id, $area_info['region_id'], $area_city, 0, 1));
        $smarty->assign('guess_goods', get_guess_goods($_SESSION['user_id'], 1, 1, 7, $region_id, $area_info['region_id'], $area_city));         //猜你喜欢
        ...
        if ($display == 'list') { //店铺列表
            ...
            $size = 10;
            $store_shop_list = get_store_shop_list(1, $keywords, $count, $size, $page, $sort, $order, $region_id, $area_id, $area_city);
            ...
            $shop_goods_list = get_store_shop_goods_list($keywords, $size, $page, $sort, $order, $region_id, $area_id, $area_city);
        ...
        if(defined('THEME_EXTENSION')){
            ...
            $store_best_list = get_shop_goods_count_list(0, $region_id, $area_id, $area_city, 1, 'store_best');
