...
function load_config()
{
    ...
    if ($data === false || empty($data)) {
        ...
        if (empty($arr['integrate_code'])) {
            $arr['integrate_code'] = 'ecshop'; // 默认的会员整合插件为 dscshop
        }
...
function is_promotion($goods_id)
{
    ...
    if (!empty($goods_id)) {
        ...
        $sql = 'SELECT ga.act_id, ga.act_type, g.goods_sn FROM ' . $GLOBALS['ecs']->table('goods_activity') . " AS ga, " .
            $GLOBALS['ecs']->table('goods') . " g" .
            " WHERE ga.goods_id = g.goods_id AND ga.goods_id " . db_create_in($goods_id) . " GROUP BY ga.goods_id";
        ...
        $sql = 'SELECT ga.act_id, ga.act_type, g.goods_sn FROM ' . $GLOBALS['ecs']->table('goods_activity') . " AS ga, " .
            $GLOBALS['ecs']->table('package_goods') . " pg," .
            $GLOBALS['ecs']->table('goods') . " g" .
            " WHERE ga.act_id = pg.package_id AND pg.goods_id = g.goods_id AND pg.goods_id " . db_create_in($goods_id) . " GROUP BY pg.goods_id";
...
function is_seckill($goods_id)
{
    ...
    if (!empty($goods_id)) {
        $gmtime = gmtime();
        $sql = 'SELECT GROUP_CONCAT(g.goods_sn) AS goods_sn FROM ' . $GLOBALS['ecs']->table('seckill_goods') . " AS sg, " .
            $GLOBALS['ecs']->table('goods') . " AS g " .
            " WHERE sg.goods_id = g.goods_id AND sg.goods_id " . db_create_in($goods_id);
...
{
    $sql = "SELECT (SELECT count(*) FROM " . $GLOBALS['ecs']->table('snatch_log') . " AS lg2  WHERE lg2.snatch_id = '$id') as num, lg.user_id, lg.bid_price, lg.bid_time FROM " .
        $GLOBALS['ecs']->table('snatch_log') .
        " AS lg WHERE lg.snatch_id = '$id' ORDER BY num, lg.bid_price ASC, lg.bid_time ASC LIMIT 1";
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, $order_type = 0, $deposit_fee = 0)
{
    ...
    if ($change_desc && $order_type) {
        ...
        if (count($change_desc_arr) >= 2) {
            ...
            if (!empty($order_sn)) {
                $sql = "SELECT order_id, main_order_id FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_sn = '$order_sn' LIMIT 1";
            ...
            if ($ordor_res) {
                if ($ordor_res['main_order_id'] > 0) {  //操作无效或取消订单时，先查询该订单是否有主订单
                    ...
                    if ($ordor_res && $ordor_res['order_id'] > 0) {
...
function get_package_info($id, $path = '')
{
    ...
    $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, pg.product_id, " .
        " g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, g.is_real, g.cloud_id, g.goods_number AS stock_number, " .
...
function get_for_purchasing_goods($start_date = 0, $end_date = 0, $goods_id = 0, $user_id = 0, $extension_code = '', $attr_id = '')
{
    ...
    $sql = "SELECT SUM(og.goods_number) AS goods_number FROM " . $GLOBALS['ecs']->table('order_goods') . " as og, " . $GLOBALS['ecs']->table('order_info') . " as oi " .
...
function cate_history($size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $area_city = 0, $ship = 0, $self = 0)
{
    ...
    if (!empty($_COOKIE['ECS']['list_history'])) {
        ...
        $sql = 'SELECT b.brand_name,g.is_shipping, g.goods_sn, g.brand_id, g.goods_id, g.goods_name, g.user_id, g.goods_thumb,g.sales_volume, g.user_id, msi.self_run, g.model_attr, ' .
            ...
            "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, " .
            "g.product_price, g.product_promote_price, g.promote_start_date, g.promote_end_date " .
            ' FROM ' . $GLOBALS['ecs']->table('goods') . " as g " .
            " left join " . $GLOBALS['ecs']->table('brand') . " as b" . " on g.brand_id = b.brand_id " .
            $leftJoin .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
            "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            " WHERE $where AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show = 1 group by g.goods_id  ORDER BY $sort $order";
    ...
    if (!empty($_COOKIE['ECS']['sec_history'])) {
        ...
        $sql = 'SELECT b.brand_name,g.is_shipping, g.goods_sn, g.brand_id, g.goods_id, g.goods_name, g.user_id, g.goods_thumb,g.sales_volume, g.user_id, g.model_attr, sg.id, sg.sec_price ' .
            ' FROM ' . $GLOBALS['ecs']->table('goods') . " as g " .
            " left join " . $GLOBALS['ecs']->table('brand') . " as b" . " on g.brand_id = b.brand_id " .
            " LEFT JOIN " . $GLOBALS['ecs']->table('seckill_goods') . " as sg on sg.goods_id = g.goods_id " .
            " WHERE $where AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_show = 1 AND g.is_delete = 0 group by g.goods_id ";
        $res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size);
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            $sec_arr[$row['goods_id']]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
            ...
            $sec_arr[$row['goods_id']]['rz_shopName'] = get_shop_name($row['user_id'], 1); //店铺名称
            $sec_arr[$row['goods_id']]['user_id'] = $row['user_id'];

            $build_uri = array(
                'urid' => $row['user_id'],
                'append' => $sec_arr[$row['goods_id']]['rz_shopName'],
            );
            ...
            $sec_arr[$row['goods_id']]['pictures'] = get_goods_gallery($row['goods_id']); // 商品相册
            ...
            /*  @author-bylu 判断当前商家是否允许"在线客服" */
            $shop_information = get_shop_name($seller_id); //通过ru_id获取到店铺信息;
            $sec_arr[$row['goods_id']]['is_IM'] = $shop_information['is_IM']; //平台是否允许商家使用"在线客服";
            if ($seller_id == 0) {
                //判断平台是否开启了IM在线客服
                if ($GLOBALS['db']->getOne("SELECT kf_im_switch FROM " . $GLOBALS['ecs']->table('seller_shopinfo') . "WHERE ru_id = 0", true)) {
...
function cate_history_count()
{
    ...
    if (!empty($_COOKIE['ECS']['list_history'])) {
        ...
        $sql = 'SELECT b.brand_name, g.brand_id, g.goods_id, g.goods_name, g.goods_thumb, g.shop_price, g.promote_price, g.is_promote FROM ' . $GLOBALS['ecs']->table('goods') . " as g left join " . $GLOBALS['ecs']->table('brand') . " as b" . " on g.brand_id = b.brand_id " .
            " WHERE $where AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show = 1";
...
//店铺搜索 start
function get_store_shop_list($libType = 0, $keywords = '', $count = 0, $size = 16, $page = 1, $sort = 'shop_id', $order = 'DESC', $warehouse_id = 0, $area_id = 0, $area_city = 0, $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '', $goods_type = 0)
{
    ...
    if (!empty($keywords)) {
        ...
        /* 店铺名称 start */
        $where .= " AND IF(s.check_sellername = 0, i.shoprz_brandName LIKE '%$keywords%' OR i.shopNameSuffix LIKE '%$keywords%' OR  CONCAT(i.shoprz_brandName, i.shopNameSuffix) LIKE '%$keywords%', IF(s.check_sellername = 1, i.rz_shopName LIKE '%$keywords%', s.shop_name LIKE '%$keywords%')) ";
        $leftjoin = " LEFT JOIN " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS s ON s.ru_id = i.user_id ";

        $sql = "SELECT GROUP_CONCAT(i.user_id) AS user_id FROM " . $GLOBALS['ecs']->table('merchants_shop_information') . " AS i " . $leftjoin . " WHERE " . $where;
        ...
        $sql = "SELECT GROUP_CONCAT(user_id) AS user_id FROM " . $GLOBALS['ecs']->table('goods') . " WHERE 1 $goods_keywords $reviewGodds AND user_id > 0 AND is_delete = 0 AND is_on_sale = 1 ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($goods_type == 0) {
            $arr[$row['shop_id']]['goods_count'] = get_shop_goods_count_list($row['user_id'], $warehouse_id, $area_id, $area_city); //商品数量
            $arr[$row['shop_id']]['goods_list'] = get_shop_goods_count_list($row['user_id'], $warehouse_id, $area_id, $area_city, 1); //商品数量
        }
...
function get_store_shop_goods_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id, $area_city = 0)
{
...
//商家商品数量
function get_shop_goods_count_list($user_id, $warehouse_id, $area_id, $area_city = 0, $type = 0, $isType = '', $show_type = 0, $limit = 0)
{
...
//商家商品数量
function get_shop_goods_cmt_list($user_id, $warehouse_id, $area_id, $area_city, $price_min, $price_max, $page, $size, $sort, $order)
{
...
function get_category_brands_ad($cat_id)
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name,  b.brand_logo, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag " .
        ...
        "GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC LIMIT 0,20";
...
function get_merchant_category($cat_id = 0, $ru_id = 0)
{
    ...
    $sql = "SELECT cat_id, cat_name FROM " . $GLOBALS['ecs']->table('merchants_category') .
        " WHERE parent_id = '$cat_id' AND user_id = '$ru_id' ORDER BY sort_order, cat_id DESC";
    $res = $GLOBALS['db']->getAll($sql);
...
function search_brand_list($goods_id = 0, $ru_id = null)
{
    ...
    $brand_list = read_static_cache('pin_brands');
    ...
    if ($brand_list === false && empty($keyword)) {
        ...
        write_static_cache('pin_brands', $brand_list);
    } else {
        ...
        if ($brand_list) {
            foreach ($brand_list AS $key => $val) {
                ...
                } elseif (empty($letter) && !empty($keyword)) {
                    if ($keyword == $brand_list[$key]['brand_name']) {
                        $arr[$key] = $val;
                    }
                } else {
                    $arr = $brand_list;
    ...
    if ($store_list) {
        ...
        foreach ($store_list as $key => $row) {
            ...
            if (is_spec($is_spec) == true) {
                $products = get_warehouse_id_attr_number($row['goods_id'], $spec_arr, $row['ru_id'], 0, 0, 0, '', $row['id']); //获取属性库存
...
function get_filter_goods_list($filter = array('goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0, 'mer_ids' => ''), $size = 10, $page = 1, $sort = "sort_order", $order = "ASC", $warehouse_id = 0, $area_id = 0, $area_city = 0, $type = '')
{
    $leftJoin = '';
    $where = " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show = 1 ";
    ...
    $sql = " SELECT g.goods_id, g.goods_name, g.goods_thumb, g.promote_start_date, g.promote_end_date, g.product_price, g.product_promote_price, " .
        ...
        " FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
        $leftJoin .
        " WHERE 1 $where $where_area GROUP BY g.goods_id ORDER BY $sort $order ";
    //分页
    $start = ($page - 1) * $size;
...
function get_top_presale_goods($goods_id, $cat_id)
{
    ...
    $sql = "SELECT a.*, g.goods_thumb, g.goods_img, g.goods_name, g.shop_price, g.market_price, g.sales_volume, s.* FROM " . $GLOBALS['ecs']->table('presale_activity') . " AS a "
        ...
        . "WHERE a.cat_id = '$cat_id' AND g.is_on_sale = 0 AND a.review_status = 3 AND g.is_show = 1 AND a.start_time <= '$now' AND a.end_time >= '$now' AND g.goods_id <> '$goods_id' ORDER BY g.click_count DESC LIMIT 5 ";
...
{
    $sql = "SELECT merchants_comment_number,goods_id FROM" . $GLOBALS['ecs']->table('intelligent_weight') .
    ...
    if ($res['goods_id']) {
        ...
        return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('intelligent_weight'), $num, 'INSERT');
    }
}
...
{
    $sql = "SELECT user_attention_number,goods_id FROM" . $GLOBALS['ecs']->table('intelligent_weight') .
        " WHERE goods_id = '$goods_id' LIMIT 1";
    $res = $GLOBALS['db']->getRow($sql);
...
//获取指定分类及上级所以分类是否存在隐藏不显示，存在返回0，不存在返回1
...
FROM" . $GLOBALS['ecs']->table('category') .
            " AS a LEFT JOIN " . $GLOBALS['ecs']->table('category') . " AS b  ON b.cat_id = a.parent_id LEFT JOIN" .
            $GLOBALS['ecs']->table('category') . " AS c ON c.cat_id = b.parent_id where a.cat_id = '$cat_id'";
...
function push_template_curl($code = '', $pushData = [], $order_url = '', $user_id = 0, $shop_url = '')
{
    if (!empty($pushData)) {
        ...
        curlGet_weixin($api_url); // curl请求接口
    }
}
