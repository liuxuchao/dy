...
function get_seller_commission_info($ru_id = 0){
    $sql = "SELECT ms.server_id, ms.commission_model, ms.suppliers_percent, mp.percent_value FROM " .$GLOBALS['ecs']->table('merchants_server'). " AS ms " .
            ...
            "WHERE ms.user_id = '$ru_id' LIMIT 1";
...
function bill_detail_list($type = 0, $bill = array()){
    ...
    if ($result === false) {
        ...
        $sql = "SELECT sbo.*, (" . order_amount_field('sbo.') . ") AS total_fee, (" . order_activity_field_add('sbo.') . ") AS activity_fee, (" . order_commission_field('sbo.') . ") AS commission_total_fee,  " .
                ...
                "WHERE IF(sbo.chargeoff_status > 0, sbo.bill_id = '" .$filter['bill_id']. "', $where)" . $limit;     
    ...
    foreach ($row as $key => $value)
    {
        ...
        $should_amount = number_format($should_amount, 2, '.', '');       //应结佣金
        $row[$key]['gain_commission'] = number_format($gain_commission, 2, '.', '');   //收取佣金
        $row[$key]['should_amount'] = $should_amount;       //应结佣金
        
        $format_should_amount = $should_amount + $goods_rate['should_amount'];
        $format_should_amount = number_format($format_should_amount, 2, '.', '');       //应结佣金
        ...
        if ($bill) {
            $sql = "SELECT log_id FROM " . $GLOBALS['ecs']->table('seller_account_log') . " AS sal, " .
                    $GLOBALS['ecs']->table('seller_bill_order') . " AS sbo " .
                    " WHERE sal.is_paid = 1 AND sal.order_id = sbo.order_id" .
                    " AND sal.ru_id = '" . $value['seller_id'] . "'" .
                    " AND sal.order_id = '" . $value['order_id'] . "'" .
                    " AND sal.log_type = 2 AND add_time < '" . $bill['chargeoff_time'] . "'";
    ...
    foreach ($row as $key => $value)
    {
        ...
        if(empty($value['chargeoff_status'])){
            ...
            if ($value['chargeoff_status'] == 1) {
                $where_order = " AND " . db_create_in(array(OS_UNCONFIRMED), "sbo.shipping_status") .
                        " AND " . db_create_in(array(PS_UNPAYED), "sbo.pay_status") .
                        " AND " . db_create_in(array(OS_RETURNED), "sbo.order_status");

                $sql = "SELECT SUM(sbo.return_amount) AS return_amount, SUM(sbo.return_shippingfee) AS return_shippingfee FROM " . 
                        $GLOBALS['ecs']->table('seller_bill_order') . " AS sbo " .
                        " WHERE sbo.bill_id = '" . $value['id'] . "'" . $where_order . " LIMIT 1";
                ...
                $detail_other = array(
                    ...
                    'chargeoff_time' => $value['chargeoff_time'],
                    'commission_model' => $value['commission_model'],
                    'order_status' => OS_RETURNED
                );
        ...
        if ($value['chargeoff_status'] == 1 || $value['chargeoff_status'] == 3) {
            ...
            if ($settle_accounts > 0 && $value['should_amount'] >= $settle_accounts) {
...
function bill_goods_list(){
    ...
    foreach ($row as $key => $value)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('return_goods'). " AS rg, " . 
                ...
                " WHERE rg.ret_id = o.ret_id AND rg.goods_id = '" .$value['goods_id']. "' ".
                " AND o.refound_status = 1 " . 
                " AND o.order_id = '" .$value['order_id']. "'";
        if($GLOBALS['db']->getOne($sql)){
            $row[$key]['is_return'] = 1;
        }else{
            $row[$key]['is_return'] = 0;
...
function bill_notake_order_list(){
    ...
    if ($result === false) {
        ...
        $sql = "SELECT o.*, (" . order_amount_field('o.') . ") AS total_fee, ".
                ...
                "sbo.goods_amount FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                "LEFT JOIN" . $GLOBALS['ecs']->table('order_action') . " AS oa ON o.order_id = oa.order_id " .
                "LEFT JOIN" . $GLOBALS['ecs']->table('seller_bill_order') . " AS sbo ON o.order_id = sbo.order_id " .
...
function get_order_take_brokerage($seller_id = 0, $order_id = 0, $chargeoff_time = 0){
    ...
    $sql = "SELECT SUM(sal.amount) FROM " .$GLOBALS['ecs']->table('seller_account_log') ." AS sal, ".
            $GLOBALS['ecs']->table('seller_bill_order') ." AS sbo ". 
            " WHERE $where AND sal.is_paid = 1 AND sal.order_id = sbo.order_id AND sal.log_type = 2 AND add_time > '$chargeoff_time'";
...
function get_alone_goods_rate($order_id = 0, $ru_id = 0, $order = array()){
    ...
    if ($order) {
        ...
        $sql = "SELECT SUM(sbg.goods_price * sbg.goods_number - sbg.dis_amount) AS total_fee " .
    ...
    $sql = "SELECT rec_id, sbg.commission_rate, (sbg.goods_price * sbg.goods_number - sbg.dis_amount) AS goods_amount " .
            "FROM " . $GLOBALS['ecs']->table('seller_bill_goods'). " AS sbg WHERE $where AND sbg.commission_rate > 0";
    ...
    if ($goods_list) {
        ...
        foreach($row['goods'] as $key => $goods){
            ...
            $sql = "SELECT (actual_return - return_shipping_fee) AS actual_return FROM " .$GLOBALS['ecs']->table('return_goods')." AS rg, ". 
                    $GLOBALS['ecs']->table('order_return')." AS ro". 
                    " WHERE rg.rec_id = '$key' AND rg.ret_id = ro.ret_id";
            $actual_return = $GLOBALS['db']->getOne($sql);
...
function get_bill_days_number($seller_id = 0, $cycle = 0) {
    ...
    if ($mintime && $maxtime) {
        ...
        if ($min_year < $max_year) {
            ...
            if ($day_number && $server && $day_number > $server['day_number']) {
