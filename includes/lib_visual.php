...
function getAlbumList($album_id = 0) {
    ...
    if($adminru['suppliers_id'] > 0) {
        ...
        if ($adminru['ru_id'] > 0 || $album_info['suppliers_id'] > 0) {
            if ($album_info['ru_id'] != $adminru['ru_id'] || $album_info['suppliers_id'] > 0) {
...
function resetBarnd($brand_id=array(),$table='goods',$category='goods_id'){
    if($brand_id){
        if($table == 'goods'){
            ...
            $sql = "SELECT sg.id FROM".$GLOBALS['ecs']->table('goods')." AS g "
                    . "LEFT JOIN ".$GLOBALS['ecs']->table('seckill_goods')." AS sg ON sg.goods_id=g.goods_id "
                    . "LEFT JOIN ".$GLOBALS['ecs']->table('seckill')."AS s ON s.sec_id=sg.sec_id".$where;
        }
...
function getcat_atr($cat_id = 0 , $where = '') {
    ...
    $sql = 'SELECT a.article_id,a.title ' .
            ...
            ' WHERE a.is_open = 1 AND a.' . $condition .$where.
            ' ORDER BY a.article_type DESC, a.sort_order ASC, a.article_id DESC '.$limit;
