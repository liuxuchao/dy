...
function check_money($log_id, $money)
{
    if(is_numeric($log_id))
    {
        ...
        $sql = 'SELECT order_id, order_amount, surplus FROM ' . $GLOBALS['ecs']->table('order_info') .
              " WHERE order_id IN (" .$pay['order_id']. ") LIMIT 1";
            ...
            if ($pay_log && $pay_log['is_paid'] == 0) {
                ...
                if ($pay_log['order_type'] == PAY_ORDER) {
                    ...
                    foreach ($order_id_arr as $o_key => $o_val) 
                    {
                        ...
                        $store_result = array();
                        if ($stores_order && $stores_order['store_id'] > 0 && $order_id > 0) {
                            if ($order['mobile']) {
                        ...
                        if ($GLOBALS['_CFG']['sms_type'] >= 1 && $order_id > 0) {
