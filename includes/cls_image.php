...
class cls_image
{
     ...
     */
    function upload_image($upload, $dir = '', $img_name = '', $steps = 0, $name = '', $type = '', $tmp_name = '', $error = '', $size = '')
    {
        $upload_type = 0;
        if($dir && is_array($dir)){
        ...
        if (empty($dir))
        {
            /* 创建当月目录 */
            $dir = date('Ym');
            $dir = ROOT_PATH . $this->images_dir . '/' . $dir . '/' . "admin_" . $admin_id . "/";
        }
        else
        {
            /* 创建目录 */
     ...
     * @return boolen
     */
    function get_width_to_height($path, $image_width = 0, $image_height = 0) {
