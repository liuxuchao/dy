...
function get_admin_id() {
    ...
    if ($count > 1) {
        ...
        } elseif (judge_supplier_enabled() && $real_path == SUPPLLY_PATH) {
...
function get_admin_ru_id() {
    ...
    $sql = "SELECT ru_id, rs_id, suppliers_id, user_name FROM " . $GLOBALS['ecs']->table('admin_user') . " WHERE user_id = '$admin_id' LIMIT 1"; //卖场-rs_id
    return $GLOBALS['db']->getRow($sql);
}
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['steps_style'] == 1) {
            ...
            if ($ec_shop_bid > 0 || (isset($brand_m['brand_id']) && $brand_m['brand_id'] > 0)) { //更新品牌数据
                ...
                if ($_SESSION['user_id'] > 0 && !isset($_REQUEST['searchBrandZhInput']) && !isset($_REQUEST['searchBrandEnInput'])) {
                    if (empty($brandLogo)) {
...
function get_warehouse_area_goods_list($goods_id = 0) {
    $sql = "SELECT wag.a_id, wag.region_id, wag.city_id, wag.region_sn, wag.region_number, wag.region_price, wag.region_promote_price, wag.region_sort, rw.region_name, rw.parent_id, wag.give_integral, wag.rank_integral, wag.pay_integral FROM " .
            $GLOBALS['ecs']->table('warehouse_area_goods') . " as wag, " . $GLOBALS['ecs']->table('region_warehouse') . " as rw " .
            " WHERE wag.region_id = rw.region_id AND wag.goods_id = '$goods_id' ORDER BY wag.region_sort, rw.region_id asc";
    $res = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($newInfo as $key => $info) {
        ...
        do {
            ...
            if ($cartValOther && $new_orderId > 0) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('value_card_record'), $cartValOther, 'INSERT');
            }
...
function get_order_goods_toInfo($order_id = 0) {
    $sql = "SELECT og.goods_id, g.goods_name, g.goods_thumb, oi.extension_code as oi_extension_code, og.goods_number, og.goods_price, og.goods_attr, "  .
            " og.extension_code as og_extension_code, og.goods_name AS extension_name, oi.order_sn FROM " . $GLOBALS['ecs']->table('order_goods') . " as og " .
...
function get_search_city_true($region_id){
    ...
    $where = "region_id = '$region_id' AND region_type = 2";
...
//商品运费by wu start
function goodsShippingFee($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $region = array(), $seckill_price = '') {
    ...
    if($goods_list && $shipping_code != 'cac'){
        ...
        foreach($goods_list as $key=>$row){
            if($row['freight'] && $row['is_shipping'] == 0){
                ...
                if($row['is_shipping'] == 1){//如果设置商品免运费,初始化运费模板所包含的运费
                    $area_shipping['shipping_fee'] = 0;
                }
                $freight += 1;
            }
        }
...
function get_user_coupons_list($user_id = '', $is_use = false, $total = '', $cart_goods = false, $user = true, $cart_ru_id = -1, $act_type = 'user',$province = 0) {
    ...
    if ($is_use && isset($total) && $cart_goods) {
        ...
        foreach($res as $key=>$row){
            ...
            $sql = "SELECT c.*, cu.uc_id, cr.region_list, cu.cou_money AS uc_money FROM " . $GLOBALS['ecs']->table("coupons_user") . " AS cu " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table("coupons") . " AS c ON cu.cou_id = c.cou_id " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table("coupons_region") . " AS cr ON cr.cou_id = c.cou_id " .
        ...
        $leftjoin = "";
        if($act_type == 'cart'){
            $where .= " AND c.cou_end_time > $time AND 'cou_start_time' < $time";
        }else{
        ...
        $sql = " SELECT c.*, cu.*, c.cou_money AS cou_money, cu.cou_money AS uc_money, cr.region_list $select FROM " .
        ...
        if($res){
            foreach($res as $key => $row){
				...
				// 处理价格显示整数
				$res[$key]['cou_money'] = isset($row['cou_money']) ? intval($row['cou_money']) : '';
				$res[$key]['uc_money'] = isset($row['uc_money']) ? intval($row['uc_money']) : '';
				$res[$key]['order_coupons'] = isset($row['order_coupons']) ? intval($row['order_coupons']) : '';
...
function get_coupons($uc_id = 0, $select = array(), $user_id = 0, $seller_id = -1){
    ...
    $sql = " SELECT cu.cou_money AS uc_money, $select FROM " . $GLOBALS['ecs']->table('coupons_user') . " cu " .
            " LEFT JOIN " . $GLOBALS['ecs']->table('coupons') . " c ON c.cou_id = cu.cou_id " .
            " WHERE cu.uc_id = '$uc_id' " .$where. " AND c.cou_end_time > $time GROUP BY cu.uc_id LIMIT 1 ";
...
function get_user_value_card($user_id, $cart_goods, $cart_value) {
    ...
    if ($user_id > 0) {
        ...
        $sql = " SELECT t.name, t.use_condition, t.spec_goods, t.spec_cat,vc.end_time, vc.card_money, vc.vid FROM " . $GLOBALS['ecs']->table('value_card') . " AS vc " . " LEFT JOIN " . $GLOBALS['ecs']->table('value_card_type') . " AS t  ON vc.tid = t.id " . $where;
        ...
        foreach ($result as $k => $v) {
            if (empty($v['use_condition'])) {//全部自营
                $arr[$k]['vc_id'] = $v['vid'];
                $arr[$k]['name'] = $v['name'];
                $arr[$k]['end_time'] = local_date("Y-m-d H:i:s",$v['end_time']);
