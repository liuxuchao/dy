...
$shipping_lang = ROOT_PATH.'languages/' .$GLOBALS['_CFG']['lang']. '/shipping/yunda.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 配送方式插件的代码必须和文件名保持一致 */
    $modules[$i]['code']    = 'yunda';

    $modules[$i]['version'] = '1.0.0';

    /* 配送方式的描述 */
    $modules[$i]['desc']    = 'yunda_desc';
    ...
    /* 打印单背景 */
    $modules[$i]['print_bg'] = '/images/receipt/dly_yunda.jpg';
   ...
   /* 打印快递单标签位置信息 */
    ...
    /* 帐号申请方式 */
    $modules[$i]['kdniao_account'] = 1;	
	
    /* 快递编码 */
    $modules[$i]['kdniao_code'] = "YD";	
...
class yunda
{
    ...
    function query($invoice_sn)
    {
        $str = '<form style="margin:0px" methods="post" '.
            'action="http://ykjcx.yundasys.com/go.php" name="queryForm_' .$invoice_sn. '" target="_blank">'.
            '<input type="hidden" name="wen" value="' .$invoice_sn. '" />'.
