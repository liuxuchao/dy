...
$shipping_lang = ROOT_PATH.'languages/' .$GLOBALS['_CFG']['lang']. '/shipping/huitong.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 配送方式插件的代码必须和文件名保持一致 */
    $modules[$i]['code']    = 'huitong';

    $modules[$i]['version'] = '1.0.0';

    /* 配送方式的描述 */
    $modules[$i]['desc']    = 'huitong_desc';
    ...
    /* 打印单背景 */
    $modules[$i]['print_bg'] = '/images/receipt/dly_huitong.jpg';
...
class huitong
{
    ...
    function query($invoice_sn)
    {
        $str = '<form style="margin:0px" methods="post" '.
            'action="http://www.htky365.com/Bill/Track" name="queryForm_' .$invoice_sn. '" target="_blank">'.
            '<input type="hidden" name="code" value="' .$invoice_sn. '" />'.
