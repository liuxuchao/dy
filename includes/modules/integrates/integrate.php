...
class integrate
{
     ...
     * @return int
     */
    function add_user($username, $password, $registerMode_info, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        /* 将用户添加到整合方 */
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop))
        {
            ...
            if ($col)
            {
                ...
                $sql = "DELETE FROM " . $GLOBALS['ecs']->table('connect_user') . " WHERE " . db_create_in($col, 'user_id'); //删除第三方会员绑定表
                $GLOBALS['db']->query($sql);

                if (file_exists(MOBILE_WECHAT)) {
                    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('wechat_user') . " WHERE " . db_create_in($col, 'ect_uid'); //删除微信通会员
                    $GLOBALS['db']->query($sql);
                }

                if (file_exists(MOBILE_DRP)) {
                    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') . " SET drp_parent_id = 0 WHERE " . db_create_in($col, 'drp_parent_id'); //将删除用户的下级的 drp_parent_id 改为0
                    $GLOBALS['db']->query($sql);
                    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('drp_affiliate_log') . " WHERE " . db_create_in($col, 'user_id'); //删除微信通会员
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('drp_log') . " WHERE " . db_create_in($col, 'user_id'); //删除微信通会员
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('drp_shop') . " WHERE " . db_create_in($col, 'user_id'); //删除微信通会员
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('drp_type') . " WHERE " . db_create_in($col, 'user_id'); //删除微信通会员
                    $GLOBALS['db']->query($sql);
    ...
    function check_user($username, $password = '')
    {
        ...
        $sql = "SELECT " . $this->field_id .
                " FROM " . $this->table($this->user_table) .
                " WHERE " . $field_name . " AND " . $this->field_pass . " ='" . $this->compile_password(array('password' => $password)) . "'";
     ...
     * @return void
     */
    function set_cookie($username = '', $remember = null, $is_oath = 1 , $unionid = '') {
        ...
        if (empty($username)) {
            ...
            if ($is_oath == 1) {
                ...
                $sql = "SELECT u.user_id, u.user_name, u.password, u.email " .$select. " FROM " . $GLOBALS['ecs']->table('users') . " AS u " .
                        $leftJoin .
                        " WHERE $where LIMIT 1";
