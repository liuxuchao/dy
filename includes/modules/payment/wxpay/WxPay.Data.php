...
class WxPayDataBase
{
	...
	public function __construct($payment = array()){
		$this->config = array_merge($this->config, $payment);
	}
	...
	**/
	...
	public function MakeSign()
	{
		...
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".$this->config['wxpay_key'];
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
     ...
     * @throws WxPayException
     */
	public static function Init($payment, $xml)
	{	
		$obj = new self($payment);
		$obj->FromXml($xml);
		//fix bug 2015-06-29
