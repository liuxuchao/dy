...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function refund($payment, $inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid($payment['wxpay_appid']);//公众账号ID
		$inputObj->SetMch_id($payment['wxpay_mchid']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		...
		$result = WxPayResults::Init($payment, $response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, ROOT_PATH . 'data/certificate/wxpay/apiclient_cert.pem');
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, ROOT_PATH . 'data/certificate/wxpay/apiclient_key.pem');
		}
		//post提交方式
