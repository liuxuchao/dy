 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: lib_statistical.php 17217 2011-01-19 06:29:08Z liubo $
 */
if (!defined('IN_ECS')) {
    die('Hacking attempt');
}
...
function statistical_field_valid_fee(){
    //return "SUM(" . order_commission_field('o.') . ")"; //区分此有效不为商家有效结算金额
...
function shop_sale_stats($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT spi.ru_id FROM " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS spi " .
                $leftJoin .
        ...
        $sql = "SELECT spi.ru_id, " .
                ...
                " FROM " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS spi " .
                $leftJoin .
                $where_msi . $where_o . $groupBy .
...
function shop_area_stats($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT spi.district FROM " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS spi " .
                $leftJoin .
...
function get_statistical_sale($search_data = array()){
    ...
    $sql = " SELECT FROM_UNIXTIME(o.add_time+$time_diff,'%y-%m-%d') AS day, " .
        ...
        " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o" . ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . $no_main_order . $where_data . ' GROUP BY day ORDER BY day ASC ';
    $result = $GLOBALS['db']->query($sql);
...
function user_sale_stats($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT o.user_id FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                $leftJoin .
...
function user_area_stats($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        /* 查询语句 */
        $where_o = " WHERE 1 AND order_status <> '".OS_INVALID."' ";//剔除无效订单
        ...
        /* 记录总数 */
        $sql = "SELECT o.district FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                $leftJoin .
                $where_o . $groupBy;
...
function industry_analysis($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT c.cat_id FROM " . $GLOBALS['ecs']->table('category') . " AS c " .
                $leftJoin .
...
function get_statistical_today_trend($search_data = array()){
    ...
    $sql = " SELECT FROM_UNIXTIME(o.add_time,'%y-%m-%d-%H') AS hour, " .
        statistical_field_order_num() . " AS volume, " .
        statistical_field_sale_money() . " AS money " .
        " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o" . ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . $no_main_order . $where_data . ' GROUP BY hour ORDER BY hour ASC ';
    $result = $GLOBALS['db']->query($sql);
    ...
    for ($i = 0; $i < $hour_num; $i++) {
        //$hour = local_date("y-m-d-H", local_strtotime(" - " . ($hour_num - $i) . " hours"));
...
function get_statistical_today_sale($search_data = array()){
    ...
    $sql = " SELECT " .
        ...
        ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . $no_main_order . $where_data;
...
function member_account_stats($page = 0)
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT u.user_id FROM " . $GLOBALS['ecs']->table('users') . " AS u " .
                $leftJoin .
...
function balance_stats($page = 0)
{
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['formated_consumption_money'] = price_format(-$value['consumption_money']); //取正
        $row[$key]['formated_cash_money'] = price_format(-$value['cash_money']); //取正
...
function merchants_commission_list() {
    ...
    if ($result === false) {
        ...
        $sql = "SELECT mis.*, msf.companyName, msf.company_adress, msf.company_contactTel FROM " . $GLOBALS['ecs']->table("merchants_shop_information") . " as mis " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('merchants_steps_fields') . " as msf on mis.user_id = msf.user_id " . $left_join . $where .
                " ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'];
        
        $sql .= " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
        set_filter($filter, $sql); $this_sql = $sql; //记录本次
    } else {
        $sql = $result['sql'];
        $filter = $result['filter'];
    }
    ...
    set_filter($filter, $this_sql); //记录本次
...
function settlement_total_stats()
{
    ...
    if($store_num > 0){
        ...
        $sql = " SELECT SUM(" . order_commission_field('o.') . ") AS total_amount " .
            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o WHERE $child_query_ru_id " . db_create_in($seller);
    ...
    if ($seller) {
        for ($i = 0; $i < count($seller); $i++) {
            /* 微分销 */
            if (file_exists(MOBILE_DRP)) {
                $is_drp_settlement = merchants_is_settlement($seller[$i], 1); //已结算佣金金额  liu
                $no_drp_settlement = merchants_is_settlement($seller[$i], 0); //未结算佣金金额  liu
                $is_settlement = $is_drp_settlement['all_price'];
                $no_settlement = $no_drp_settlement['all_price'];
            } else {
                $is_settlement = merchants_is_settlement($seller[$i], 1); //已结算佣金金额  liu
                $no_settlement = merchants_is_settlement($seller[$i], 0); //未结算佣金金额  liu
            }
            ...
            /* 统计代码 start */
            $valid = get_merchants_order_valid_refund($seller[$i]); //订单有效总额
            $refund = get_merchants_order_valid_refund($seller[$i], 1); //订单退款总额
