...
class cls_template
{
     ...
     *
     * //ecmoban模板堂 -- zhuo $not_cache = 'yes'
     * @return  void
     */
    function display($filename, $cache_id = '', $not_cache = 'yes')
    {
        $this->_seterror++;
     ...
     * @return  sring
     */
    function fetch($filename, $cache_id = '', $not_cache = 'yes')
    {
        if (!$this->_seterror)
        {
            error_reporting(E_ALL ^ E_NOTICE);
     ...
     */
    function make_compiled($filename, $not_cache)
    {
    ...
    function html_radios($arr)
    {
        ...
        foreach ($options AS $key => $val)
        {
            $out .= $key == $checked ? "<label><input type=\"radio\" class=\"checkbox\" name=\"$name\" value=\"$key\" checked><span>{$val}</span></label>"
                : "<label><input type=\"radio\" class=\"checkbox\" name=\"$name\" value=\"$key\"><span>{$val}</span></label>";
        }
