...
function get_user_order_coupons($order_id, $ru_id = 0, $type = 0)
{
    ...
    $sql = "SELECT cu.*, c.cou_name, c.cou_money, cu.cou_money AS uc_money FROM " . $GLOBALS['ecs']->table('coupons_user') . " AS cu, " .
...
{
    $sql = "SELECT oi.extension_id,og.*, (og.goods_price * og.goods_number) AS subtotal,g.shop_price, g.is_shipping, g.goods_weight AS goodsweight, g.goods_img, g.goods_thumb, " .
        "g.goods_cause, g.give_integral, g.is_shipping FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og " .
...
{
    $sql = "SELECT goods_number FROM" . $GLOBALS['ecs']->table('intelligent_weight') .
...
{
    $sql = "SELECT return_number, goods_number FROM" . $GLOBALS['ecs']->table('intelligent_weight') .
        " WHERE goods_id = '$goods_id' LIMIT 1";
...
function cart_goods($type = CART_GENERAL_GOODS, $cart_value = '', $ru_type = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $consignee = '', $store_id = 0)
{
    ...
    $sql = "SELECT c.warehouse_id, c.area_id, c.area_city, c.rec_id, c.user_id, c.goods_id, c.ru_id, g.cat_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, g.default_shipping, g.goods_weight as goodsweight, " .//储值卡指定分类 liu
        ...
        "c.parent_id, c.group_id, c.act_id, pa.deposit, g.is_shipping, g.freight, g.tid, g.shipping_fee, g.brand_id, g.cloud_id, g.cloud_goodsname " .
    ...
    foreach ($arr as $key => $value) {
        ...
        if ($value['extension_code'] == 'package_buy') {
            ...
            if ($store_id > 0) {
                ...
                $products = get_warehouse_id_attr_number($value['goods_id'], $value['goods_attr_id'], $goodsInfo['ru_id'], 0, 0, 0, '', $store_id);//获取属性库存
                $attr_number = $products['product_number'];
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $stages_qishu = '-1', $store_id = 0, $take_time = '', $store_mobile = '', $act_id = 0)
{ //ecmoban模板堂 --zhuo $warehouse_id
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    if ($num > 0) {
        ...
        $sql = "SELECT goods_number,stages_qishu,rec_id FROM " . $GLOBALS['ecs']->table('cart') .
            ...
            " AND extension_code <> 'package_buy' " .
            " AND rec_type = '" . CART_GENERAL_GOODS . "' AND group_id='' AND is_gift = 0 AND warehouse_id = '$warehouse_id' AND store_id = '$store_id'"; //by mike add
 ...
 * @return  boolean
 */
function addto_cart_combo($goods_id, $num = 1, $spec = array(), $parent = 0, $group = '', $warehouse_id = 0, $area_id = 0, $area_city = 0, $goods_attr = '')
{
 ...
 * @return  array
 */
function get_cart_goods($cart_value = '', $type = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    $goods_where = " AND g.is_delete = 0 ";
    if ($type == CART_PRESALE_GOODS) {
        $goods_where .= " AND g.is_on_sale = 0 ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($xiangouInfo['is_xiangou'] == 1 && $nowTime > $start_date && $nowTime < $end_date) {
            ...
            if ($orderGoods['goods_number'] >= $xiangouInfo['xiangou_num']) {
                ...
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') .
                    " SET goods_number = 0 WHERE rec_id='$row[rec_id]'";
                $GLOBALS['db']->query($sql);
            } else {
                if ($xiangouInfo['xiangou_num'] > 0) {
                    if ($xiangouInfo['is_xiangou'] == 1 && $orderGoods['goods_number'] + $row['goods_number'] > $xiangouInfo['xiangou_num']) {
                        ...
                        $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') .
                            " SET goods_number = '$cart_Num' WHERE rec_id='$row[rec_id]'";
                        $GLOBALS['db']->query($sql);
                        $row['goods_number'] = $cart_Num;//更新限购购物车内商品数量
                    }
                }
            }
        }
        ...
        if (count($currency_format) > 1) {
            $goods_price = trim(get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $row['warehouse_id'], $row['area_id'], $area_city, 0, 0, $add_tocart), $currency_format[0]);
            $cart_price = trim($row['goods_price'], $currency_format[0]);
        } else {
            $goods_price = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $row['warehouse_id'], $row['area_id'], $area_city, 0, 0, $add_tocart);
        ...
        if ($warehouse_id && $row['extension_code'] != 'package_buy') {
            ...
            $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $goodsInfo['user_id'], $warehouse_id, $area_id, $area_city);
            $attr_number = $products['product_number'];
...
function order_query_sql($type = 'finished', $alias = '')
{
    ...
    if ($type == 'bill_confirm_take') {
        return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_RETURNED_PART, OS_SPLITED, OS_SPLITING_PART)) .
            " AND {$alias}shipping_status " . db_create_in(array(SS_RECEIVED)) .
            " AND {$alias}pay_status " . db_create_in(array(PS_PAYED, PS_UNPAYED)) . " ";
    }
...
function return_order_info($ret_id = 0, $order_sn = '', $order_id = 0)
{
    ...
    if ($order) {
        ...
        $all_goods_number = $GLOBALS['db']->getOne($sql);//获取订单总数
        ...
        $order['goods_thumb'] = get_image_path($order['goods_id'], $order['goods_thumb']);
...
function cart_by_favourable($merchant_goods)
{
    ...
    $id_list = array();
    $list_array = array();
    $cart_value['act_sel_id'] = empty($_SESSION['cart_value']) ? [] : explode(',', $_SESSION['cart_value']);//购物车默认选中
                                ...
                                if ($goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0) {// 活动商品
                                    if (isset($goods_row) && $goods_row) {
                                        ...
                                        switch ($fav_row['act_type']) {
                                            case 0:
                                                $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type_txt'] = $GLOBALS['_LANG']['With_a_gift'];
                                                $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type_ext_format'] = intval($fav_row['act_type_ext']); // 可领取总件数
                                                break;
                                            case 1:
                                                $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type_txt'] = $GLOBALS['_LANG']['Full_reduction'];
                                                $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type_ext_format'] = number_format($fav_row['act_type_ext'], 2); // 满减金额
                                                break;
                                            case 2:
                                                $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type_txt'] = $GLOBALS['_LANG']['discount'];
                                        ...
                                        if ($merchant_goods[$key]['new_list'][$fav_row['act_id']]['available'] && $fav_row['act_type'] == 2) {//折扣显示折扣金额
                                            $merchant_goods[$key]['new_list'][$fav_row['act_id']]['goods_fav_amount'] = price_format($merchant_goods[$key]['new_list'][$fav_row['act_id']]['cart_fav_amount'] * floatval(100 - intval($fav_row['act_type_ext'])) / 100);
                                        }
                                        // 购物车中已选活动赠品数量
                                        ...
                                        if (empty($goods_row['act_id'])) {
                                            update_cart_goods_fav($goods_row['rec_id'], $_SESSION['user_id'], $fav_row['act_id']);
                                        }
...
function cart_favourable_amount($favourable, $act_sel_id = array('act_sel_id' => '', 'act_pro_sel_id' => '', 'act_sel' => ''), $ru_id = -1)
{
    ...
    if (!empty($act_sel_id['act_sel_id'])) {
        $fav_where .= "AND c.rec_id " . db_create_in($act_sel_id['act_sel_id']);
    } else {
        $fav_where .= " AND c.rec_id = 0 ";
    }
    //ecmoban模板堂 --zhuo end
    ...
    if ($order_info) {
        ...
        if ($commission_info['commission_model']) {
            ...
            if (file_exists(MOBILE_DRP)) {
                ...
                if ($order_goods_commission['commission'] > 0) {
...
function checked_pay_Invalid_order($pay_effective_time = 0)
{
    ...
    $time = gmtime() - $pay_effective_time;//获取当前时间 - 支付时效
    $where = " AND (select count(*) from " . $GLOBALS['ecs']->table('order_info') . " as oi2 where oi2.main_order_id = oi.order_id) = 0 ";  //主订单下有子订单时，则主订单不显示
    $sql = "SELECT oi.order_id , oi.order_sn , oi.user_id,oi.surplus,oi.money_paid,oi.bonus_id,oi.integral_money,oi.bonus FROM " . $GLOBALS['ecs']->table('order_info') .
        " AS oi WHERE oi.add_time < $time " . order_query_sql('unpay_unship', 'oi.') . $where .
...
function get_favourable_info($goods_id = 0, $ru_id = 0, $goods = [])
{
    $gmtime = gmtime();
    if ($ru_id > 0) {
        $ext_where = '';
...
* 更新购物车商品促销活动
*/
function update_cart_goods_fav($rec_id, $user_id, $act_id)
{
    $sql = " UPDATE " . $GLOBALS['ecs']->table('cart') . " SET act_id = '$act_id' WHERE rec_id = '$rec_id' AND user_id = '$user_id' ";
    return $GLOBALS['db']->query($sql);
}
