...
function insert_cart_info($type = 0, $num = 0) {
    ...
    if($type > 0 || $type == 4){
        $sql = 'SELECT c.*,g.goods_thumb,g.goods_id,c.goods_number,c.goods_price' .
                ...
                " WHERE " . $c_sess . " AND rec_type = '" . CART_GENERAL_GOODS . "' and c.stages_qishu='-1' " . $limit; //这里限定不查出分期购商品 bylu;
    ...
    $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
            ...
            " WHERE " . $sess_id . " AND rec_type = '" . CART_GENERAL_GOODS . "' and stages_qishu='-1'"; //这里限定不查出分期购商品 bylu;
...
function get_ad_posti($name = '', $ad_type = '') {
    ...
    $sql = "SELECT ap.ad_width, ap.ad_height, ad.ad_id, ad.ad_name, ad.ad_code, ad.ad_link, ad.link_color, ad.start_time, ad.end_time, ad.ad_type, ad.goods_name FROM " . 
            ...
            " WHERE " . $name . " ad.media_type = 0 AND '$time' > ad.start_time AND '$time' < ad.end_time AND ad.enabled=1 AND ap.theme = '" . $GLOBALS['_CFG']['template'] . "'";
    $res = $GLOBALS['db']->getAll($sql);
...
function insert_wholesale_cart_info()
{
    ...
    if(judge_supplier_enabled()){
        $sql = 'SELECT wc.rec_id, wc.goods_name, wc.goods_attr_id,wc.goods_price, w.goods_thumb,w.goods_id,wc.goods_number,wc.goods_price' .
