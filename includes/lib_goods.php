...
function get_top10($cats = 0, $presale = '', $ru_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, SUM(og.goods_number) as goods_number,g.comments_number, g.market_price, g.market_price, ' .
        ...
        " WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show=1 $where $top10_time ";
 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '', $warehouse_id = 0, $area_id = 0, $area_city = 0, $ru_id = 0, $rec_type = 0, $presale = '')
{
    ...
    if (empty($type_goods[$type])) {
        ...
        $sql = 'SELECT g.goods_id, ' . $goods_hnb_files . ' g.is_promote, b.brand_name,g.sort_order ' .
            ...
            $leftJoin .
            ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show=1 AND (' . $goods_hot_new_best . ')' . $tag_where .
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min = 0, $max = 0, $ext = '', $warehouse_id = 0, $area_id = 0, $area_city = 0, $num = 0, $start = 0)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.comments_number ,g.sales_volume, g.model_attr,' .
        ...
        "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_show = 1 AND g.is_delete = 0 ' . $where . $brand_where . $price_where . $ext;
 ...
 * @return  void
 */
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0, $select = array())
{
    $time = gmtime();
    $tag = array();
    //ecmoban模板堂 --zhuo start    
    ...
    $sql = 'SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        "WHERE g.goods_id = '$goods_id' AND g.is_delete = 0 LIMIT 1";
    ...
    if (!empty($attr)) {
        foreach ($attr['lnk'] AS $key => $val) {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.sales_volume,g.comments_number,g.goods_img, g.shop_price AS org_price, ' .
                ...
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                "WHERE a.attr_id = '$key' AND g.is_on_sale=1 AND g.is_show=1 AND a.attr_value = '$val[value]' AND g.goods_id <> '$_REQUEST[id]' " .
...
function get_floor_ajax_goods($cat_id = 0, $num = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $goods_ids = '', $user_id = 0, $search_type = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.cat_id, g.goods_name, g.market_price, g.model_attr, ' .
        ...
        " AS goods_sort FROM " . $GLOBALS['ecs']->table('intelligent_weight') . " AS iw WHERE iw.goods_id = g.goods_id LIMIT 1) AS goods_sort,g.sort_order," .
        ' IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' .
        "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, " .
        "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
        ' g.is_promote, g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.product_price, g.product_promote_price ' .
        ...
        'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        'WHERE 1 ' . $tag_where . ' ORDER BY IF(goods_sort > 0, goods_sort + g.sort_order, g.sort_order) DESC , g.goods_id DESC';
 ...
 * @return  void
 */
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0, $order_rule = '', $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    //ecmoban模板堂 --zhuo start
    $leftJoin = '';
    $tag_where = '';
...
function group_buy_info($group_buy_id, $current_num = 0, $path = '')
{
    ...
    /* 视频路径格式化 */
    $group_buy['goods_video_path'] = !empty($group_buy['goods_video']) ? get_image_path($goods_id, $group_buy['goods_video']) : '';
...
function auction_info($act_id, $config = false, $path = '')
{
    ...
    if ($auction['status_no'] > 1) {
        $sql = "SELECT COUNT(*)" .
            ...
            " AND pay_status " . db_create_in(array(PS_PAYED));
 ...
 * @return  array
 */
function goods_info($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $select = array(), $attr_id = '', $presale = 0)
{
    $where_area = '';
    if ($GLOBALS['_CFG']['area_pricetype'] == 1) {
        $where_area = " AND wag.city_id = '$area_city'";
    ...
    if (!empty($row)) {
        ...
        $row['goods_price'] = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $warehouse_id, $area_id, $area_city, 0, $presale, $add_tocart);
 ...
 * @return  array
 */
function get_goods_fittings($goods_list = array(), $warehouse_id = 0, $area_id = 0, $area_city = 0, $rev = '', $type = 0, $goods_equal = array())
{
    $fitts_goodsList = '';
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (empty($row['goods_attr_id'])) {
            ...
            $goods = get_goods_info($row['goods_id'], $warehouse_id, $area_id, $area_city);
            $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $row['goods_name'], $warehouse_id, $area_id, $area_city);
        ...
        $arr[$temp_index]['properties'] = get_goods_properties($row['goods_id'], $warehouse_id, $area_id, $area_city, 0, $row['goods_attr_id']);
 ...
 * @return  array
 */
function get_goods_fittings_info($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $rev = '', $type = 0, $fittings_goods = 0, $fittings_attr = array())
{
    ...
    $sql = 'SELECT g.goods_id,g.goods_number,g.sales_volume,g.goods_name, g.goods_thumb, g.goods_img, g.user_id, g.comments_number, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($GLOBALS['_CFG']['add_shop_price'] == 0) {
            ...
            $goods_price = get_final_price($fittings_goods, 1, true, $fittings_attr, $warehouse_id, $area_id, $area_city, 0, 0, $add_tocart);
        } else {
        ...
        $products = get_warehouse_id_attr_number($goods_id, $row['goods_attr_id'], $row['goods_name'], $warehouse_id, $area_id, $area_city);
        $attr_number = $products['product_number'];
        ...
        $arr[$temp_index]['properties'] = get_goods_properties($goods_id, $warehouse_id, $area_id, $area_city, $row['goods_attr_id']);
 ...
 * @return      array
 */
function get_products_info($goods_id, $spec_goods_attr_id, $warehouse_id = 0, $area_id = 0, $area_city = 0, $store_id = 0)
{
    //ecmoban模板堂 --zhuo start
    $model_attr = get_table_date("goods", "goods_id = '$goods_id'", array('model_attr'), 2);
    //ecmoban模板堂 --zhuo end
...
function get_cat_id_goods_list($cat_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $num = '')
{
    ...
    $sql = 'Select g.goods_id, g.cat_id,c.parent_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.is_shipping, ' .
        "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
        "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price," .
        ...
        "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show = 1 $tag_where ";
...
function get_seckill_goods($seckillid = array())
{
    ...
    $soon = array();
    $sql = " SELECT g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.goods_name, sg.id, sg.sec_price, sg.sec_num, sg.sec_limit, stb.begin_time, stb.end_time, s.sec_id, s.acti_title, s.acti_time,stb.id AS stb_id FROM " . $GLOBALS['ecs']->table('seckill_goods') .
 ...
 *                  status          状态：
 */
function seckill_info($seckill_id, $current_num = 0, $path = '', $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    $where = '';
    if (empty($path)) {
        $where = " AND b.review_status = 3 ";
    ...
    $sql = " SELECT g.*, sg.*, s.*, stb.begin_time, stb.end_time, " .
        "IF(g.model_inventory < 1, g.goods_number, IF(g.model_inventory < 2, wg.region_number, wag.region_number)) as goods_number " .
        "FROM " . $GLOBALS['ecs']->table('seckill_goods') . " AS sg " .
...
function sec_goods_stats($sec_id, $begin_time = '', $end_time = '')
{
    ...
    $sql = "SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods " .
        "FROM " . $GLOBALS['ecs']->table('order_info') . " AS o, " .
        $GLOBALS['ecs']->table('order_goods') . " AS g " .
        " WHERE o.order_id = g.order_id " . $where .
