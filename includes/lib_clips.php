...
function get_collection_goods($user_id, $record_count, $page, $pageFunc, $size = 10)
{
    ...
    $sql = 'SELECT g.goods_thumb, g.user_id, g.goods_id, g.goods_name, g.market_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' .
            ...
            'g.promote_start_date, g.promote_end_date, g.model_attr, c.rec_id, c.is_attention, c.add_time, g.product_price, g.product_promote_price ' .
...
function get_collection_brands($user_id, $record_count, $page, $pageFunc, $size = 5)
{
    ...
    $sql = "SELECT cb.*, b.brand_name, b.brand_logo FROM " .$GLOBALS['ecs']->table('collect_brand'). " AS cb, " . $GLOBALS['ecs']->table('brand'). " AS b " .
            " WHERE cb.brand_id = b.brand_id AND cb.user_id = '$user_id' ORDER BY rec_id DESC " .$limit;
    $res = $GLOBALS['db'] -> getAll($sql);
...
function get_collection_store($user_id, $record_count, $page, $pageFunc, $size = 5)
{
    ...
    $sql = "SELECT m.shoprz_brandName, m.shopNameSuffix, m.shop_id, s.shop_logo, c.ru_id, c.add_time, s.kf_type, s.kf_ww, s.kf_qq, s.kf_tel, brand_thumb  FROM " .$GLOBALS['ecs']->table('collect_store') ." as c, " .$GLOBALS['ecs']->table('seller_shopinfo') ." as s, " .
    ...
    foreach($res as $key=>$row)
    {
        ...
        $store_list[$key]['is_IM'] = $shop_information['is_IM']; //平台是否允许商家使用"在线客服";
...
function get_user_store_goods_list($user_id, $region_id, $area_id, $type = '', $sort = 'last_update', $order = 'DESC', $limit = 'LIMIT 0,10') {
    ...
    $sql = 'SELECT g.goods_thumb, g.goods_id, g.goods_name, g.market_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' .
            "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
            "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, " .
            'g.promote_start_date, g.promote_end_date, g.model_attr, g.product_price, g.product_promote_price ' .
            ' FROM ' . $GLOBALS['ecs']->table('goods') . " AS g " .
            $leftJoin .
            " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
            "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            " WHERE g.user_id = '$user_id' " . $type . " $where ORDER BY g." . $sort . " $order " . $limit;
    $res = $GLOBALS['db']->getAll($sql);
...
function insert_user_account($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid,deposit_fee)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,'$surplus[deposit_fee]')";
    $GLOBALS['db']->query($sql);
...
//        $sql .= " AND pay_code <> 'balance' ";
        $sql .= " AND pay_code not in('balance','chunsejinrong','onlinepay')";//不包含"余额支付","白条支付","在线支付按钮"(充值时不应该包含) bylu;
    }
...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $rows['pay_status']       = ($rows['is_paid'] == 0) ? $GLOBALS['_LANG']['un_confirmed'] : (($rows['is_paid'] == 2) ? $GLOBALS['_LANG']['is_dismiss'] : $GLOBALS['_LANG']['is_confirmed']);
...
function get_user_prompt ($user_id)
{
    ...
    $sql = "SELECT act_id, goods_name, end_time " .
            ...
            " WHERE act_type = '" . GAT_SNATCH . "' AND review_status = 3" .
    ...
    $sql = "SELECT act_id, goods_name, end_time " .
            "FROM " . $GLOBALS['ecs']->table('goods_activity') .
            " WHERE act_type = '" . GAT_AUCTION . "' AND review_status = 3" .
            " AND (is_finished = 1 OR (is_finished = 0 AND end_time <= '$now'))";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
...
function get_user_order_comment_list($user_id, $type = 0, $sign=0, $order_id = 0, $size = 0, $start = 0){
    ...
    if($type == 1){
        $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('order_goods') ." AS og ".
                ...
                "WHERE og.goods_id = g.goods_id AND oi.user_id = '$user_id' $where ORDER BY oi.add_time DESC";
        $arr = $GLOBALS['db']->getOne($sql);
    }else{
        $sql = "SELECT og.rec_id, og.order_id, og.goods_id, og.goods_name, oi.add_time, g.goods_thumb, g.goods_product_tag, og.ru_id,oi.order_sn,og.goods_number,og.goods_price,og.goods_attr FROM ".
...
function get_order_goods_comment($goods_id, $rec_id, $user_id){
    $sql = "SELECT c.comment_id, c.comment_rank, c.content, c.id_value, c.order_id, c.rec_id, c.user_id, c.goods_tag FROM " .$GLOBALS['ecs']->table('comment').
