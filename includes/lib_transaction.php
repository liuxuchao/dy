...
function get_user_orders($user_id, $record_count, $page, $is_delete = 0, $where = '', $order = '', $handle_type = 0, $pagesize = 10)
{
    ...
    if ($order && is_object($order)) {
        ...
        if ($type == 'text') { //订单编号、商品编号、商品名称模糊查询
    ...
    /* 取得订单列表 */
    $arr    = array();
    $sql = "SELECT IFNULL(bai.is_stages,0) is_stages, oi.main_order_id, oi.consignee,oi.pay_name, oi.order_id, oi.order_sn, oi.pay_id, " .
            " oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time, " .
            "(oi.goods_amount + oi.shipping_fee + oi.insure_fee + oi.pay_fee + oi.pack_fee + oi.card_fee + oi.tax - oi.discount - oi.coupons) AS total_fee, " .
            "oi.invoice_no, oi.shipping_name, oi.tel, oi.email, oi.address, oi.province, oi.city, oi.district, oi.user_id, oi.surplus, oi.money_paid, " .
            "oi.bonus_id, oi.integral_money, oi.bonus " .
            ...
            $act .
            " ORDER BY oi.add_time DESC " . $limit;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if  ($pay_effective_time > 0 && $row['pay_status'] == PS_UNPAYED && in_array($row['order_status'],array(OS_UNCONFIRMED, OS_CONFIRMED)) 
                && in_array($row['shipping_status'],array(SS_UNSHIPPED, SS_PREPARING)) && !in_array($pay_code,array('bank','cod','post'))) {
            //计算时效性时间戳
            $pay_effective_time = $pay_effective_time * 60;
            
            //如果订单超出时间设为无效
            if(gmtime() - $row['add_time'] > $pay_effective_time){
        ...
        if ($GLOBALS['_CFG']['open_delivery_time'] == 1) {
            if ($row['order_status'] == OS_SPLITED && $row['shipping_status'] == SS_SHIPPED && $row['pay_status'] == PS_PAYED) { //发货状态
                $delivery_time = $row['shipping_time'] + 24 * 3600 * $auto_delivery_time;
        ...
        } else if ($row['order_status'] == OS_SPLITED || $row['order_status'] == OS_CONFIRMED || $row['order_status'] == OS_RETURNED_PART) {
            // 对配送状态的处理
            if ($row['shipping_status'] == SS_SHIPPED) {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'original_handler_return' => $row['original_handler_return'],
                        ...
                        'auto_delivery_data' => $row['auto_delivery_data'],
...
{	
    ...
    $sql = " SELECT o.order_id, o.mobile, o.order_sn, o.add_time, o.inv_payee, o.inv_content, o.invoice_type, g.user_id FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
            ...
            " WHERE o.user_id = '$user_id' AND (SELECT count(*) FROM " . $GLOBALS['ecs']->table('order_info') . " AS oi2 WHERE oi2.main_order_id = o.order_id) = 0  GROUP BY o.order_id ORDER BY o.add_time DESC " . $limit;
