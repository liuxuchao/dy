    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        ...
        foreach ($res AS $row) {
            ...
            foreach ($brands_list AS $key => $val) {
                ...
                $brands_list[$temp_key]['url'] = build_uri('category', array('cid' => $cat_id, 'bid' => $val['brand_id']), $cat['cat_name']);
 ...
 * @return      string
 */
function get_goods_attr_info_new($arr, $type = 'pice', $warehouse_id = 0, $area_id = 0, $area_city = 0) {
    $attr = '';
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($row['is_real'] == 0) {
            ...
            if (file_exists($filename)) {
                ...
                if (!empty($_LANG[$row['extension_code'] . '_link'])) {
                    $row['goods_name'] = $row['goods_name'] . sprintf($_LANG[$row['extension_code'] . '_link'], $row['goods_id'], $order_id);
                }
            }
        }
        ...
        if ($row['model_attr'] == 1) {
            ...
            if ($GLOBALS['_CFG']['area_pricetype'] == 1) {
                $where_area = " AND city_id = '" .$row['area_city']. "'";
            }

            $table_products = "products_area";
            $type_files = " and area_id = '" . $row['area_id'] . "' $where_area";
        } else {
...
function get_user_default($user_id = 0) {
    ...
    if ($user_id && $rank = $GLOBALS['db']->getRow($sql)) {
        ...
        $info['rank_sort'] = get_user_rank_sort($rank['rank_id']);
    } else {
...
function get_default_user_orders($user_id, $record_count, $where = '', $page = 1) {
    /* 取得订单列表 */
    $arr = array();
    $sql = "SELECT oi.main_order_id, oi.extension_code as oi_extension_code, oi.consignee, oi.order_id, oi.order_sn, oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time, " .
            ...
            " ORDER BY oi.add_time DESC limit 0,5";
    ...
    foreach ($res as $key => $row) {
        ...
        $sql = "SELECT ru_id, goods_id, extension_code as og_extension_code FROM " .$GLOBALS['ecs']->table('order_goods'). " WHERE order_id = '" .$row['order_id']. "' LIMIT 1";
        ...
        $arr[$key]['og_extension_code'] = $order_goods['og_extension_code'];
...
function get_shop_name($ru_id = 0, $type = 0) {
    ...
    $sql = "SELECT concat(shoprz_brandName, shopNameSuffix) as shop_name, shoprz_brandName, shopNameSuffix, rz_shopName, is_IM, self_run, shop_close FROM " . $GLOBALS['ecs']->table('merchants_shop_information') . " WHERE user_id = '$ru_id' LIMIT 1";
    $shop_information = $GLOBALS['db']->getRow($sql);

    if (empty($shop_information)) {
        $shop_information['shop_name'] = $shopinfo['shop_name'];
        $shop_information['shopNameSuffix'] = '';
        $shop_information['shoprz_brandName'] = '';
        $shop_information['rz_shopName'] = '';
        $shop_information['self_run'] = 0;
    }
...
//查询商品上架下架时间
function get_auto_manage_info($goods_id = 0, $type = '') {
...
function get_discuss_all_list($goods_id = 0, $dis_type = 0, $reply_page = 1, $size = 40, $revType = 0, $sort = 'add_time', $did = 0) {
    ...
    if ($dis_type == 4 || $dis_type == 0) {//晒单贴
        ...
        foreach ($res as $key => $row) {
            if($dis_type == 0){
                ...
                $arr1[$key]['reply_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('discuss_circle') . "WHERE parent_id = '" . $row['dis_id'] . "' AND review_status = 3 ");   
                ...
                $arr[$key]['reply_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('discuss_circle') . "WHERE parent_id = '" . $row['dis_id'] . "' AND review_status = 3 ");
    ...
    if($dis_action == 1)
    {
        ...
        $sql = "SELECT DC1.dis_id, DC1.dis_type, DC1.dis_title, DC1.user_name, DC1.add_time, DC1.dis_browse_num, u.nick_name, " .
                "(SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('discuss_circle') . " AS DC2" . " WHERE DC2.parent_id = DC1.dis_id AND DC2.review_status = 3) AS reply_num" .
                " FROM " . $GLOBALS['ecs']->table('discuss_circle') . " AS DC1" .
				" LEFT JOIN ". $GLOBALS['ecs']->table('users') . " AS u ON u.user_id = DC1.user_id " .
				" WHERE $where AND DC1.parent_id = 0 AND DC1.goods_id = '$goods_id' AND DC1.dis_id <> '$did' AND DC1.review_status = 3 ORDER BY $sort DESC " . $limit;
        $res = $GLOBALS['db']->getAll($sql);
...
function get_discuss_type_count($goods_id, $dis_type = 0) {
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('discuss_circle') . " WHERE $where AND parent_id = 0 AND goods_id = '$goods_id' AND review_status = 3";
    return $GLOBALS['db']->getOne($sql);
}
...
function get_history_goods($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0) {
    ...
    if (!empty($_COOKIE['ECS']['history'])) {
        ...
        $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, g.goods_thumb, g.goods_img, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' .
                ...
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                $leftJoin .
                " WHERE $where AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_show = 1 order by INSTR('" . $_COOKIE['ECS']['history'] . "',g.goods_id) limit 0,10";
...
function presale_info($presale_id, $current_num = 0, $user_id = 0, $path = '') {
    ...
    $sql = "SELECT b.*, b.cat_id AS pa_catid,g.goods_name, g.shop_price, g.goods_desc, g.user_id, g.goods_id, g.goods_product_tag, g.cat_id, b.review_status, b.review_content, " .
            "g.xiangou_start_date, g.xiangou_end_date, g.xiangou_num, g.is_xiangou, b.user_id AS ru_id, g.brand_id " .
 ...
 */
function get_guess_goods($user_id, $history = 0, $page = 1, $limit = 5, $warehouse_id = 0, $area_id = 0, $area_city = 0) {
    ...
    if (empty($history)) {//用户中心
        ...
        if ($order_arr) {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.user_id, ' .
                    "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
                    "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price," .
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                    " WHERE $link_cats_str AND g.goods_id NOT IN ($finished_goodsStr) AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $tag_where ORDER BY g.sales_volume DESC  LIMIT 8";
        ...
        if (!empty($_COOKIE['ECS']['history'])) {
            ...
            $sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, g.model_attr, " .
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                    " WHERE $where_cat $goods_str AND g.is_on_sale = 1 AND g.is_hot = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.sales_volume > 0 $tag_where LIMIT $start, $limit";
            $query = $GLOBALS['db']->query($sql);
        }
    }
    //默认
    if ((empty($query) || count($query) < $limit) && $history == 1) {
        $sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, g.user_id, g.model_attr, " .
                ...
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp " .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                " WHERE  (g.sales_volume > 0 OR g.is_hot = 1) AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $tag_where ORDER BY g.sort_order,g.sales_volume DESC LIMIT $start, $limit";
        $query = $GLOBALS['db']->query($sql);
    }

    $guess_goods = array();
    if ($query) {
        while ($query && $row = $GLOBALS['db']->fetch_array($query)) {
            if ($row['promote_price'] > 0) {
                $promote_price = bargain_price($row['promote_price'], $row['promote_start_date'], $row['promote_end_date']);
            } else {
                $promote_price = 0;
...
function get_self_point($district, $point_id = 0, $limit = 100) {
    ...
    $sql = "SELECT ar.shipping_area_id,ar.region_id ,sp.id as point_id,sp.name,sp.mobile,sp.address,sp.anchor,sa.shipping_id,ss.shipping_code, ss.shipping_name ,cr.parent_id as city FROM " . $GLOBALS['ecs']->table('shipping_point') . " AS sp " .
...
function sms_ali($params = array(), $send_time = '', $test = array()) {
    ...
    if ($sms_type == 2) {
        ...
        $result = array(
            'TemplateContent' => !empty($test['temp_content'])? $test['temp_content']:$row['temp_content'], //短信模板内容
            'TemplateParam' => json_encode($smsParams),
            'PhoneNumbers' => $params['mobile_phone']
        );
    } else {
...
function shipping_info($shipping, $select = array())
{
    ...
    if(is_array($shipping)){
        ...
        if(isset($shipping['shipping_code'])){
            $where = "AND shipping_code = '" .$shipping['shipping_code']. "'";
        }elseif(isset($shipping['shipping_id'])){
            $where = "AND shipping_id = '" .$shipping['shipping_id']. "'";
        }

    }else{
        $where = "AND shipping_id = '$shipping'";
    }
    ...
    $sql = "SELECT " .$select. " FROM " . $GLOBALS['ecs']->table('shipping') .
            " WHERE 1 $where AND enabled = 1 LIMIT 1";
    $row = $GLOBALS['db']->getRow($sql);
...
//获取商品的智能权重数据
function get_manual_intervention($goods_id){
	if($goods_id){
		$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('intelligent_weight') . " WHERE goods_id = '$goods_id'";
		$res = $GLOBALS['db']->getRow($sql);
	}
	return $res;
}
